...
# + TRADE MANAGEMENT BOARD v1 (Professional Edition)
# ====================================================================
...
# - NEW: Trade Management Board v1 (7-analyst governance + Chairman)
# ====================================================================
...
# ============================================================
# DYNAMIC TRADE MANAGER (Legacy - kept for compatibility)
# ============================================================
...
def tg_board_decision(symbol, verdict, reason):
    icon = "🧠"
    send_once(f"{icon} <b>BOARD: {verdict}</b>\n📊 {symbol}\n📌 {reason[:150]}", f"board_{symbol}_{verdict}", 30)
...
class PositionSnapshot:
    ...
    def to_dict(self):
        return {
            ...
            "sl_price": self.sl_price, "partial_closed": self.partial_closed,
            "stale": self.stale, "updated_at": self.updated_at, "source": self.source
        }
...
class ExchangeSyncService:
    ...
    def _paper_snapshot(self, symbol):
        ...
        snap.qty = STATE.get("remaining_qty", STATE.get("qty", 0))
    ...
    def reconcile(self, symbol, local_state):
        ...
        with _TRADE_LOCK:
            ...
            # --- FIX: preserve original qty when partials have occurred ---
            if STATE.get("partial_closed") or STATE.get("tp1_hit"):
            ...
            TRADE_STATE.update({
                ...
                "entry": snap.entry_price, "qty": STATE.get("qty", snap.qty),
...
# ========== close_partial (Legacy - kept, will be patched) ==========
...
# ========== close_position_full (Legacy - kept, will be patched) ==========
...
class LiveTradeManager:
    ...
    def _compute_tp1_hold_score(self, smart, momentum, adx, adx_slope, trade_state, continuation_eval,
                                 distribution_risk, rejection_detected, failed_breakout, roe):
        score = 0
    ...
    def _compute_institutional_exit_warning(self, smart, momentum, distribution_risk, continuation_prob,
                                             rejection_detected, adx_slope, di_spread_change):
        warning = 0
    ...
    def _log_live_status(self):
        ...
        board_str = STATE.get("lifecycle_stage", "?")
        log_msg = (f"{BLUE}[LIVE_MGMT]{RESET} {direction_icon} {STATE['current_symbol']} {side} | "
                   ...
                   f"State: {state_str} | Board: {board_str}")
        log_execution(log_msg, "INFO")

    def _apply_management(self, symbol, now):
        # This method will be REPLACED by Trade Management Board v1 below
        # The original body remains here for reference (patched at bottom of file)
        if not STATE.get("open"):
            return
        ...
        if now - self.last_heavy_calc_ts >= 5:
            ...
            market_state = {
                ...
                "counter_displacement": counter_displacement, "volume_ratio": volume_ratio,
                "df": df_live, "last_candle": last_candle,
...
def sync_position_state(symbol=None):
    if PAPER_MODE:
        if STATE.get("open"):
            ...
            if price:
                ...
                qty_use = STATE.get("remaining_qty", STATE.get("qty", 0))
                if STATE["side"] == "BUY":
                    STATE["unrealized_pnl_usdt"] = (price - STATE["entry"]) * qty_use
                else:
                    STATE["unrealized_pnl_usdt"] = (STATE["entry"] - price) * qty_use
    ...
    with _TRADE_LOCK:
        if not STATE.get("open"):
            ...
            STATE["entry"] = snap.entry_price
            # --- FIX: preserve original qty if partials occurred ---
            if STATE.get("partial_closed") or STATE.get("tp1_hit"):
                STATE["remaining_qty"] = snap.qty
            else:
                STATE["qty"] = snap.qty
...
STATE = {
    ...
    # ===== NEW: Board v1 fields =====
    "board_decision": {},
    "lifecycle_stage": "SEEDING",
    "last_board_partial_ts": 0.0
}
...
def finalize_trade_with_reality(symbol):
    ...
    if PAPER_MODE:
        entry = STATE["entry"]
        side = STATE["side"]
        qty_use = STATE.get("qty", 0)
        if side == "BUY":
            pnl_pct = (mark_price - entry) / entry * 100 if entry > 0 else 0
        else:
            pnl_pct = (entry - mark_price) / entry * 100 if entry > 0 else 0
        pnl_usdt = pnl_pct / 100 * entry * qty_use
    else:
    ...
    with _TRADE_LOCK:
        ...
        STATE["scale_ins"] = 0
        STATE["board_decision"] = {}
        STATE["lifecycle_stage"] = "SEEDING"
    return pnl_usdt, pnl_pct
...
def update_position_dashboard(symbol, side, entry, qty, pnl=0.0):
    DASHBOARD_STATE["position"] = {
        ...
        "board_decision": STATE.get("board_decision", {}),
        "lifecycle_stage": STATE.get("lifecycle_stage", "SEEDING")
    }
...
def render_live_supervisor_panel():
    ...
    <div id="rf-live-panel" style="display:none;" class="rf-live-supervisor">
      <div class="rf-live-header">
        <span class="rf-live-title">🧠 RF v28 + Board v1 Live Supervisor</span>
        <span id="rf-live-status-badge" class="rf-live-pill rf-live-pill-idle">⚡ BOARD GOVERNANCE</span>
      </div>
      <div class="rf-live-grid">
        ...
        <div class="rf-live-card"><div class="rf-live-metric-icon">🧠</div><div class="rf-live-metric-label">Board Verdict</div><div class="rf-live-metric-value" id="rf-sup-board">-</div></div>
      ...
      <div class="rf-live-status-row">
        <span id="rf-pill-thesis" class="rf-live-pill rf-live-pill-active">🧠 BOARD ACTIVE</span>
      ...
      <div id="board-votes-panel" style="margin-top:14px; background:#0a0f17; padding:10px; border-radius:10px;">
        <div style="color:#9ca3af; font-size:12px; margin-bottom:6px;">📋 BOARD VOTES (7 Analysts)</div>
        <div id="board-votes-grid" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:6px; font-size:11px;"></div>
      </div>
    </div>
    <style>
    ...
    .board-vote-buy {color:#00ffa6;} .board-vote-sell {color:#ff4d4d;} .board-vote-hold {color:#95a5a6;}
    </style>
    """
...
def dashboard():
    ...
    board_panel_html = """
    <div class="section smart-layer">
      <div class="title">🏛️ TRADE MANAGEMENT BOARD v1</div>
      <div class="grid" style="grid-template-columns: repeat(3,1fr);">
        <div class="card">Verdict<div id="board-verdict" style="font-weight:bold;">-</div></div>
        <div class="card">Confidence<div id="board-conf">-</div></div>
        <div class="card">Lifecycle<div id="board-lifecycle">-</div></div>
        <div class="card">Chair Reason<div id="board-reason" style="font-size:11px;">-</div></div>
        <div class="card">Avg Score<div id="board-avg">-</div></div>
      </div>
    </div>
    """
...
function updateUI(d) {{
    ...
    if(d.board_decision) {{
        const bd = d.board_decision;
        document.getElementById("board-verdict").innerText = bd.verdict || "-";
        document.getElementById("board-conf").innerText = (bd.confidence || 0);
        document.getElementById("board-lifecycle").innerText = bd.lifecycle || "-";
        document.getElementById("board-reason").innerText = bd.reason || "-";
        document.getElementById("board-avg").innerText = bd.avg_score || "-";
    }}
    ...
    if(d.position) {{
        ...
        document.getElementById("pos").innerHTML = `
            ...
            <div>Lifecycle: ${{d.position.lifecycle_stage}} | Board: ${{d.position.board_decision?.verdict || "N/A"}}</div>
        `;
    }} else {{
    ...
    if(d.live_trade_mode && d.supervisor) {{
        ...
        document.getElementById("rf-sup-board").innerText = (d.board_decision && d.board_decision.verdict) || "-";
        ...
        document.getElementById("rf-sup-lifecycle").innerText = d.lifecycle_stage || "-";
        ...
        // Board votes grid
        if (d.board_decision && d.board_decision.votes) {{
            let votesHtml = "";
            d.board_decision.votes.forEach(v => {{
                let colorClass = "board-vote-hold";
                if (v.verdict && v.verdict.includes("RUNNER")) colorClass = "board-vote-buy";
                else if (v.verdict && (v.verdict.includes("EXIT") || v.verdict.includes("TIGHTEN") || v.verdict.includes("PARTIAL"))) colorClass = "board-vote-sell";
                votesHtml += `<div style="background:#111827; padding:6px; border-radius:6px;"><b>${{v.analyst}}</b><br><span class="${{colorClass}}">${{v.verdict}}</span> (${{v.confidence}})<br><span style="color:#95a5a6; font-size:10px;">${{v.reason}}</span></div>`;
            }});
            document.getElementById("board-votes-grid").innerHTML = votesHtml;
        }}
    }} else {{
    ...
    if(d.watchlist) {{
        ...
        for (let sym in d.watchlist) {{
            ...
            wHtml += `<div style="margin-bottom:8px;border-bottom:1px solid #2c3e50;padding-bottom:4px;"><b>${{sideIcon}} ${{w.symbol}}</b> | Score: ${{w.score}} | ${{w.state}}</div>`;
        }}
        document.getElementById("watchlist-panel").innerHTML = wHtml || "No candidates";
    }}
    ...
    try:
        ...
        if STATE.get("open") and "dynamic_manager" in STATE:
            ...
            dynamic_trade_data = {
                ...
                "lifecycle": STATE.get("lifecycle_stage", "N/A")
            }
        else:
            dynamic_trade_data = {
        ...
        payload = {
            ...
            "board_decision": DASHBOARD_STATE.get("board_decision", {}),
            "lifecycle_stage": STATE.get("lifecycle_stage", "SEEDING"),
            **live_data
        }
...
@app.route("/board")
def board_endpoint():
    return jsonify(safe_json(DASHBOARD_STATE.get("board_decision", {})))
...
def print_snapshot():
    ...
    perf = get_dashboard_metrics()
    print("\n" + "="*70)
    print(color_text(f"🔥 RF v28 + Board v1 ({mode}) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", BOLD))
    ...
    if STATE["open"]:
        ...
        print(f"   Entry: {STATE['entry']:.4f} | ROE: {roe_colored}")
        print(f"   SL: {STATE.get('synthetic_sl',0):.4f} | TP1: {STATE.get('synthetic_tp1',0):.4f} | TP2: {STATE.get('tp2_price',0):.4f}")
        bd = STATE.get("board_decision", {})
        if bd:
            print(f"   🏛️ BOARD: {bd.get('verdict','?')} | Stage: {bd.get('lifecycle','?')} | Conf: {bd.get('confidence','?')}")
            print(f"   📋 Reason: {bd.get('reason','?')}")
            votes = bd.get("votes", [])
            if votes:
                vote_str = " | ".join([f"{v['analyst']}={v['verdict']}" for v in votes[:7]])
                print(f"   🗳️ Votes: {vote_str}")
    else:
        print("📊 POSITION: None")
    if USE_EXECUTION_QUEUE:
...
def execute_entry(side, symbol, price, sl, tp1, tp2, score, reason, atr_val, trade_type, entry_type, classification):
    ...
    if PAPER_MODE:
        ...
        STATE.update({
            ...
            "board_decision": {}, "lifecycle_stage": "SEEDING",
            "last_board_partial_ts": 0.0
        })
        TRADE_STATE.update({
...
# ██                                                                  ██
# ██      TRADE MANAGEMENT BOARD v1 – PROFESSIONAL EDITION            ██
# ██      مجلس إدارة الصفقة الاحترافي (7 محللين + رئيس المجلس)          ██
# ██                                                                  ██
# ██      Additive patch: كل الكود الأصلي محفوظ + Board v1 يتحكم        ██
# ██████████████████████████████████████████████████████████████████████
# ====================================================================

import enum as _board_enum

# ---------- 1. VERDICT & LIFECYCLE ENUMS ----------
class BoardVerdict(_board_enum.Enum):
    RUNNER_MODE     = -1
    HOLD            = 0
    ACTIVATE_TRAIL  = 1
    TIGHTEN_TRAIL   = 2
...
class LifecycleStage(_board_enum.Enum):
    SEEDING     = "SEEDING"
    GROWING     = "GROWING"
    MATURING    = "MATURING"
...
class BoardDecision:
    ...
    new_trail_mult: Optional[float] = None
    move_sl_to: Optional[float] = None
    metadata: Dict[str, Any] = field(default_factory=dict)

# ---------- 3. PER-SYMBOL CLOSE LOCKS ----------
_CLOSE_LOCK_REGISTRY: Dict[str, threading.RLock] = {}
_CLOSE_LOCK_GUARD = threading.Lock()

def _get_symbol_lock(symbol: str) -> threading.RLock:
    with _CLOSE_LOCK_GUARD:
        if symbol not in _CLOSE_LOCK_REGISTRY:
            _CLOSE_LOCK_REGISTRY[symbol] = threading.RLock()
        return _CLOSE_LOCK_REGISTRY[symbol]

def _acquire_symbol_lock(symbol: str, timeout: float = 15.0) -> bool:
    lock = _get_symbol_lock(symbol)
    return lock.acquire(timeout=timeout)

def _release_symbol_lock(symbol: str) -> None:
    lock = _CLOSE_LOCK_REGISTRY.get(symbol)
    if lock:
        try:
            lock.release()
        except RuntimeError:
...
# ---------- 4. SAFE CLOSE FUNCTIONS v2 ----------
def _close_partial_v2(ratio: float) -> bool:
    if not STATE.get("open"):
        log_execution("[CLOSE_PARTIAL_V2] No open position", "WARN")
        return False
    symbol = STATE.get("current_symbol")
    if not symbol:
        log_execution("[CLOSE_PARTIAL_V2] No current_symbol", "WARN")
        return False
    if ratio <= 0 or ratio >= 1.0:
        log_execution(f"[CLOSE_PARTIAL_V2] Invalid ratio={ratio}", "WARN")
        return False
    if not _acquire_symbol_lock(symbol, timeout=15.0):
        log_execution(f"[CLOSE_PARTIAL_V2] Lock timeout for {symbol}", "ERROR")
        return False
    try:
        if PAPER_MODE:
            return _paper_partial_close(ratio)
        return _live_partial_close(symbol, ratio)
    except Exception as e:
        log_execution(f"[CLOSE_PARTIAL_V2] Exception: {traceback.format_exc()}", "ERROR")
        return False
    finally:
        _release_symbol_lock(symbol)

def _paper_partial_close(ratio: float) -> bool:
    if not paper.get("position"):
        return False
    paper["position"]["remaining_qty"] = paper["position"].get("remaining_qty", paper["position"]["qty"]) * (1 - ratio)
    STATE["remaining_qty"] = paper["position"]["remaining_qty"]
    STATE["qty"] = STATE["remaining_qty"]
    TRADE_STATE["qty"] = STATE["remaining_qty"]
    STATE["partial_closed"] = True
    log_execution(f"[PAPER_PARTIAL] Closed {ratio*100:.0f}% → remaining={STATE['remaining_qty']:.6f}", "SUCCESS")
    return True

def _live_partial_close(symbol: str, ratio: float) -> bool:
    if STATE["remaining_qty"] <= 0:
        log_execution("[LIVE_PARTIAL] No qty remaining", "WARN")
        return False
    qty_to_close = STATE["remaining_qty"] * ratio
    sym_norm = normalize_symbol(symbol)
    try:
        qty_precise = float(ex.amount_to_precision(sym_norm, qty_to_close))
    except Exception as e:
        log_execution(f"[LIVE_PARTIAL] amount_to_precision failed: {e}", "ERROR")
        return False
    if qty_precise <= 0:
        log_execution("[LIVE_PARTIAL] Rounded qty is 0", "WARN")
        return False
    close_side = "sell" if STATE["side"] == "BUY" else "buy"
    for attempt in range(3):
        try:
            order = safe_api_call(ex.create_order, sym_norm, "market", close_side, qty_precise, params={"reduceOnly": True})
            if order is None:
                log_execution(f"[LIVE_PARTIAL] Order None (attempt {attempt+1})", "WARN")
                time.sleep(0.5)
                continue
            order_id = order.get("id")
            filled, _ = verify_order_filled(symbol, order_id, close_side, qty_precise, timeout=8)
            if filled:
                time.sleep(0.6)
                pos = fetch_position(symbol)
                if pos is None:
                    ...
                    TRADE_STATE["in_position"] = False
                    DASHBOARD_STATE["live_trade_mode"] = False
                    log_execution("[LIVE_PARTIAL] Position fully closed (unexpected)", "WARN")
                    return True
                new_qty = safe_float(pos.get("contracts", 0))
                STATE["remaining_qty"] = new_qty
                STATE["qty"] = new_qty
                TRADE_STATE["qty"] = new_qty
                STATE["partial_closed"] = True
                log_execution(f"[LIVE_PARTIAL] Closed {ratio*100:.0f}% → remaining={new_qty:.6f}", "SUCCESS")
                return True
            log_execution(f"[LIVE_PARTIAL] Fill unconfirmed (attempt {attempt+1})", "WARN")
        except Exception as e:
            log_execution(f"[LIVE_PARTIAL] Error: {e}", "ERROR")
            time.sleep(0.5)
    log_execution("[LIVE_PARTIAL] All attempts failed", "ERROR")
    return False

def _close_full_v2(force: bool = False) -> bool:
    if not STATE.get("open"):
        return False
    symbol = STATE.get("current_symbol")
    if not symbol:
        return False
    if not _acquire_symbol_lock(symbol, timeout=20.0):
        log_execution(f"[CLOSE_FULL_V2] Lock timeout for {symbol}", "ERROR")
        return False
    try:
        if PAPER_MODE:
            return _paper_full_close()
        return _live_full_close(symbol, force=force)
    except Exception as e:
        log_execution(f"[CLOSE_FULL_V2] Exception: {traceback.format_exc()}", "ERROR")
        return False
    finally:
        _release_symbol_lock(symbol)

def _paper_full_close() -> bool:
    ...
    try:
        ...
        log_execution(f"[PAPER_FULL_CLOSE] finalize error: {e}", "ERROR")
    log_execution("[PAPER_FULL_CLOSE] Position closed", "SUCCESS")
    return True

def _live_full_close(symbol: str, force: bool = False) -> bool:
    qty_to_close = STATE.get("remaining_qty", 0)
    if qty_to_close <= 0:
        pos = fetch_position(symbol)
        if pos:
            qty_to_close = safe_float(pos.get("contracts", 0))
        if qty_to_close <= 0:
            STATE["open"] = False
            STATE["remaining_qty"] = 0
            STATE["qty"] = 0
    ...
    try:
        ...
        qty_precise = qty_to_close
    for attempt in range(4):
        try:
            order = safe_api_call(ex.create_order, sym_norm, "market", close_side, qty_precise, params={"reduceOnly": True})
            if order is None:
                log_execution(f"[CLOSE_FULL_V2] Order None (attempt {attempt+1})", "WARN")
                time.sleep(1)
                continue
            order_id = order.get("id")
            verify_order_filled(symbol, order_id, close_side, qty_precise, timeout=8)
            time.sleep(0.8)
            pos = fetch_position(symbol)
            if pos is None:
                STATE["open"] = False
                ...
                try:
                    ...
                    log_execution(f"[CLOSE_FULL_V2] finalize error: {e}", "ERROR")
                log_execution("[CLOSE_FULL_V2] Position fully closed", "SUCCESS")
                return True
            new_qty = safe_float(pos.get("contracts", 0))
            if new_qty <= 0.0000001:
                ...
                try:
                    ...
                    pass
                return True
            log_execution(f"[CLOSE_FULL_V2] Still {new_qty:.6f}, retrying", "WARN")
            qty_precise = float(ex.amount_to_precision(sym_norm, new_qty))
        except Exception as e:
            log_execution(f"[CLOSE_FULL_V2] Exception: {e}", "ERROR")
            time.sleep(1)
    log_execution("[CLOSE_FULL_V2] FAILED. Manual intervention required.", "ERROR")
    return False

# ---------- 5. ANALYSTS ----------
def _analyst_trend(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    adx = ctx.get("adx", 20)
    adx_slope = ctx.get("adx_slope", 0)
    di_plus = ctx.get("di_plus", 20)
    di_minus = ctx.get("di_minus", 20)
    di_spread = (di_plus - di_minus) if side == "BUY" else (di_minus - di_plus)
    if adx < 18:
        return BoardVote("trend", BoardVerdict.TIGHTEN_TRAIL, 0.7, 1.0, f"ADX chop ({adx:.1f})", {"adx": adx})
    if di_spread < 0:
        if roe > 1.0:
            return BoardVote("trend", BoardVerdict.PARTIAL_EXIT, 0.75, 1.2, f"DI flip (spread={di_spread:.1f})", {})
        return BoardVote("trend", BoardVerdict.FULL_EXIT, 0.65, 1.3, f"DI flip, no profit", {})
    if adx_slope < -2 and adx > 25:
        return BoardVote("trend", BoardVerdict.TIGHTEN_TRAIL, 0.7, 1.0, f"ADX rolling over ({adx_slope:.1f})", {})
    if adx > 30 and adx_slope > 0 and di_spread > 8 and roe > 2.0:
        return BoardVote("trend", BoardVerdict.RUNNER_MODE, 0.8, 1.0, f"Strong trend ADX={adx:.1f}", {})
    return BoardVote("trend", BoardVerdict.HOLD, 0.6, 1.0, f"Trend stable ({adx:.1f})", {})

def _analyst_liquidity(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    df = ctx.get("df")
    price = ctx.get("price", 0)
    if df is None or len(df) < 30 or price <= 0:
        return BoardVote("liquidity", BoardVerdict.HOLD, 0.3, 0.5, "No data", {})
    try:
        supports, resistances = get_clustered_zones(df, lookback=80, cluster_pct=0.002)
    except Exception:
    ...
    if side == "BUY":
        if resistances:
            ...
            if candidates:
                next_res = min(candidates)
                dist_pct = (next_res - price) / price
                if dist_pct < 0.003 and roe > 1.5:
                    return BoardVote("liquidity", BoardVerdict.PARTIAL_EXIT, 0.75, 1.3, f"Approaching resistance ({dist_pct*100:.2f}%)", {})
                if dist_pct < 0.006 and roe > 2.5:
                    return BoardVote("liquidity", BoardVerdict.TIGHTEN_TRAIL, 0.65, 1.0, "Near resistance", {})
        if swept_h and roe > 0.5:
            return BoardVote("liquidity", BoardVerdict.TIGHTEN_TRAIL, 0.6, 0.9, "Buy-side sweep (targets us)", {})
    else:
        if supports:
            candidates = [s for s in supports if s < price]
            if candidates:
                next_sup = max(candidates)
                dist_pct = (price - next_sup) / price
                if dist_pct < 0.003 and roe > 1.5:
                    return BoardVote("liquidity", BoardVerdict.PARTIAL_EXIT, 0.75, 1.3, f"Approaching support ({dist_pct*100:.2f}%)", {})
                if dist_pct < 0.006 and roe > 2.5:
                    return BoardVote("liquidity", BoardVerdict.TIGHTEN_TRAIL, 0.65, 1.0, "Near support", {})
        if swept_l and roe > 0.5:
            return BoardVote("liquidity", BoardVerdict.TIGHTEN_TRAIL, 0.6, 0.9, "Sell-side sweep (targets us)", {})
    return BoardVote("liquidity", BoardVerdict.HOLD, 0.5, 1.0, "No opposing liquidity ahead", {})

def _analyst_structure(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    df = ctx.get("df")
    if df is None or len(df) < 15:
        return BoardVote("structure", BoardVerdict.HOLD, 0.3, 0.5, "No data", {})
    try:
        struct_shift = detect_structure_shift(df)
        bos_up, bos_down = detect_bos(df, lookback=5)
    except Exception:
    ...
    if side == "BUY":
        if struct_shift == "bearish_shift":
            return BoardVote("structure", BoardVerdict.FULL_EXIT, 0.8, 1.4, "Bearish MSS against long", {})
        if bos_down:
            return BoardVote("structure", BoardVerdict.PARTIAL_EXIT, 0.7, 1.1, "BOS down against long", {})
        if struct_shift == "bullish_shift" or bos_up:
            return BoardVote("structure", BoardVerdict.RUNNER_MODE, 0.7, 1.0, "Bullish structure intact", {})
    else:
        if struct_shift == "bullish_shift":
            return BoardVote("structure", BoardVerdict.FULL_EXIT, 0.8, 1.4, "Bullish MSS against short", {})
        if bos_up:
            return BoardVote("structure", BoardVerdict.PARTIAL_EXIT, 0.7, 1.1, "BOS up against short", {})
        if struct_shift == "bearish_shift" or bos_down:
            return BoardVote("structure", BoardVerdict.RUNNER_MODE, 0.7, 1.0, "Bearish structure intact", {})
    return BoardVote("structure", BoardVerdict.HOLD, 0.5, 1.0, "Structure neutral", {})

def _analyst_momentum(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    mom = ctx.get("momentum", {}) or {}
    if not mom:
        return BoardVote("momentum", BoardVerdict.HOLD, 0.3, 0.6, "No momentum data", {})
    mom_health = mom.get("momentum_health", 50)
    exhaustion = mom.get("exhaustion_risk", 0)
    climax = mom.get("climax_risk", 0)
    decay = mom.get("momentum_decay", False)
    expansion = mom.get("trend_expansion", False)
    greed = mom.get("greed_state", False)
    if decay and roe > 1.0:
        return BoardVote("momentum", BoardVerdict.TIGHTEN_TRAIL, 0.75, 1.2, "Momentum decay with profit", {})
    if climax > 70 and roe > 2.0:
        return BoardVote("momentum", BoardVerdict.PARTIAL_EXIT, 0.8, 1.3, f"Climax risk {climax:.0f}", {})
    if exhaustion > 70 and roe > 2.0:
        return BoardVote("momentum", BoardVerdict.PARTIAL_EXIT, 0.75, 1.2, f"Exhaustion risk {exhaustion:.0f}", {})
    if greed and roe > 4.0:
        return BoardVote("momentum", BoardVerdict.PARTIAL_EXIT, 0.7, 1.1, "Retail greed", {})
    if expansion and mom_health > 60 and roe > 1.5:
        return BoardVote("momentum", BoardVerdict.RUNNER_MODE, 0.7, 1.0, f"Momentum expanding ({mom_health:.0f})", {})
    if mom_health < 30:
        return BoardVote("momentum", BoardVerdict.TIGHTEN_TRAIL, 0.65, 1.0, f"Momentum weak ({mom_health:.0f})", {})
    return BoardVote("momentum", BoardVerdict.HOLD, 0.5, 1.0, "Momentum ok", {})

def _analyst_institutional(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    smart = ctx.get("smart_money", {}) or {}
    if not smart:
        return BoardVote("institutional", BoardVerdict.HOLD, 0.3, 0.7, "No smart data", {})
    dist_risk = smart.get("distribution_risk", 0)
    acc = smart.get("accumulation_strength", 0)
    dominant = smart.get("smart_money_dominant", False)
    bias = smart.get("institutional_bias", "NEUTRAL")
    if side == "BUY":
        if dist_risk > 65 and roe > 0.5:
            return BoardVote("institutional", BoardVerdict.FULL_EXIT, 0.85, 1.4, f"Distribution {dist_risk:.0f} against long", {})
        if dist_risk > 50 and roe > 1.0:
            return BoardVote("institutional", BoardVerdict.PARTIAL_EXIT, 0.75, 1.2, "Distribution building", {})
        if dominant and bias == "SELL":
            return BoardVote("institutional", BoardVerdict.FULL_EXIT, 0.7, 1.3, "Smart money flipped bearish", {})
        if dominant and bias == "BUY" and acc > 60 and roe > 1.5:
            return BoardVote("institutional", BoardVerdict.RUNNER_MODE, 0.75, 1.0, "Accumulation continues", {})
    else:
        if acc > 60 and roe > 0.5:
            return BoardVote("institutional", BoardVerdict.FULL_EXIT, 0.85, 1.4, f"Accumulation {acc:.0f} against short", {})
        if dominant and bias == "BUY":
            return BoardVote("institutional", BoardVerdict.FULL_EXIT, 0.7, 1.3, "Smart money flipped bullish", {})
        if dominant and bias == "SELL" and dist_risk > 50 and roe > 1.5:
            return BoardVote("institutional", BoardVerdict.RUNNER_MODE, 0.75, 1.0, "Distribution continues", {})
    return BoardVote("institutional", BoardVerdict.HOLD, 0.5, 1.0, "Institutional neutral", {})

def _analyst_risk(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    peak_roe = state.get("peak_roe", 0) or 0
    drawdown = state.get("drawdown_from_peak", 0) or 0
    entry_time = state.get("entry_time", time.time()) or time.time()
    duration_min = (time.time() - entry_time) / 60
    if peak_roe > 3.0 and drawdown > peak_roe * 0.6:
        return BoardVote("risk", BoardVerdict.FULL_EXIT, 0.85, 1.5, f"Gave back {drawdown:.1f}% of {peak_roe:.1f}%", {})
    if peak_roe > 2.0 and drawdown > peak_roe * 0.45:
        return BoardVote("risk", BoardVerdict.PARTIAL_EXIT, 0.75, 1.2, f"Drawdown from peak {drawdown:.1f}%", {})
    if roe < -1.5:
        return BoardVote("risk", BoardVerdict.FULL_EXIT, 0.75, 1.4, f"Loss {roe:.2f}%", {})
    if duration_min > 240 and abs(roe) < 0.5:
        return BoardVote("risk", BoardVerdict.TIGHTEN_TRAIL, 0.6, 1.0, f"Stale ({duration_min:.0f}min)", {})
    if roe > 3.0 and drawdown < 0.5:
        return BoardVote("risk", BoardVerdict.RUNNER_MODE, 0.6, 1.0, "Healthy profit, low drawdown", {})
    return BoardVote("risk", BoardVerdict.HOLD, 0.5, 1.0, "Risk ok", {})

def _analyst_exhaustion(ctx: dict, side: str, roe: float, state: dict) -> BoardVote:
    df = ctx.get("df")
    atr = ctx.get("atr", 0)
    price = ctx.get("price", 0)
    if df is None or len(df) < 20 or atr <= 0:
        return BoardVote("exhaustion", BoardVerdict.HOLD, 0.3, 0.6, "No data", {})
    try:
        vol_state = classify_volume(df)
    except Exception:
        vol_state = "neutral"
    ...
    if range_ > 0:
        if side == "BUY":
            upper_wick = last["high"] - max(last["open"], last["close"])
            if upper_wick / range_ > 0.65 and body < atr * 0.4:
                return BoardVote("exhaustion", BoardVerdict.TIGHTEN_TRAIL, 0.7, 1.1, "Shooting star wick", {})
        else:
            lower_wick = min(last["open"], last["close"]) - last["low"]
            if lower_wick / range_ > 0.65 and body < atr * 0.4:
                return BoardVote("exhaustion", BoardVerdict.TIGHTEN_TRAIL, 0.7, 1.1, "Hammer wick", {})
    if vol_state == "exhaustion" and roe > 1.5:
        return BoardVote("exhaustion", BoardVerdict.PARTIAL_EXIT, 0.7, 1.1, "Volume exhaustion", {})
    try:
        ema20 = ema(df["close"], 20).iloc[-1]
        extension = abs(price - ema20) / atr
        if extension > 3.5 and roe > 2.0:
            return BoardVote("exhaustion", BoardVerdict.PARTIAL_EXIT, 0.7, 1.1, f"Overextended ({extension:.1f} ATR)", {})
    except Exception:
        pass
    return BoardVote("exhaustion", BoardVerdict.HOLD, 0.5, 1.0, "No exhaustion", {})

# ---------- 6. CHAIRMAN ----------
class _BoardChairman:
    STAGE_WEIGHTS = {
        LifecycleStage.SEEDING:    {"trend": 1.2, "structure": 1.3, "risk": 1.4, "liquidity": 0.8, "momentum": 0.9, "institutional": 1.0, "exhaustion": 0.7},
        LifecycleStage.GROWING:    {"trend": 1.2, "structure": 1.1, "risk": 1.2, "liquidity": 1.0, "momentum": 1.0, "institutional": 1.1, "exhaustion": 1.0},
        LifecycleStage.MATURING:   {"trend": 1.0, "structure": 1.0, "risk": 1.3, "liquidity": 1.3, "momentum": 1.2, "institutional": 1.2, "exhaustion": 1.2},
        LifecycleStage.HARVESTING: {"trend": 0.9, "structure": 0.9, "risk": 1.5, "liquidity": 1.5, "momentum": 1.3, "institutional": 1.3, "exhaustion": 1.4},
        LifecycleStage.DEFENDING:  {"trend": 1.0, "structure": 1.3, "risk": 1.5, "liquidity": 1.3, "momentum": 1.2, "institutional": 1.3, "exhaustion": 1.3},
        LifecycleStage.EXITING:    {"trend": 1.0, "structure": 1.0, "risk": 1.0, "liquidity": 1.0, "momentum": 1.0, "institutional": 1.0, "exhaustion": 1.0},
    }

    def determine_lifecycle(self, roe: float, peak_roe: float, votes: List[BoardVote]) -> LifecycleStage:
        for v in votes:
            if v.verdict in (BoardVerdict.FULL_EXIT, BoardVerdict.EMERGENCY_EXIT) and v.confidence >= 0.75:
                return LifecycleStage.EXITING
        tightening = sum(1 for v in votes if v.verdict == BoardVerdict.TIGHTEN_TRAIL and v.confidence >= 0.65)
        if tightening >= 2:
            return LifecycleStage.DEFENDING
        if roe < 0.5:
            return LifecycleStage.SEEDING
        if roe < 2.0:
            return LifecycleStage.GROWING
        if roe < 5.0:
            return LifecycleStage.MATURING
        return LifecycleStage.HARVESTING

    def convene(self, symbol: str, df, price: float, atr: float, state: dict, ctx: dict) -> BoardDecision:
        side = state.get("side", "BUY")
        roe = state.get("roe_pct", 0.0) or 0.0
        peak_roe = state.get("peak_roe", 0.0) or 0.0
        ctx = dict(ctx)
        ctx["df"] = df
        ctx["price"] = price
        ctx["atr"] = atr
        votes = [
            _analyst_trend(ctx, side, roe, state),
            _analyst_liquidity(ctx, side, roe, state),
            _analyst_structure(ctx, side, roe, state),
            _analyst_momentum(ctx, side, roe, state),
            _analyst_institutional(ctx, side, roe, state),
            _analyst_risk(ctx, side, roe, state),
            _analyst_exhaustion(ctx, side, roe, state),
        ]
        lifecycle = self.determine_lifecycle(roe, peak_roe, votes)
        stage_w = self.STAGE_WEIGHTS.get(lifecycle, self.STAGE_WEIGHTS[LifecycleStage.GROWING])
        total_score = 0.0
        total_weight = 0.0
        for v in votes:
            stage_mult = stage_w.get(v.analyst, 1.0)
            eff_weight = v.weight * stage_mult * v.confidence
            total_score += v.verdict.severity * eff_weight
            total_weight += eff_weight
        avg_score = total_score / total_weight if total_weight > 0 else 0.0
        emergency_votes = [v for v in votes if v.verdict == BoardVerdict.EMERGENCY_EXIT and v.confidence >= 0.7]
        if emergency_votes:
            final = BoardVerdict.EMERGENCY_EXIT
            reason = f"EMERGENCY: {emergency_votes[0].reason}"
        else:
            if avg_score >= 4.5:
                final = BoardVerdict.FULL_EXIT
            elif avg_score >= 3.5:
                final = BoardVerdict.PARTIAL_EXIT
            elif avg_score >= 2.5:
                final = BoardVerdict.MOVE_TO_BE
            elif avg_score >= 1.5:
                final = BoardVerdict.TIGHTEN_TRAIL
            elif avg_score >= 0.5:
                final = BoardVerdict.ACTIVATE_TRAIL
            elif avg_score <= -0.3 and roe > 1.5:
                final = BoardVerdict.RUNNER_MODE
            else:
                final = BoardVerdict.HOLD
            reason = f"Score {avg_score:.2f} | Stage={lifecycle.value}"
        partial_ratio = 0.0
        if final == BoardVerdict.PARTIAL_EXIT:
            if lifecycle == LifecycleStage.HARVESTING:
                partial_ratio = 0.6
                ...
                partial_ratio = 0.4
        new_trail = None
        if final in (BoardVerdict.TIGHTEN_TRAIL, BoardVerdict.MOVE_TO_BE):
            base_mult = state.get("smart_trail_mult", 1.5)
            new_trail = max(0.5, base_mult * 0.7)
        elif final == BoardVerdict.RUNNER_MODE:
            new_trail = min(4.5, state.get("smart_trail_mult", 1.5) * 1.3)
        return BoardDecision(
            ...
            new_trail_mult=new_trail,
            metadata={"avg_score": avg_score, "total_weight": total_weight},
        )

_board_chairman = _BoardChairman()

# ---------- 7. NEW _apply_management (v2) ----------
def _apply_management_v2(self, symbol, now):
    if not STATE.get("open"):
        return
    if self.lifecycle_state != TradeLifecycleState.LIVE:
        return
    ...
    entry = STATE["entry"]
    roe = STATE.get("roe_pct", 0.0) or 0.0
    if roe > (STATE.get("peak_roe", 0.0) or 0.0):
        STATE["peak_roe"] = roe
        STATE["peak_price"] = mark_price
    peak = STATE.get("peak_roe", roe) or roe
    STATE["drawdown_from_peak"] = max(0.0, peak - roe) if peak > 0 else 0.0
    ...
    if now - self.last_heavy_calc_ts >= 5:
        ...
        adx_now = STATE.get("adx_live", 20.0)
        adx_slope = 0.0
        smart_money = STATE.get("smart_money", {}) or {}
        momentum = STATE.get("momentum_flow", {}) or {}
        regime = STATE.get("market_regime", "UNKNOWN")
        trade_state = STATE.get("trade_state", "RANGE_CHOP")

    ctx = {
        "adx": adx_now, "adx_slope": adx_slope,
        "di_plus": plus_di, "di_minus": minus_di,
        "smart_money": smart_money, "momentum": momentum, "regime": regime,
    }

    decision = _board_chairman.convene(symbol, df_live, mark_price, atr, STATE, ctx)

    STATE["board_decision"] = {
        "verdict": decision.verdict.value,
        "confidence": round(decision.confidence, 2),
        "lifecycle": decision.lifecycle.value,
        "reason": decision.chair_reason,
        "votes": [
            {"analyst": v.analyst, "verdict": v.verdict.name,
             "confidence": round(v.confidence, 2), "reason": v.reason}
            for v in decision.votes
        ],
        "avg_score": round(decision.metadata.get("avg_score", 0), 2),
    }
    DASHBOARD_STATE["board_decision"] = STATE["board_decision"]
    STATE["lifecycle_stage"] = decision.lifecycle.value

    v = decision.verdict

    if v in (BoardVerdict.EMERGENCY_EXIT, BoardVerdict.FULL_EXIT):
        log_execution(f"[BOARD] {v.value} on {symbol}: {decision.chair_reason}", "WARN")
        try:
            tg_board_decision(symbol, v.value, decision.chair_reason)
        except Exception:
            pass
        _close_full_v2(force=True)
        self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
        DASHBOARD_STATE["live_trade_mode"] = False
        return

    if v == BoardVerdict.PARTIAL_EXIT:
        ratio = decision.partial_ratio or 0.5
        last_partial_ts = STATE.get("last_board_partial_ts", 0)
        if now - last_partial_ts >= 30:
            log_execution(f"[BOARD] PARTIAL {ratio*100:.0f}% on {symbol}: {decision.chair_reason}", "WARN")
            try:
                tg_board_decision(symbol, f"PARTIAL {ratio*100:.0f}%", decision.chair_reason)
            except Exception:
                pass
            _close_partial_v2(ratio)
            STATE["last_board_partial_ts"] = now
            if roe > 0.8:
                STATE["synthetic_sl"] = entry
                STATE["move_to_be_ts"] = now
        if decision.new_trail_mult is not None:
            STATE["smart_trail_mult"] = decision.new_trail_mult

    elif v == BoardVerdict.MOVE_TO_BE:
        if roe > 0.5 and STATE.get("synthetic_sl", 0) != entry:
            log_execution(f"[BOARD] MOVE_TO_BE: {decision.chair_reason}", "INFO")
            STATE["synthetic_sl"] = entry
            STATE["move_to_be_ts"] = now

    elif v == BoardVerdict.TIGHTEN_TRAIL:
        if decision.new_trail_mult is not None:
            old = STATE.get("smart_trail_mult", 1.5)
            if decision.new_trail_mult < old:
                STATE["smart_trail_mult"] = decision.new_trail_mult
                log_execution(f"[BOARD] TIGHTEN {old:.2f}→{decision.new_trail_mult:.2f}: {decision.chair_reason}", "INFO")

    elif v == BoardVerdict.ACTIVATE_TRAIL:
        if not STATE.get("trail_activated") and roe > 0.3:
            STATE["trail_activated"] = True
            STATE["trail_stop"] = STATE.get("synthetic_sl",
                                            entry - atr * 1.5 if side == "BUY" else entry + atr * 1.5)
            log_execution(f"[BOARD] ACTIVATE_TRAIL: {decision.chair_reason}", "INFO")

    elif v == BoardVerdict.RUNNER_MODE:
        if decision.new_trail_mult is not None:
            STATE["smart_trail_mult"] = max(STATE.get("smart_trail_mult", 1.5), decision.new_trail_mult)
        STATE["runner_mode"] = True

    # TP1 MANAGEMENT
    if not STATE.get("tp1_hit", False):
        if decision.lifecycle in (LifecycleStage.SEEDING, LifecycleStage.GROWING, LifecycleStage.MATURING) and \
           trade_state in ("ACCUMULATION", "EXPANSION", "TREND_RIDE", "HEALTHY_PULLBACK"):
            STATE["delay_tp1"] = True
            tp1_orig = STATE.get("synthetic_tp1", 0)
            if tp1_orig > 0:
                if side == "BUY":
                    STATE["synthetic_tp1"] = max(tp1_orig, entry * 1.025)
                else:
                    STATE["synthetic_tp1"] = min(tp1_orig, entry * 0.975)
        else:
            STATE["delay_tp1"] = False
            tp1 = STATE.get("synthetic_tp1", 0)
            if tp1 > 0:
                if (side == "BUY" and mark_price >= tp1) or (side == "SELL" and mark_price <= tp1):
                    log_execution(f"[BOARD_TP1] Hit at {mark_price:.4f} (target {tp1:.4f})", "SUCCESS")
                    _close_partial_v2(0.5)
                    STATE["tp1_hit"] = True
                    STATE["synthetic_sl"] = entry
                    STATE["runner_mode"] = True
                    STATE["trail_activated"] = True
    ...
    # TRAIL UPDATE
    trail_mult = STATE.get("smart_trail_mult", 1.5)
    if (smart_money.get("distribution_risk", 0) or 0) > 45:
        trail_mult *= 0.7
    if (momentum.get("momentum_health", 50) or 50) < 30:
        trail_mult *= 0.8
    if STATE.get("runner_mode", False):
        trail_mult *= 1.15
    trail_mult = max(0.5, min(4.5, trail_mult))

    if STATE.get("trail_activated") and roe > 0.3:
        if side == "BUY":
            new_trail = mark_price - trail_mult * atr
            new_trail = max(new_trail, STATE.get("trail_stop", 0) or 0)
            if STATE.get("tp1_hit"):
                new_trail = max(new_trail, entry)
            STATE["trail_stop"] = new_trail
        else:
            new_trail = mark_price + trail_mult * atr
            prev = STATE.get("trail_stop", float("inf")) or float("inf")
            new_trail = min(new_trail, prev)
            if STATE.get("tp1_hit"):
                new_trail = min(new_trail, entry)
            STATE["trail_stop"] = new_trail

    # HARD STOPS
    syn_sl = STATE.get("synthetic_sl", 0)
    if syn_sl > 0:
        if (side == "BUY" and mark_price <= syn_sl) or (side == "SELL" and mark_price >= syn_sl):
            log_execution(f"[HARD_SL] {mark_price:.4f} <= SL {syn_sl:.4f}", "WARN")
            _close_full_v2(force=True)
            self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
            DASHBOARD_STATE["live_trade_mode"] = False
            return
    ...
    if STATE.get("trail_activated") and trail_stop > 0:
        if (side == "BUY" and mark_price <= trail_stop) or (side == "SELL" and mark_price >= trail_stop):
            log_execution(f"[TRAIL_HIT] {mark_price:.4f} <= trail {trail_stop:.4f}", "WARN")
            _close_full_v2(force=True)
            self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
            DASHBOARD_STATE["live_trade_mode"] = False
            return

    # TP2 (PARTIAL, KEEP RUNNER)
    if STATE.get("tp1_hit") and not STATE.get("tp2_hit"):
        tp2_pct = 0.05
        if decision.lifecycle == LifecycleStage.HARVESTING:
            tp2_pct = 0.08
        tp2_price = entry * (1 + tp2_pct) if side == "BUY" else entry * (1 - tp2_pct)
        STATE["tp2_price"] = tp2_price
        if (side == "BUY" and mark_price >= tp2_price) or (side == "SELL" and mark_price <= tp2_price):
            log_execution(f"[BOARD_TP2] Hit at {mark_price:.4f}", "SUCCESS")
            _close_partial_v2(0.6)
            STATE["tp2_hit"] = True
            STATE["runner_mode"] = True
            try:
                tg_tp_hit(symbol, 2, roe)
            except Exception:
                pass
    ...
    if now - self.last_live_debug_ts >= 5:
        self.last_live_debug_ts = now
        log_execution(
            f"[BOARD_LIVE] {symbol} | Stage={decision.lifecycle.value} | "
            f"Verdict={decision.verdict.name} | ROE={roe:.2f}% | "
            f"Trail={STATE.get('trail_stop',0):.4f} | SL={syn_sl:.4f} | "
            f"TP1={STATE.get('tp1_hit',False)} | TP2={STATE.get('tp2_hit',False)} | "
            f"Qty={STATE.get('remaining_qty',0):.6f} | Conf={decision.confidence:.2f}",
            "INFO",
        )

# ---------- 8. SYNC STATE PATCH ----------
def _sync_position_state_v2(symbol=None):
    result = _original_sync_position_state(symbol)
    try:
        if STATE.get("partial_closed") and STATE.get("open"):
            original_qty = STATE.get("_original_qty")
            if original_qty is None:
                original_qty = STATE.get("qty", 0)
                STATE["_original_qty"] = original_qty
            STATE["qty"] = original_qty
    except Exception:
        pass
    return result

# ---------- 9. INSTALL PATCHES ----------
_original_close_partial = close_partial
_original_close_position_full = close_position_full
_original_sync_position_state = sync_position_state
LiveTradeManager._apply_management_legacy = LiveTradeManager._apply_management

close_partial = _close_partial_v2
close_position_full = _close_full_v2
sync_position_state = _sync_position_state_v2
LiveTradeManager._apply_management = _apply_management_v2

log_execution("[BOARD] ✅ Trade Management Board v1 installed successfully. All original features preserved.", "SUCCESS")

# ====================================================================
# END TRADE MANAGEMENT BOARD v1
# ====================================================================
