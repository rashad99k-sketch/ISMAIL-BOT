...
# ====================================================================
# ENHANCEMENTS (2026-09-11) – TRADE MANAGEMENT COUNCIL:
# - Single Authority: TradeManagementCouncil for all post-entry decisions
# - 9-Council-Member Evidence Gathering (Structure/Liquidity/Zone/Momentum/
#   Volume/SmartMoney/Trend/Risk/Profit)
# - Event-Based TP1 (requires price_reached_tp1 == True)
# - No Early BE Rule
# - Trend Rider Mode
# - State Machine: INITIAL→PROFIT_FORMING→TREND_HEALTHY→TREND_RIDER→
#   PROFIT_PROTECTION→DISTRIBUTION→THESIS_FAILURE→EXIT
# - Confirmation/Hysteresis for dangerous decisions
# - Explainable Decision Object
# - Legacy Profit Engine preserved but bypassed when USE_TRADE_COUNCIL=True
# ====================================================================
...
# ====================================================================
# [COUNCIL-CONFIG] Feature flags — global toggles for Council vs Legacy
# ====================================================================
...
# ============================================================
# DYNAMIC TRADE MANAGER - PRESERVED (not used when Council active)
# ============================================================
class DynamicTradeManager:
    """
    [PRESERVED-LEGACY] Manages an active trade with adaptive SL, multi-stage TP.
    Bypassed when USE_TRADE_COUNCIL=True. Kept for reference & rollback.
    """
...
# ============================================================
# WATCHLIST PRIORITY MANAGER - UNCHANGED
# ============================================================
...
# ====================================================================
# [COUNCIL-CORE] Trade Management Council — SINGLE AUTHORITY
# ====================================================================
...
class CouncilEvidence:
    """All evidence collected from 9 council members in a single cycle."""
    # 1. Structure Council
...
class CouncilDecision:
    """Explainable decision object produced by the Council each cycle."""
...
class TradeManagementCouncil:
    """
    [COUNCIL] Single Authority for post-entry trade management.
    
    Rules:
      - No early BE (Entry + X% → SL to entry is FORBIDDEN)
      - TP1 is EVENT-based (price_reached_tp1 must be True first)
      - All decisions pass through confirmation/hysteresis (except Hard Risk)
      - Trend Rider preserves runner while structure/momentum healthy
      - Full EXIT only on: Hard Risk / Thesis Failure / Confirmed Institutional Exit
    """
    ...
    def evaluate(self, side, entry, mark_price, state_dict, df_live, atr, ob=None,
        ...
        """
        Called each management cycle. Returns a single CouncilDecision.
        All parameters are EVIDENCE; the Council alone decides.
        """
        ev = CouncilEvidence()
        ev.atr = atr or 0.0
        ...
        # Structure bias from side perspective
        if side == "BUY":
    ...
    def _classify_pullback(self, side, ev, df, entry, mark_price, atr):
        """
        Classifies the current adverse move as:
        NONE / MICRO / HEALTHY / FILL_TEST / STRONG / DISTRIBUTION / REVERSAL / THESIS_FAILURE
        Symmetric for BUY & SELL.
        """
        if df is None or len(df) < 5 or entry <= 0 or atr <= 0:
            return PullbackType.NONE.value

        # Is the price against us?
        if side == "BUY":
    ...
    def _decide(self, side, ev, state_dict, df, entry, mark_price, atr):
        """
        Priority ladder per plan §12:
          L1 Risk Emergency -> FULL_EXIT
          L2 Thesis Failure -> FULL_EXIT
          L3 Institutional Distribution -> PARTIAL / PROTECT
          L4 Structure Weakening -> TIGHTEN_TRAIL
          L5 Healthy Pullback -> HOLD
          L6 Trend Continuation -> HOLD + TRAIL
          L7 Strong Trend -> TREND_RIDER
        """
        now = time.time()
        entry_time = float(state_dict.get("entry_time", now) or now) if state_dict else now
        seconds_held = max(0.0, now - entry_time)

        # ---------- L1: Hard Risk Emergency (bypasses confirmation) ----------
        if ev.drawdown_from_peak >= self.HARD_EXIT_ROE_DRAWDOWN and ev.peak_roe > 20:
            ...
            return CouncilDecision(
                action=CouncilAction.EXIT,
                new_state=CouncilState.EXIT,
                reason=f"HARD_RISK: drawdown_from_peak={ev.drawdown_from_peak:.1f}% (peak={ev.peak_roe:.1f}%)",
                evidence=ev, levels_hit=["HARD_RISK_EMERGENCY"]
            )

        # Liquidation proximity proxy: adverse excursion > 80% of SL distance
        ...
        if entry > 0 and sl_price > 0:
            ...
            if sl_dist > 0 and ev.adverse_excursion_pct >= sl_dist * 0.95:
                # Price essentially at SL — the exchange/close will handle; Council confirms EXIT
                self._transition(CouncilState.EXIT, "Adverse excursion at SL boundary")
                return CouncilDecision(
                    action=CouncilAction.EXIT,
                    new_state=CouncilState.EXIT,
                    reason=f"HARD_RISK: adverse excursion {ev.adverse_excursion_pct:.2f}% near SL",
                    evidence=ev, levels_hit=["NEAR_SL"]
                )

        # ---------- L2: Thesis Failure (needs confirmation unless very strong) ----------
        ...
        if ev.has_confirmed_thesis_failure:
            if thesis_failed_strong or self._confirm("thesis_failure"):
                ...
                return CouncilDecision(
                    action=CouncilAction.EXIT,
                    new_state=CouncilState.THESIS_FAILURE,
                    reason=f"THESIS_FAILURE score={ev.thesis_failure_score:.1f} reasons={ev.thesis_failure_reasons}",
                    evidence=ev, levels_hit=["THESIS_FAILURE"]
                )

        # ---------- L3: Institutional Distribution ----------
        ...
        if confirmed_distribution and self._confirm("distribution", required=2):
            ...
            # Partial close only if we have meaningful profit AND haven't partially closed recently
            if (ev.roe > 5.0
                ...
                return CouncilDecision(
                    action=CouncilAction.PARTIAL,
                    new_state=CouncilState.DISTRIBUTION,
                    reason=f"DISTRIBUTION dist_risk={ev.distribution_risk:.1f} roe={ev.roe:.1f}%",
                    partial_ratio=0.40,
                    evidence=ev, levels_hit=["DISTRIBUTION_PARTIAL"]
                )
            else:
                self._transition(CouncilState.PROFIT_PROTECTION, "Distribution evidence – protect")
                return CouncilDecision(
                    action=CouncilAction.PROTECT,
                    new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"PROTECT: distribution evidence without profit threshold (roe={ev.roe:.1f}%)",
                    trail_mult=1.0,
                    evidence=ev, levels_hit=["PROTECT_DISTRIBUTION"]
                )

        # ---------- L4: Structure Weakening / Strong Counter ----------
        if ev.structure_bias in ("BEARISH" if side == "BUY" else "BULLISH",) and ev.counter_pressure > 0.5:
            if self._confirm("struct_weaken", required=2):
                self._transition(CouncilState.PROFIT_PROTECTION, "Structure weakening")
                return CouncilDecision(
                    action=CouncilAction.TIGHTEN_TRAIL,
                    new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"TIGHTEN: structure weak, counter_pressure={ev.counter_pressure:.2f}",
                    trail_mult=1.0,
                    evidence=ev, levels_hit=["STRUCTURE_WEAKEN"]
                )

        # ---------- TP1 EVENT-BASED (plan §8) ----------
        # IMPORTANT: Must check price_reached_tp1 FIRST. No phantom TP1.
        if not (state_dict.get("tp1_hit", False) if state_dict else False):
            if ev.tp1_reached:
                # Price actually reached TP1. Now let Council decide:
                #   If trend strong -> HOLD RUNNER (no partial, expand target)
                #   If distribution -> PARTIAL_PROFIT
                #   If neutral -> standard partial
                trend_strong = (
                    ev.continuation_probability >= 0.7
                    and ev.continuation_probability >= 0.7
                    and ev.structure_health >= 65
                    and ev.momentum_health >= 55
                    and not ev.distribution_risk > 45
                )
                if trend_strong:
                    # Delay TP1 — move synthetic_tp1 further out
                    new_tp1 = entry * (1 + 0.06) if side == "BUY" else entry * (1 - 0.06)
                    self._transition(CouncilState.TREND_HEALTHY, "TP1 reached but trend strong → hold runner")
                    return CouncilDecision(
                        action=CouncilAction.HOLD,
                        new_state=CouncilState.TREND_HEALTHY,
                        reason=f"TP1_REACHED held (strong trend cont={ev.continuation_probability:.2f})",
                        sl_adjust=0.0,
                        trail_mult=2.0,
                        evidence=ev, levels_hit=["TP1_REACHED_HELD"],
                    )
                else:
                    # Standard partial
                    if seconds_held >= COUNCIL_MIN_HOLD_SECONDS:
                        ...
                        return CouncilDecision(
                            action=CouncilAction.PARTIAL,
                            new_state=CouncilState.PROFIT_FORMING,
        ...
        # ---------- L5/L6/L7: Trend continuation / pullback ----------
        pb = ev.pullback_type
        if pb == "MICRO_PULLBACK" or pb == "HEALTHY_PULLBACK" or pb == "FILL_TEST":
            # Trend still valid → HOLD and let it breathe
            if ev.continuation_probability >= 0.65 and ev.structure_health >= 60:
                ...
                return CouncilDecision(
                    action=CouncilAction.HOLD,
                    new_state=CouncilState.TREND_HEALTHY,
                    reason=f"HOLD: pullback={pb} cont={ev.continuation_probability:.2f} struct_health={ev.structure_health:.0f}",
                    trail_mult=2.5,
                    evidence=ev, levels_hit=["HEALTHY_PULLBACK_HOLD"]
                )
            else:
                self._transition(CouncilState.PROFIT_PROTECTION, f"Pullback {pb} but weak continuation")
                return CouncilDecision(
                    action=CouncilAction.PROTECT,
                    new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"PROTECT: pullback={pb} cont={ev.continuation_probability:.2f}",
                    trail_mult=1.2,
                    evidence=ev, levels_hit=["PULLBACK_PROTECT"]
                )
        ...
        if pb in ("STRONG_PULLBACK", "REVERSAL"):
            if self._confirm(f"pb_{pb}", required=2):
                ...
                return CouncilDecision(
                    ...
                    trail_mult=0.9,
                    evidence=ev, levels_hit=[f"PULLBACK_{pb}"]
                )

        # ---------- TREND_RIDER (plan §6) ----------
        # Strong trend, no adverse evidence, allow runner to breathe.
        ...
        if trend_rider_conditions:
            ...
            return CouncilDecision(
                action=CouncilAction.TRAIL,
                new_state=CouncilState.TREND_RIDER,
                reason=f"TREND_RIDER cont={ev.continuation_probability:.2f} adx={ev.adx:.1f} struct={ev.structure_health:.0f}",
                trail_mult=3.5,  # Wide trail to allow pullback
                evidence=ev, levels_hit=["TREND_RIDER"]
            )
        ...
        if ev.continuation_probability >= 0.65 and ev.structure_health >= 55:
            ...
            return CouncilDecision(
                action=CouncilAction.TRAIL,
                new_state=CouncilState.TREND_HEALTHY,
                reason=f"TRAIL: cont={ev.continuation_probability:.2f} struct_health={ev.structure_health:.0f}",
                trail_mult=2.0,
                evidence=ev, levels_hit=["TREND_HEALTHY"]
            )
        ...
        if ev.roe > 0.5:
            ...
            return CouncilDecision(
                ...
                trail_mult=1.5,
                evidence=ev, levels_hit=["PROFIT_FORMING"]
            )
        ...
        return CouncilDecision(
            action=CouncilAction.HOLD,
            new_state=CouncilState.INITIAL,
            reason=f"HOLD: initial (roe={ev.roe:.2f}% struct={ev.structure_bias})",
            trail_mult=0.0,
            evidence=ev, levels_hit=["INITIAL_HOLD"]
        )
...
# ========== TRADE STATE MACHINE (UNCHANGED - legacy) ==========
...
def tg_start(balance, mode):
    send_once(f"🚀 <b>RF v28 Professional Edition (COUNCIL)</b>\nBalance: {balance:.2f} USDT\nMode: {mode}\nCouncil: SINGLE AUTHORITY", "startup", 86400)
...
USE_PPE = True  # [PRESERVED-LEGACY] Profit Engine bypassed when USE_TRADE_COUNCIL=True
...
# ========== EXCHANGE POSITION SYNC (UNCHANGED) ==========
...
def get_mark_price(symbol):
    ...
    return get_ticker_safe(symbol)#!/usr/bin/env python3
# ====================================================================
# RF LIQUIDITY ENGINE v28 – INSTITUTIONAL INTENT EDITION
# [PRODUCTION READY] Institutional Discovery + Dynamic Execution Queue
# ====================================================================
...
def get_mark_price(symbol):
    ...
    if pos and 'markPrice' in pos and pos['markPrice']:
        return float(pos['markPrice'])
    return get_ticker_safe(symbol)# ========== TRADE THESIS ENGINE (UNCHANGED) ==========
...
# ========== LIVE TRADE MANAGEMENT SYSTEM (UNCHANGED STRUCTURE) ==========
...
class PositionSnapshot:
    ...
    def to_dict(self):
        return {
            ...
            "liquidation_price": self.liquidation_price, "tp1_hit": self.tp1_hit,
            "tp2_hit": self.tp2_hit, "trailing_active": self.trailing_active,
            "trailing_stop": self.trailing_stop, "sl_price": self.sl_price,
            "partial_closed": self.partial_closed, "stale": self.stale,
...
# ========== ORDER VERIFICATION HELPER (UNCHANGED) ==========
...
# ====================================================================
# [COUNCIL-BYPASS-GUARD] apply_50_50_profit_engine
# ====================================================================
# LEGACY: Full code preserved below unchanged. When USE_TRADE_COUNCIL=True,
# the Council in LiveTradeManager._apply_management has already made the
# decision. This function is neutralized to prevent over-closing / conflicting
# SL writes. All logic is preserved for rollback (set USE_TRADE_COUNCIL=False).
# ====================================================================
def apply_50_50_profit_engine(df, idx, price, atr, side, entry, state, roe_pct, trade_state=None):
    # -------- [COUNCIL-BYPASS] --------
    if USE_TRADE_COUNCIL:
        # Council already decided. Do NOT override SL/TP here.
...
# ====================================================================
# The Council is now the SINGLE AUTHORITY for post-entry management.
# LiveTradeManager still:
#   - reconciles position from exchange
#   - collects evidence (indicators, smart money, momentum, continuation)
#   - calls _trade_council.evaluate(...)
#   - executes the Council's decision (HOLD/PARTIAL/TIGHTEN_TRAIL/TRAIL/EXIT)
# ====================================================================
class LiveTradeManager:
    ...
    # ---------- LEGACY HELPERS (preserved for rollback & diagnostics) ----------
    ...
    def _apply_management(self, symbol, now):
        ...
        if now - self.last_heavy_calc_ts >= 5:
            ...
            # Note: Legacy thesis failure full-exit is REMOVED here when Council is active.
            # The Council alone will decide on thesis failure EXIT. If USE_TRADE_COUNCIL=False,
            # the legacy behavior below runs.
        ...
        if now - self.last_live_debug_ts >= 5:
            ...
            log_execution(
                ...
                f"TP1HoldScore={tp1_hold_score} | ExitWarning={exit_warning} | Council={council_state}",
                "INFO"
            )

        # ====================================================================
        # [COUNCIL-PRIMARY] When USE_TRADE_COUNCIL=True, delegate to Council.
        # ====================================================================
        if USE_TRADE_COUNCIL:
            ...
            # Log the Council decision each cycle (debounced by action+state)
            log_execution(
            ...
            if action == CouncilAction.PARTIAL:
                ...
                # Move SL to entry after first partial (safe cushion)
                if side == "BUY":
                ...
                # Update dashboard snapshot & continue next cycle
                STATE["council_last_partial_ts"] = time.time()
                return  # do not continue processing this cycle after a partial

            if action in (CouncilAction.TIGHTEN_TRAIL, CouncilAction.TRAIL):
                # Council dictates trail multiplier; we apply structure+ATR aware trail.
                council_mult = decision.trail_mult if decision.trail_mult > 0 else 1.5
                STATE["smart_trail_mult"] = council_mult
                # Build a structure-aware trail floor
            ...
            # action == HOLD: nothing to execute; continue to monitor synthetic SL below.
            # Fall through to synthetic SL hard-check for safety.

        # ====================================================================
        # [LEGACY FALLBACK] When USE_TRADE_COUNCIL=False, run legacy behavior.
        # ====================================================================
        else:
        ...
        # ====================================================================
        # [SAFETY-NET] Always check synthetic SL hit, regardless of Council.
        # This guarantees that the initial protective SL is honored.
        # ====================================================================
...
def sync_position_state(symbol=None):
    ...
    with _TRADE_LOCK:
        if not STATE.get("open"):
            ...
            STATE["side"] = snap.side
            TRADE_STATE.update({
                "entry": snap.entry_price, "qty": snap.qty, "side": snap.side
            })
...
class RFEngine:
    ...
    def compute(self, df, src="close"):
        ...
        return {
            "signal": signal, "triggered": triggered, "filt": filt.iloc[-1],
...
def council_decision_legacy(context):
    # [PRESERVED] renamed to avoid clash with CouncilDecision dataclass
    score = 0
    reasons = []
...
# ========== EXECUTION LAYER STATE (UNCHANGED) ==========
STATE = {
...
# ========== DASHBOARD STATE (UNCHANGED + council) ==========
DASHBOARD_STATE = {
...
def finalize_trade_with_reality(symbol):
    ...
    if PAPER_MODE:
        ...
        side = STATE["side"]
        if side == "BUY":
            pnl_pct = (mark_price - entry) / entry * 100 if mark_price else 0.0
        else:
            pnl_pct = (entry - mark_price) / entry * 100 if mark_price else 0.0
    ...
    TRADE_STATE.update({
        "in_position": False, "symbol": None, "side": None, "entry": 0.0,
        "qty": 0.0, "tp1_hit": False, "tp2_hit": False, "trail_on": False,
    ...
    try:
        tg_close(STATE["current_symbol"] or symbol, pnl_pct, (time.time() - STATE["entry_time"])/60 if STATE.get("entry_time") else 0, STATE["side"] or "?")
    except Exception:
        pass
    with _TRADE_LOCK:
        STATE["open"] = False
...
def dynamic_spread_tolerance(symbol):
    ...
    return MAX_SPREAD_PERCENT_DEFAULT# ========== RF SCANNER (UNCHANGED) ==========
...
def record_watchlist_entry(symbol, side, narrative, score, smart_money=None, momentum=None):
    ...
    entry = {
        ...
        "state": state, "reasons": reasons_list, "trade_type": trade_type,
        "strength": strength, "last_update": now
    }
    if smart_money:
...
def get_vwap_narrative(df):
    ...
    return {
        "vwap": vwap_last, "distance": distance, "above": above,
        "below": below, "reclaim": reclaim, "reject": reject,
...
def classify_market_narrative(df, ob, atr, side, rf_signal):
    ...
    return {
        ...
        "adx_slope": adx_slope,
        "vwap_reclaim": vwap_n["reclaim"], "vwap_reject": vwap_n["reject"]
    }
        ...
        if not any(d["symbol"] == sym for d in debug_data):
            ...
            if df is not None:
                ...
                debug_data.append({
                    ...
                    "score": narrative["narrative_score"],
                    "reasons": narrative["reasons"][:5], "timestamp": time.time()
                })
...
# ========== TRADE MANAGEMENT HELPERS (UNCHANGED) ==========
...
def council_exit(df, price):
    """
    [PRESERVED-LEGACY] Simple safety exit check.
    When USE_TRADE_COUNCIL=True, the Council handles exits and this is only
    used for emergency fallback (ADX collapse, hard SL boundary).
    """
    adx_series = compute_adx(df)
    adx = adx_series.iloc[-1] if adx_series is not None else 0
    # When Council active, do NOT let legacy ADX-collapse force exit
    if not USE_TRADE_COUNCIL and adx < 18:
        log_execution(f"Exit: ADX dropped to {adx:.1f}", "WARN")
        close_position_full()
        return True
    # Hard SL boundary check is always kept (safety net)
...
# ========== FRESH LIQUIDITY RADAR (UNCHANGED) ==========
...
def global_discovery_scan():
    ...
    for item in top_candidates:
        ...
        side = item["side"]
        narrative = {
            "sweep": False, "choch_bos": False, "retest": False,
...
# ========== EXECUTE ENTRY (UNCHANGED + Council hook) ==========
def execute_entry(side, symbol, price, sl, tp1, tp2, score, reason, atr_val, trade_type, entry_type, classification):
    ...
    if PAPER_MODE:
        ...
        STATE.update({
            ...
            "market_regime": regime_class, "adx_live": market_state["adx"],
            ...
            "council_decision": {}, "council_state": "INITIAL",
            "council_last_partial_ts": 0.0,
        })
        TRADE_STATE.update({
            ...
            if is_pivot_high(df, lookback=3):
                ...
                if strong_top:
                    zone = {
                        ...
                        "high": df['high'].iloc[-1], "low": df['high'].iloc[-1] - atr_val * 0.5,
                        "time": time.time(), "state": "WAIT", "confirm_count": 0,
                        "reasons": reasons_top
                    }
            ...
            if is_pivot_low(df, lookback=3):
                ...
                if strong_bottom:
                    zone = {
                        ...
                        "low": df['low'].iloc[-1], "high": df['low'].iloc[-1] + atr_val * 0.5,
                        "time": time.time(), "state": "WAIT", "confirm_count": 0,
...
# ========== DASHBOARD HELPERS (UNCHANGED + Council Panel) ==========
...
def render_live_supervisor_panel():
    ...
    <div id="rf-live-panel" style="display:none;" class="rf-live-supervisor">
      <div class="rf-live-header">
        <span class="rf-live-title">🧠 RF v28 + TRADE MANAGEMENT COUNCIL</span>
        <span id="rf-live-status-badge" class="rf-live-pill rf-live-pill-idle">⚡ COUNCIL SINGLE AUTHORITY</span>
      </div>
      <div class="rf-live-grid">
        ...
        <div class="rf-live-card"><div class="rf-live-metric-icon">⚙️</div><div class="rf-live-metric-label">Council State</div><div class="rf-live-metric-value" id="rf-sup-state">-</div></div>
        ...
        <div class="rf-live-card"><div class="rf-live-metric-icon">🧠</div><div class="rf-live-metric-label">Pullback</div><div class="rf-live-metric-value" id="rf-sup-pb">-</div></div>
      </div>
    ...
    .rf-live-pill-flow-buy{background:rgba(0,255,166,0.1);border-color:#00ffa6;color:#00ffa6;}
    .rf-live-pill-flow-sell{background:rgba(255,77,77,0.1);border-color:#ff4d4d;color:#ff4d4d;}
...
def dashboard():
    ...
    # ===== NEW: COUNCIL PANEL =====
    council_panel_html = """
    <div class="section smart-layer" id="council-panel">
      <div class="title">🏛️ Trade Management Council — Single Authority</div>
      <div class="grid" style="grid-template-columns: repeat(4,1fr); margin-bottom:10px;">
        <div class="card">Council State<div id="council-state" class="green">-</div></div>
        <div class="card">Last Action<div id="council-action">-</div></div>
        <div class="card">Pullback Type<div id="council-pb">-</div></div>
        <div class="card">Reason<div id="council-reason" style="font-size:11px;">-</div></div>
      </div>
      <div class="grid" style="grid-template-columns: repeat(4,1fr); margin-bottom:10px;">
        ...
        <div class="card">Smart Money<div id="council-sm">-</div></div>
      </div>
      <div class="grid" style="grid-template-columns: repeat(4,1fr); margin-bottom:10px;">
        <div class="card">Trend Dir<div id="council-trend">-</div></div>
        <div class="card">Cont. Prob<div id="council-cont">-</div></div>
        <div class="card">Counter Pressure<div id="council-cp">-</div></div>
        <div class="card">Thesis Valid<div id="council-thesis">-</div></div>
      </div>
      <div class="grid" style="grid-template-columns: repeat(4,1fr);">
        <div class="card">ROE<div id="council-roe">-</div></div>
        <div class="card">Peak ROE<div id="council-proe">-</div></div>
        <div class="card">Drawdown<div id="council-dd">-</div></div>
        <div class="card">TP1 Reached<div id="council-tp1">-</div></div>
      </div>
    </div>
    """
    ...
    html = f"""
<!DOCTYPE html>
<html><head><title>RF v28 + Council</title>
...
<body>
<div class="header">🏛️ RF v28 + Trade Management Council</div>
{council_panel_html}
{decision_panel_html}
{scanner_v2_section}
{queue_panel_html}
...
<script>
let lastFetch=0, cachedData=null;
...
function updateUI(d) {{
    ...
    // === COUNCIL PANEL ===
    if(d.council_decision) {{
        const cd=d.council_decision;
        document.getElementById("council-state").innerText=cd.state||"-";
        document.getElementById("council-action").innerText=cd.action||"-";
        document.getElementById("council-reason").innerText=(cd.reason||"-").substring(0,80);
        const ev=cd.evidence||{{}};
        document.getElementById("council-pb").innerText=ev.pullback_type||"-";
        document.getElementById("council-struct").innerText=ev.structure_bias?ev.structure_bias+" ("+(ev.structure_health||0).toFixed(0)+")":"-";
        document.getElementById("council-liq").innerText=ev.liquidity_health?ev.liquidity_health.toFixed(0):"-";
        document.getElementById("council-mom").innerText=ev.momentum_health?ev.momentum_health.toFixed(0):"-";
        document.getElementById("council-sm").innerText=ev.smart_money_bias||"-";
        document.getElementById("council-trend").innerText=ev.trend_direction||"-";
        document.getElementById("council-cont").innerText=ev.continuation_probability?(ev.continuation_probability*100).toFixed(0)+"%":"-";
        document.getElementById("council-cp").innerText=ev.counter_pressure?ev.counter_pressure.toFixed(2):"-";
        document.getElementById("council-thesis").innerText=ev.thesis_valid?"✅ VALID":"❌ FAILED";
        document.getElementById("council-roe").innerText=ev.roe?ev.roe.toFixed(2)+"%":"-";
        document.getElementById("council-proe").innerText=ev.peak_roe?ev.peak_roe.toFixed(2)+"%":"-";
        document.getElementById("council-dd").innerText=ev.drawdown_from_peak?ev.drawdown_from_peak.toFixed(2)+"%":"-";
        document.getElementById("council-tp1").innerText=ev.tp1_reached?"✅":"❌";
    }}
    if(d.position) {{
        ...
        document.getElementById("pos").innerHTML=`
            ...
            <div>Council State: ${{d.position.council_state||'-'}} | Trade State: ${{d.position.trade_state}}</div>
        `;
    }} else {{
    ...
    if(d.live_trade_mode && d.supervisor) {{
        ...
        document.getElementById("rf-sup-state").innerText=sup.council_state||sup.trade_state||"-";
        document.getElementById("rf-sup-trail-mult").innerText=sup.trail_multiplier||"1.5";
        if(d.council_decision && d.council_decision.evidence) {{
            document.getElementById("rf-sup-pb").innerText=d.council_decision.evidence.pullback_type||"-";
        }}
    ...
    let top5Html="";
    (d.top5||[]).forEach(o=>{{top5Html+=`<div><b>${{o.symbol}}</b> | Score: ${{o.score.toFixed(2)}}</div><hr>`;}});
...
def data():
    ...
    try:
        ...
        if DASHBOARD_STATE.get("live_trade_mode", False) and STATE.get("open"):
            supervisor_data = {
                "side": STATE["side"], "entry_price": STATE["entry"],
                ...
                "position_size": STATE["qty"], "leverage": LEVERAGE,
                "tp1_hit": STATE.get("tp1_hit", False),
        ...
        if STATE.get("open") and "dynamic_manager" in STATE:
            ...
            dynamic_trade_data = {
                ...
                "tp1_hit": mgr.tp1_hit, "tp2_hit": mgr.tp2_hit,
                "runner_active": mgr.runner_active,
                "drawdown": mgr.drawdown, "lifecycle": mgr.lifecycle
            }
        else:
            dynamic_trade_data = {
        ...
        council_decision_data = STATE.get("council_decision", {})
        if not council_decision_data and USE_TRADE_COUNCIL and STATE.get("open"):
            council_decision_data = {
                "action": "HOLD",
                "state": _trade_council.state.value,
                "reason": "Awaiting next evaluation cycle",
                "evidence": {}
            }
        ...
        payload = {
            ...
            "council_decision": council_decision_data,
            "council_enabled": USE_TRADE_COUNCIL,
            **live_data
        }
        if USE_EXECUTION_QUEUE:
            queue_status = queue.get_status()
            payload['queue'] = {
                'enabled': True, 'total': queue_status['total_candidates'],
                'ready': queue_status['ready'], 'best_score': queue_status['best_score'],
...
def council_endpoint():
    """Debug endpoint for Council decisions history."""
    return jsonify({
        "state": _trade_council.state.value,
        "decision_log": list(_trade_council.decision_log)[-30:],
        "enabled": USE_TRADE_COUNCIL,
        "config": {
            "hysteresis_cycles": COUNCIL_HYSTERESIS_CYCLES,
            "min_hold_seconds": COUNCIL_MIN_HOLD_SECONDS,
            "partial_cooldown_sec": COUNCIL_PARTIAL_COOLDOWN_SEC,
            "emergency_roe_drawdown": COUNCIL_EMERGENCY_ROE_DRAWDOWN
        }
    })
...
@app.route("/health")
def health():
    return jsonify({"ok": True, "council_enabled": USE_TRADE_COUNCIL, "council_state": _trade_council.state.value})
...
def print_snapshot():
    ...
    perf = get_dashboard_metrics()
    print("\n" + "="*70)
    print(color_text(f"🏛️  RF v28 + COUNCIL ({mode}) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", BOLD))
    ...
    regime = MEMORY.get("regime", "RANGE")
    print(f"🧠 Regime: {color_text(regime, CYAN)} | Scanned: {len(MEMORY.get('top_candidates', []))}")
    print_rf_dashboard()
    if STATE["open"]:
        roe = STATE.get("roe_pct", 0.0)
        roe_colored = color_pnl(roe)
        print(f"📊 POSITION: {STATE['current_symbol']} {STATE['side']} | Entry: {STATE['entry']:.4f} | ROE: {roe_colored}")
        print(f"   🏛️  COUNCIL: state={_trade_council.state.value} | SL={STATE.get('synthetic_sl',0):.4f} | TP1={STATE.get('synthetic_tp1',0):.4f}")
        cd = STATE.get("council_decision", {})
        if cd:
            print(f"   Council Action: {cd.get('action','-')} | Reason: {cd.get('reason','-')[:80]}")
    else:
        print("📊 POSITION: None")
    if USE_EXECUTION_QUEUE:
        qstat = queue.get_status()
        print(f"🎯 QUEUE: {qstat['total_candidates']} candidates, {qstat['ready']} ready, best: {qstat['best_score']:.1f}")
    print("="*70 + "\n")
...
def main_loop_sniper():
    ...
    try:
        ex.load_markets()
        log_execution(f"Markets loaded | Council enabled: {USE_TRADE_COUNCIL}", "INFO")
    except Exception as e:
        log_execution(f"Failed to load markets: {e}", "ERROR")
...
if __name__ == "__main__":
    print("="*70)
    print("🏛️  RF v28 + TRADE MANAGEMENT COUNCIL")
    print(f"   Council Enabled: {USE_TRADE_COUNCIL}")
