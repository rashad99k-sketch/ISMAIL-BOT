#!/usr/bin/env python3
# ====================================================================
# RF LIQUIDITY ENGINE v29 – TRADE MANAGEMENT COUNCIL EDITION
# [PRODUCTION READY] Unified Council-Based Trade Management
# ====================================================================
# ENHANCEMENTS (2026-09-11):
# - Unified Trade Management Council (8 members + RiskOfficer VETO)
# - Single Source of Truth (STATE)
# - Removed all legacy competing exit logic
# - Fixed LifecycleState activation
# - Unified ROE-based PnL accounting
# ====================================================================
...
class SmartMoneyEngine:
    ...
    def _default_state():
        return {
            ...
            "smart_money_dominant": False, "retail_euphoria": False,
            "distribution_risk": 0.0, "accumulation_strength": 0.0,
            "institutional_bias": "NEUTRAL", "institutional_bias_detailed": "NEUTRAL",
            "trend_quality": 0.0, "flow_alignment": 25.0
        }
...
class MomentumFlowEngine:
    ...
    def _default_state():
        return {
            "continuation_strength": 0.0, "momentum_health": 50.0,
            "trend_expansion": False, "momentum_decay": False,
            "climax_risk": 0.0, "exhaustion_risk": 0.0,
            "greed_state": False, "flow_bias": "NEUTRAL"
        }


# ============================================================
# INSTITUTIONAL INTENT ENGINE (9 LAYERS)
# ============================================================
class InstitutionalIntentEngine:
    ...
    def _get_regime_weights(regime):
        if regime == "RANGE":
            return {'liquidity': 20, 'absorption': 18, 'volatility': 10, 'institutional_flow': 15,
                    'structure': 12, 'momentum': 5, 'volume': 8, 'narrative': 12}
        elif regime == "TREND":
            return {'liquidity': 12, 'absorption': 12, 'volatility': 10, 'institutional_flow': 18,
                    'structure': 15, 'momentum': 18, 'volume': 8, 'narrative': 7}
        elif regime == "NEWS":
            return {'liquidity': 10, 'absorption': 15, 'volatility': 5, 'institutional_flow': 20,
                    'structure': 10, 'momentum': 10, 'volume': 20, 'narrative': 10}
        else:
...
class TradeStateMachine:
    ...
    def get_trail_multiplier(self) -> float:
        mult_map = {
            ...
            "PROFIT_DEFENSE": 1.2, "DISTRIBUTION": 1.2, "EXHAUSTION": 1.0,
            "LIQUIDITY_EXHAUSTION": 0.8, "FAKE_BREAKOUT": 0.8, "MOMENTUM_COLLAPSE": 0.6,
            "PANIC_EXIT": 0.5, "RANGE_CHOP": 1.5
        }
...
# ============================================================
# TRADE MANAGEMENT COUNCIL (v29) - THE PROFESSIONAL BOARD
# ============================================================
...
class CouncilContext:
    ...
    regime: str = "UNKNOWN"
    trade_state: str = "RANGE_CHOP"
    adx: float = 20.0
    adx_slope: float = 0.0
    plus_di: float = 20.0
    minus_di: float = 20.0
    structure: str = "NONE"
    pullback_type: str = "NO_PULLBACK"
    trend_health: int = 5
    trend_direction: str = "NEUTRAL"
    nearest_opposing_zone: Optional[float] = None
    zone_strength: float = 0.0
    is_at_opposing_zone: bool = False
...
class TradeManagementCouncil:
    """
    مجلس إدارة الصفقة - يدير كل قرارات الصفقة بعد الدخول.
    8 أعضاء + RiskOfficer بحق النقض (VETO).
    """
    VETO_DRAWDOWN_PCT = 3.5
    PANIC_DRAWDOWN_PCT = 5.0
    TP1_ROE_THRESHOLD = 1.2
    TP2_ROE_THRESHOLD = 2.8
    RUNNER_ROE_THRESHOLD = 4.0

    @staticmethod
    def _trend_governor(ctx: CouncilContext) -> CouncilVote:
        df = ctx.df
        side = ctx.side
        try:
            recent_highs = df['high'].iloc[-10:].max()
            recent_lows = df['low'].iloc[-10:].min()
            prev_highs = df['high'].iloc[-20:-10].max() if len(df) >= 20 else recent_highs
            prev_lows = df['low'].iloc[-20:-10].min() if len(df) >= 20 else recent_lows
            hh = recent_highs > prev_highs
            hl = recent_lows > prev_lows
            lh = recent_highs < prev_highs
            ll = recent_lows < prev_lows
            if hh and hl:
                structural_trend = "BULLISH"
            elif lh and ll:
                structural_trend = "BEARISH"
            else:
        ...
        if side == "BUY":
            if structural_trend == "BULLISH" and ctx.trend_health >= 6:
                return CouncilVote("TrendGovernor", CouncilAction.HOLD, 85, "HH/HL intact", 2.0)
            elif structural_trend == "BEARISH" or ctx.trend_health <= 3:
                return CouncilVote("TrendGovernor", CouncilAction.EXIT_FULL, 80,
                                   f"Trend broken: {structural_trend}, health={ctx.trend_health}", -5.0)
            else:
                return CouncilVote("TrendGovernor", CouncilAction.TIGHTEN_SL, 60,
                                   f"Trend weakening, health={ctx.trend_health}", -1.0)
        else:
            if structural_trend == "BEARISH" and ctx.trend_health >= 6:
                return CouncilVote("TrendGovernor", CouncilAction.HOLD, 85, "LH/LL intact", 2.0)
            elif structural_trend == "BULLISH" or ctx.trend_health <= 3:
                return CouncilVote("TrendGovernor", CouncilAction.EXIT_FULL, 80,
                                   f"Trend broken: {structural_trend}, health={ctx.trend_health}", -5.0)
            else:
                return CouncilVote("TrendGovernor", CouncilAction.TIGHTEN_SL, 60,
    ...
    @staticmethod
    def _liquidity_analyst(ctx: CouncilContext) -> CouncilVote:
        df = ctx.df
        side = ctx.side
            ...
            if len(df) >= 3:
                ...
                if side == "BUY":
                    ...
                    if dist_to_high < 0.003:
                        return CouncilVote("LiquidityAnalyst", CouncilAction.PARTIAL_TP, 75,
                                           f"Approaching liquidity high ({dist_to_high*100:.2f}%)", -2.0)
                    prev_min = df['low'].iloc[-5:-1].min() if len(df) >= 6 else prev['low']
                    if prev['low'] < prev_min and last['close'] > prev['low']:
                        return CouncilVote("LiquidityAnalyst", CouncilAction.HOLD, 70,
                                           "Recent low sweep, bullish", 1.5)
                else:
                    dist_to_low = (price - recent_low) / price
                    if dist_to_low < 0.003:
                        return CouncilVote("LiquidityAnalyst", CouncilAction.PARTIAL_TP, 75,
                                           f"Approaching liquidity low ({dist_to_low*100:.2f}%)", -2.0)
                    prev_max = df['high'].iloc[-5:-1].max() if len(df) >= 6 else prev['high']
                    if prev['high'] > prev_max and last['close'] < prev['high']:
                        return CouncilVote("LiquidityAnalyst", CouncilAction.HOLD, 70,
                                           "Recent high sweep, bearish", 1.5)
        except Exception:
            pass
        return CouncilVote("LiquidityAnalyst", CouncilAction.HOLD, 50, "No liquidity event", 0.0)

    @staticmethod
    def _structure_analyst(ctx: CouncilContext) -> CouncilVote:
        struct = ctx.structure
        side = ctx.side
        if side == "BUY":
            if struct == "bullish_shift":
                return CouncilVote("StructureAnalyst", CouncilAction.HOLD, 80, "Bullish MSS", 2.0)
            elif struct == "bearish_shift":
                return CouncilVote("StructureAnalyst", CouncilAction.EXIT_FULL, 85,
                                   "Bearish CHoCH against long", -4.0)
        else:
            if struct == "bearish_shift":
                return CouncilVote("StructureAnalyst", CouncilAction.HOLD, 80, "Bearish MSS", 2.0)
            elif struct == "bullish_shift":
                return CouncilVote("StructureAnalyst", CouncilAction.EXIT_FULL, 85,
                                   "Bullish CHoCH against short", -4.0)
        return CouncilVote("StructureAnalyst", CouncilAction.HOLD, 50, "No structure event", 0.0)

    @staticmethod
    def _momentum_judge(ctx: CouncilContext) -> CouncilVote:
        adx = ctx.adx
        slope = ctx.adx_slope
        side = ctx.side
        mom = ctx.momentum
        mom_health = mom.get("momentum_health", 50)
        exhaustion = mom.get("exhaustion_risk", 0)
        climax = mom.get("climax_risk", 0)
        decay = mom.get("momentum_decay", False)
        di_ok = (side == "BUY" and ctx.plus_di > ctx.minus_di) or \
                (side == "SELL" and ctx.minus_di > ctx.plus_di)
        if exhaustion > 70 and decay:
            return CouncilVote("MomentumJudge", CouncilAction.EXIT_FULL, 80,
                               f"Momentum exhausted (exh={exhaustion:.0f}, decay)", -4.0)
        if climax > 70:
            return CouncilVote("MomentumJudge", CouncilAction.TIGHTEN_SL, 75,
                               f"Climax risk {climax:.0f}", -2.0)
        if adx > 25 and slope > 0 and di_ok and mom_health > 55:
            return CouncilVote("MomentumJudge", CouncilAction.HOLD, 80,
                               f"ADX={adx:.1f} rising, DI aligned", 2.5)
        if adx < 18 or (slope < -2 and adx > 20):
            return CouncilVote("MomentumJudge", CouncilAction.TIGHTEN_SL, 65,
                               f"ADX weak/falling ({adx:.1f}, slope={slope:.1f})", -1.5)
        if not di_ok:
            return CouncilVote("MomentumJudge", CouncilAction.TIGHTEN_SL, 70,
                               "DI flipped against position", -2.0)
        return CouncilVote("MomentumJudge", CouncilAction.HOLD, 55, "Momentum neutral", 0.0)

    @staticmethod
    def _volume_inspector(ctx: CouncilContext) -> CouncilVote:
        df = ctx.df
        if len(df) < 20:
            return CouncilVote("VolumeInspector", CouncilAction.HOLD, 50, "Insufficient volume data", 0.0)
        vol = df['volume']
        avg = vol.iloc[-20:].mean()
        last = vol.iloc[-1]
        ratio = last / avg if avg > 0 else 1.0
        side = ctx.side
        price_action_up = df['close'].iloc[-1] > df['close'].iloc[-2]
        if ratio < 0.6:
            return CouncilVote("VolumeInspector", CouncilAction.TIGHTEN_SL, 60,
                               f"Volume dying ({ratio:.2f}x avg)", -1.0)
        if ratio > 2.5:
            body = abs(df['close'].iloc[-1] - df['open'].iloc[-1])
            range_ = df['high'].iloc[-1] - df['low'].iloc[-1]
            if range_ > 0 and body / range_ < 0.3:
                return CouncilVote("VolumeInspector", CouncilAction.PARTIAL_TP, 70,
                                   f"Climax volume {ratio:.2f}x weak body", -1.5)
        if ratio > 1.3 and ((side == "BUY" and price_action_up) or (side == "SELL" and not price_action_up)):
            return CouncilVote("VolumeInspector", CouncilAction.HOLD, 70,
                               f"Volume confirms ({ratio:.2f}x)", 1.5)
        return CouncilVote("VolumeInspector", CouncilAction.HOLD, 50, "Volume neutral", 0.0)

    @staticmethod
    def _institutional_flow(ctx: CouncilContext) -> CouncilVote:
        sm = ctx.smart_money
        side = ctx.side
        if not sm:
            return CouncilVote("InstitutionalFlow", CouncilAction.HOLD, 50, "No institutional data", 0.0)
        banker = sm.get("banker_pressure", 50)
        retail = sm.get("retailer_pressure", 50)
        dist_risk = sm.get("distribution_risk", 0)
        acc = sm.get("accumulation_strength", 0)
        dominant = sm.get("smart_money_dominant", False)
        if side == "BUY":
            if dist_risk > 65 and retail > banker:
                return CouncilVote("InstitutionalFlow", CouncilAction.EXIT_FULL, 85,
                                   f"Distribution risk {dist_risk:.0f}", -5.0)
            if dist_risk > 45:
                return CouncilVote("InstitutionalFlow", CouncilAction.PARTIAL_TP, 70,
                                   f"Distribution risk rising ({dist_risk:.0f})", -2.0)
            if banker > retail + 10 and dominant:
                return CouncilVote("InstitutionalFlow", CouncilAction.HOLD, 80,
                                   "Institutional accumulation", 2.0)
        else:
            if acc > 65 and banker > retail:
                return CouncilVote("InstitutionalFlow", CouncilAction.EXIT_FULL, 85,
                                   f"Accumulation {acc:.0f} against short", -5.0)
            if acc > 45:
                return CouncilVote("InstitutionalFlow", CouncilAction.PARTIAL_TP, 70,
                                   f"Accumulation rising ({acc:.0f})", -2.0)
            if retail > banker + 10 and dominant:
                return CouncilVote("InstitutionalFlow", CouncilAction.HOLD, 80,
                                   "Institutional distribution", 2.0)
        return CouncilVote("InstitutionalFlow", CouncilAction.HOLD, 50, "Flow neutral", 0.0)

    @staticmethod
    def _risk_officer(ctx: CouncilContext) -> CouncilVote:
        dd = ctx.drawdown_from_peak
        roe = ctx.roe_pct
        if dd >= TradeManagementCouncil.PANIC_DRAWDOWN_PCT:
            return CouncilVote("RiskOfficer", CouncilAction.VETO_EXIT, 100,
                               f"PANIC drawdown {dd:.2f}% from peak", -10.0)
        if ctx.peak_roe > 8 and dd >= TradeManagementCouncil.VETO_DRAWDOWN_PCT:
            return CouncilVote("RiskOfficer", CouncilAction.VETO_EXIT, 95,
                               f"Protect {ctx.peak_roe:.1f}% profit, dd {dd:.1f}%", -8.0)
        if roe < -2.0 and dd >= 2.0:
            return CouncilVote("RiskOfficer", CouncilAction.EXIT_FULL, 80,
                               f"Loss {roe:.1f}% accelerating", -5.0)
        if ctx.is_at_opposing_zone and ctx.zone_strength >= 6:
            return CouncilVote("RiskOfficer", CouncilAction.TIGHTEN_SL, 80,
                               f"At strong opposing zone (str={ctx.zone_strength:.1f})", -3.0)
        if roe >= TradeManagementCouncil.TP1_ROE_THRESHOLD and not ctx.tp1_hit:
            return CouncilVote("RiskOfficer", CouncilAction.MOVE_TO_BE, 70,
                               f"ROE {roe:.2f}% → lock BE", 1.0)
        return CouncilVote("RiskOfficer", CouncilAction.HOLD, 50, "Risk acceptable", 0.0)

    @staticmethod
    def _profit_taker(ctx: CouncilContext) -> CouncilVote:
        roe = ctx.roe_pct
        tp1_hit = ctx.tp1_hit
        tp2_hit = ctx.tp2_hit
        trend_health = ctx.trend_health
        cont = ctx.momentum.get("continuation_strength", 50)
        mom_health = ctx.momentum.get("momentum_health", 50)
        regime = ctx.regime
        if not tp1_hit and roe >= TradeManagementCouncil.TP1_ROE_THRESHOLD:
            if trend_health >= 8 and cont > 70 and mom_health > 60 and regime in ("STRONG_TREND", "EXPANSION", "TREND"):
                return CouncilVote("ProfitTaker", CouncilAction.ACTIVATE_RUNNER, 75,
                                   f"Strong trend (h={trend_health}, cont={cont:.0f}) → run", 2.0)
            return CouncilVote("ProfitTaker", CouncilAction.PARTIAL_TP, 80,
                               f"TP1 at ROE {roe:.2f}%", 2.0)
        if tp1_hit and not tp2_hit and roe >= TradeManagementCouncil.TP2_ROE_THRESHOLD:
            if trend_health >= 7 and cont > 65:
                return CouncilVote("ProfitTaker", CouncilAction.ACTIVATE_RUNNER, 75,
                                   "Trail for TP2 (strong trend)", 2.0)
            return CouncilVote("ProfitTaker", CouncilAction.PARTIAL_TP, 75,
                               f"TP2 at ROE {roe:.2f}%", 2.0)
        if tp2_hit:
            if trend_health <= 4 or cont < 40:
                return CouncilVote("ProfitTaker", CouncilAction.EXIT_FULL, 80,
                                   f"Trend dying after TP2 (h={trend_health}, cont={cont:.0f})", 0.0)
    ...
    @classmethod
    def convene(cls, ctx: CouncilContext) -> CouncilDecision:
        votes = [
            cls._trend_governor(ctx),
            cls._liquidity_analyst(ctx),
            cls._structure_analyst(ctx),
            cls._momentum_judge(ctx),
            cls._volume_inspector(ctx),
            cls._institutional_flow(ctx),
            cls._profit_taker(ctx),
        ]
        risk_vote = cls._risk_officer(ctx)

        if risk_vote.action == CouncilAction.VETO_EXIT:
            return cls._finalize(CouncilAction.EXIT_FULL, 100, votes + [risk_vote],
                                 f"🚨 RISK OFFICER VETO: {risk_vote.reason}",
                                 exit_urgency="CRITICAL", ctx=ctx)

        exit_votes = [v for v in votes if v.action == CouncilAction.EXIT_FULL]
        if len(exit_votes) >= 3 or (len(exit_votes) >= 2 and risk_vote.action in
                                     (CouncilAction.EXIT_FULL, CouncilAction.VETO_EXIT)):
            reason = " | ".join([v.reason for v in exit_votes[:3]])
            return cls._finalize(CouncilAction.EXIT_FULL, 90, votes + [risk_vote],
                                 f"🚪 Institutional exit: {reason}", exit_urgency="HIGH", ctx=ctx)

        partial_votes = [v for v in votes if v.action == CouncilAction.PARTIAL_TP]
        if len(partial_votes) >= 2:
            reason = " | ".join([v.reason for v in partial_votes[:2]])
            return cls._finalize(CouncilAction.PARTIAL_TP, 80, votes + [risk_vote],
                                 f"💰 Partial profit: {reason}", ctx=ctx, partial_ratio=0.5)

        tighten_votes = [v for v in votes if v.action == CouncilAction.TIGHTEN_SL]
        if len(tighten_votes) >= 2 or risk_vote.action == CouncilAction.TIGHTEN_SL:
            reason = " | ".join([v.reason for v in tighten_votes[:2]] or [risk_vote.reason])
            return cls._finalize(CouncilAction.TIGHTEN_SL, 75, votes + [risk_vote],
                                 f"🔒 Tighten SL: {reason}", ctx=ctx)

        runner_votes = [v for v in votes if v.action == CouncilAction.ACTIVATE_RUNNER]
        if len(runner_votes) >= 1 and ctx.trend_health >= 7:
            return cls._finalize(CouncilAction.ACTIVATE_RUNNER, 75, votes + [risk_vote],
                                 f"🏃 Runner activated: {runner_votes[0].reason}", ctx=ctx)

        if risk_vote.action == CouncilAction.MOVE_TO_BE:
            return cls._finalize(CouncilAction.MOVE_TO_BE, 80, votes + [risk_vote],
                                 f"🛡️ {risk_vote.reason}", ctx=ctx)

        return cls._finalize(CouncilAction.HOLD, 60, votes + [risk_vote],
                             "Consensus: hold position", ctx=ctx)
    ...
    def _finalize(cls, action, confidence, votes, reason, ctx, partial_ratio=0.0, exit_urgency="NORMAL"):
        ...
        if action == CouncilAction.MOVE_TO_BE:
            ...
            atr = ctx.current_atr
            if ctx.side == "BUY":
                candidate = ctx.current_price - atr * 1.0
                new_sl = max(ctx.entry_price, candidate, ctx.trail_stop)
            else:
                candidate = ctx.current_price + atr * 1.0
                trail_ref = ctx.trail_stop if ctx.trail_stop > 0 else ctx.entry_price
                new_sl = min(ctx.entry_price, candidate, trail_ref)
        summary = " | ".join([f"{v.member}:{v.action.value}({v.confidence:.0f})" for v in votes])
        return CouncilDecision(
            action=action, confidence=confidence, votes=votes, reason=reason,
            new_sl=new_sl, new_trail_stop=new_trail_stop, partial_ratio=partial_ratio,
            exit_urgency=exit_urgency, members_summary=summary
        )


# ============================================================
# UNIFIED POSITION MANAGER - THE ONLY TRADE CONTROLLER
# ============================================================
class UnifiedPositionManager:
    def __init__(self, event_bus):
        self.event_bus = event_bus
        self.council = TradeManagementCouncil()
        self.last_run = 0.0
        self.run_interval = 3.0
        self.last_council_decision = None
        self.last_log = 0.0
        self.consecutive_exit_votes = 0

    def _build_context(self) -> Optional[CouncilContext]:
        if not STATE.get("open") or not STATE.get("current_symbol"):
            return None
        symbol = STATE["current_symbol"]
        df_closed = get_ohlcv_safe(symbol, 100)
        if df_closed is None or len(df_closed) < 50:
            return None
        mark_price = STATE.get("mark_price") or get_ticker_safe(symbol)
        if not mark_price or mark_price <= 0:
            return None
        df_live = get_live_hybrid_df(symbol, df_closed, mark_price)
        atr_now = compute_atr(df_live).iloc[-1] if len(df_live) > 14 else mark_price * 0.01
        ...
        regime = MarketRegimeClassifier.classify(df_live)
        structure = detect_structure_shift(df_live) or "NONE"
        pullback = trend_engine.analyze_pullback(df_live, STATE["side"], atr_now)
        trend_health = trend_engine.get_trend_health(df_live, STATE["side"])
        trend_direction = get_trend_direction(df_live)

        nearest_zone = None
        zone_strength = 0.0
        is_at_zone = False
        try:
            nearest_zone, zone_type = find_nearest_opposing_zone(df_live, STATE["side"])
            if nearest_zone:
                dist = abs(mark_price - nearest_zone) / mark_price
                is_at_zone = dist < 0.003
                ob = get_orderbook_cached(symbol, 10)
                zone_strength = compute_opposing_zone_strength(
                    df_live, ob, atr_now, STATE["side"], nearest_zone, zone_type
                )
        except Exception:
            pass
        ...
        dd = max(0.0, peak_roe - roe) if peak_roe > 0 else 0.0

        # Update Trade State Machine
        try:
            adx_val = adx_now if adx_now else 20.0
            new_state = _state_machine.update(smart_money, momentum, adx_val, regime)
            STATE["trade_state"] = new_state
        except Exception:
            pass

        return CouncilContext(
            symbol=symbol, side=STATE["side"], entry_price=STATE["entry"],
            entry_atr=STATE.get("entry_atr", atr_now), current_price=mark_price,
            mark_price=mark_price, current_atr=atr_now, roe_pct=roe,
            peak_roe=peak_roe, drawdown_from_peak=dd, df=df_live, ob=None,
            ...
            remaining_qty=STATE.get("remaining_qty", 0),
            smart_money=smart_money, momentum=momentum, regime=regime,
            trade_state=STATE.get("trade_state", "RANGE_CHOP"),
            adx=adx_now, adx_slope=adx_slope, plus_di=plus_di, minus_di=minus_di,
            structure=structure, pullback_type=pullback, trend_health=trend_health,
            trend_direction=trend_direction, nearest_opposing_zone=nearest_zone,
            zone_strength=zone_strength, is_at_opposing_zone=is_at_zone,
        )

    def _apply_decision(self, decision: CouncilDecision, ctx: CouncilContext):
        action = decision.action
        STATE["last_council_decision"] = {
            "action": action.value, "confidence": decision.confidence,
            "reason": decision.reason, "votes": decision.members_summary,
            "ts": time.time()
        }
        STATE["last_council_action"] = action.value
        STATE["last_council_reason"] = decision.reason
        STATE["last_council_confidence"] = decision.confidence

        if action == CouncilAction.EXIT_FULL:
            log_execution(f"🏛️ [COUNCIL] EXIT_FULL: {decision.reason}", "WARN")
        ...
        if action == CouncilAction.PARTIAL_TP:
            ratio = decision.partial_ratio or 0.5
            log_execution(f"🏛️ [COUNCIL] PARTIAL_TP {ratio*100:.0f}%: {decision.reason}", "SUCCESS")
            close_partial(ratio)
            STATE["tp1_hit"] = True
            STATE["trail_activated"] = True
            if ctx.side == "BUY":
                STATE["synthetic_sl"] = max(STATE.get("synthetic_sl", 0), ctx.entry_price)
            else:
                cur = STATE.get("synthetic_sl", 0)
                if cur == 0:
                    STATE["synthetic_sl"] = ctx.entry_price
                else:
                    STATE["synthetic_sl"] = min(cur, ctx.entry_price)
            STATE["trail_stop"] = STATE["synthetic_sl"]
            return

        if action == CouncilAction.MOVE_TO_BE:
            new_sl = decision.new_sl if decision.new_sl is not None else ctx.entry_price
            old_sl = STATE.get("synthetic_sl", 0)
            if ctx.side == "BUY":
                if new_sl > old_sl:
                    STATE["synthetic_sl"] = new_sl
                    log_execution(f"🏛️ [COUNCIL] MOVE_TO_BE: SL {old_sl:.4f} → {new_sl:.4f}", "INFO")
            else:
        ...
        if action == CouncilAction.TIGHTEN_SL:
            ...
            if new_sl is not None:
                old_sl = STATE.get("synthetic_sl", 0)
                if ctx.side == "BUY" and new_sl > old_sl:
                    STATE["synthetic_sl"] = new_sl
                    STATE["trail_stop"] = max(STATE.get("trail_stop", 0), new_sl)
                    STATE["trail_activated"] = True
                    log_execution(f"🏛️ [COUNCIL] TIGHTEN_SL: {old_sl:.4f} → {new_sl:.4f}", "INFO")
                elif ctx.side == "SELL" and (old_sl == 0 or new_sl < old_sl):
                    STATE["synthetic_sl"] = new_sl
                    if STATE.get("trail_stop", 0) == 0:
                        STATE["trail_stop"] = new_sl
                    else:
                        STATE["trail_stop"] = min(STATE["trail_stop"], new_sl)
                    STATE["trail_activated"] = True
                    log_execution(f"🏛️ [COUNCIL] TIGHTEN_SL: {old_sl:.4f} → {new_sl:.4f}", "INFO")
            return
        ...
        if action == CouncilAction.ACTIVATE_RUNNER:
            ...
            if ctx.side == "BUY":
                ...
                STATE["trail_stop"] = ctx.current_price + ctx.current_atr * 1.5
            log_execution(f"🏛️ [COUNCIL] ACTIVATE_RUNNER: {decision.reason}", "SUCCESS")
            return

        if action == CouncilAction.HOLD and STATE.get("trail_activated", False):
            atr = ctx.current_atr
            mult = STATE.get("smart_trail_mult", 1.5)
            if ctx.side == "BUY":
                new_trail = ctx.current_price - atr * mult
                if new_trail > STATE.get("trail_stop", 0):
                    STATE["trail_stop"] = new_trail
            else:
                new_trail = ctx.current_price + atr * mult
                old_t = STATE.get("trail_stop", 0)
                if old_t == 0 or new_trail < old_t:
                    STATE["trail_stop"] = new_trail
    ...
    def run(self):
        ...
        if ctx.roe_pct > STATE.get("peak_roe", 0.0):
            STATE["peak_roe"] = ctx.roe_pct
            STATE["peak_price"] = ctx.current_price

        # Hard SL safety net
        sl = STATE.get("synthetic_sl", 0)
        if sl and sl > 0:
            if (ctx.side == "BUY" and ctx.current_price <= sl) or \
               (ctx.side == "SELL" and ctx.current_price >= sl):
        ...
        # Trail stop safety net
        trail = STATE.get("trail_stop", 0)
        if STATE.get("trail_activated", False) and trail and trail > 0:
            if (ctx.side == "BUY" and ctx.current_price <= trail) or \
               (ctx.side == "SELL" and ctx.current_price >= trail):
        ...
        if now - self.last_log >= 8:
            self.last_log = now
            log_execution(
                f"🏛️ [COUNCIL] {ctx.symbol} {ctx.side} | ROE={ctx.roe_pct:.2f}% DD={ctx.drawdown_from_peak:.2f}% | "
                f"Action={decision.action.value} (conf={decision.confidence:.0f}) | {decision.reason}",
                "INFO"
            )
            log_execution(f"    Votes: {decision.members_summary}", "INFO")
...
PERF = {
    "total_pnl_pct": 0.0, "total_pnl_usdt": 0.0, "trades": 0,
    "wins": 0, "losses": 0, "last_trade": None
}
...
class TradeThesisEngine:
    def build_thesis(self, symbol: str, side: str, trade_type: str, market_state: Dict,
        ...
        return TradeThesis(
            thesis_id=f"{symbol}_{int(time.time())}", symbol=symbol, side=side,
            trade_type=trade_type, created_at=time.time(), entry_reason=reasons,
            continuation_factors=continuation, invalidation_factors=invalidation,
            risk_factors=risks, market_context=market_state, confidence=round(confidence, 2),
...
class ContinuationPressureEngine:
    ...
    def calculate_pressure(df, side, entry_price, atr, entry_time):
        ...
        if side == "BUY" and adx_state["slope"] > 0 and di_state["dominant"] == "BUY":
            ...
            score -= 10; reasons.append("adx_falling")
...
class ThesisFailureEngine:
    ...
    def evaluate_failure(thesis: Dict, market_state: Dict, current_price, entry_price, side):
        ...
        if market_state.get("strong_reclaim", False):
            failure_score += 30; reasons.append("strong_reclaim")
        df = market_state.get("df", None)
        di_state = ADXDIIntelligence.get_di_state(df) if df is not None else {"dominant": "NEUTRAL"}
        if side == "BUY" and di_state.get("dominant") == "SELL":
            failure_score += 25; reasons.append("di_flip_bearish")
        elif side == "SELL" and di_state.get("dominant") == "BUY":
            failure_score += 25; reasons.append("di_flip_bullish")
        adx_state = ADXDIIntelligence.get_adx_state(df) if df is not None else {"state": "UNKNOWN", "value": 20}
        if adx_state.get("state") == "CHOP" and adx_state.get("value", 20) < 18:
            failure_score += 20; reasons.append("adx_collapse")
        ...
        if side == "SELL" and last_candle.get("close", 0) > last_candle.get("open", 0):
            ...
            if body > atr * 0.6:
                failure_score += 20; reasons.append("strong_bullish_candle")
        elif side == "BUY" and last_candle.get("close", 0) < last_candle.get("open", 0):
            body = abs(last_candle.get("close",0)-last_candle.get("open",0))
            if body > atr * 0.6:
                failure_score += 20; reasons.append("strong_bearish_candle")
        cont = market_state.get("continuation_pressure", 50)
        if cont < 30:
            failure_score += 25; reasons.append("low_continuation_pressure")
        return failure_score >= 50, reasons, failure_score
...
class MarketRegimeClassifier:
    ...
    def classify(df, ob=None):
        ...
        range_pct = (range_20 / price) * 100 if price > 0 else 0
...
class ConfidenceEngine:
    ...
    def calculate_initial_confidence(entry_score, narrative_score, regime, adx, di_spread, location_quality):
        ...
        return min(100, base + regime_bonus + adx_bonus + di_bonus + location_bonus)
    ...
    def apply_institutional_modifiers(base_confidence, smart_money, momentum, continuation_strength):
        ...
        if smart_money.get("smart_money_dominant", False): conf += 10
        else: conf -= 8
...
class ContinuationProbabilityEngine:
    ...
    def evaluate(self, side: str, df, market_state: Dict, thesis: Dict) -> ContinuationEvaluation:
        ...
        di_spread = (di_plus - di_minus) if side == "BUY" else (di_minus - di_plus)
        ...
        thesis_strength = thesis.get("thesis_strength", 5) if isinstance(thesis, dict) else 5
...
# ========== LIFECYCLE / SNAPSHOT / EVENT BUS / SYNC ==========
...
class PositionSnapshot:
    def __init__(self):
        self.symbol = None; self.side = None; self.qty = 0.0
        self.entry_price = 0.0; self.mark_price = 0.0
        self.unrealized_pnl = 0.0; self.realized_pnl = 0.0
        self.roe_pct = 0.0; self.leverage = LEVERAGE; self.margin = 0.0
        self.liquidation_price = 0.0; self.tp1_hit = False
        self.tp2_hit = False; self.trailing_active = False
        self.trailing_stop = 0.0; self.sl_price = 0.0
        self.partial_closed = False; self.stale = False
        self.updated_at = 0.0; self.source = "unknown"
...
class ExchangeSyncService:
        ...
        try:
            ...
            if snapshot.margin > 0:
                ...
                if snapshot.side == "BUY":
                    raw_move = (snapshot.mark_price - snapshot.entry_price)/snapshot.entry_price*100
                else:
                    raw_move = (snapshot.entry_price - snapshot.mark_price)/snapshot.entry_price*100
    ...
    def _paper_snapshot(self, symbol):
        ...
        if snap.side == "BUY":
            snap.unrealized_pnl = (snap.mark_price - snap.entry_price) * snap.qty
        else:
            snap.unrealized_pnl = (snap.entry_price - snap.mark_price) * snap.qty
    ...
    def reconcile(self, symbol, local_state):
        ...
        with _TRADE_LOCK:
            ...
            TRADE_STATE.update({
                "symbol": symbol, "side": snap.side, "entry": snap.entry_price,
...
def close_partial(ratio):
    ...
    try:
        ...
        if order is None:
            log_execution("[CLOSE_PARTIAL] Order failed", "ERROR")
            return
        order_id = order.get('id')
        if not order_id:
            log_execution("[CLOSE_PARTIAL] No order ID", "ERROR")
            return
        ...
        if filled:
            ...
            if abs(current_qty - expected_remaining) < 0.0001 * max(expected_remaining, 1e-6):
                STATE["remaining_qty"] = current_qty
                TRADE_STATE["qty"] = current_qty
                log_execution(f"[CLOSE_PARTIAL] Partial confirmed, remaining: {current_qty:.6f}", "SUCCESS")
            else:
                STATE["remaining_qty"] = current_qty
                TRADE_STATE["qty"] = current_qty
                if current_qty <= 0:
            ...
            log_execution(f"[CLOSE_PARTIAL] Fill timeout", "ERROR")
    except Exception as e:
...
def close_position_full():
    ...
    try:
        ...
        try:
            ...
            log_execution(f"[CLOSE] Emergency failed: {e}", "ERROR")
        return False
    except Exception as e:
...
# ========== LIVE TRADE MANAGER (v29 - Simplified) ==========
class LiveTradeManager:
    """Live Trade Manager - delegates ALL management to UnifiedPositionManager."""
    ...
    def _on_reconciled(self, snapshot):
        ...
        # ★★★ CRITICAL FIX: activate LIVE from any pre-live state ★★★
        if self.lifecycle_state in (TradeLifecycleState.RECOVERING,
                                     TradeLifecycleState.OPEN_PENDING_CONFIRMATION,
                                     TradeLifecycleState.OPEN_REQUESTED,
                                     TradeLifecycleState.IDLE):
    ...
    def manage_live_trade(self):
        """SINGLE entry point for managing all open trades."""
        ...
        if self.lifecycle_state != TradeLifecycleState.LIVE:
            ...
            DASHBOARD_STATE["lifecycle_state"] = "LIVE"
        now = time.time()
        # Sync from exchange periodically
        ...
        # ★ Unified Council manages everything
    ...
    def _log_live_status(self):
        ...
        council_action = STATE.get("last_council_action", "HOLD")
        log_msg = (f"{BLUE}[LIVE]{RESET} {direction_icon} {STATE['current_symbol']} {side} | "
                   f"Entry: {STATE['entry']:.2f} | Mark: {STATE.get('mark_price',0):.2f} | "
                   f"ROE: {roe_color} | SL: {STATE.get('synthetic_sl',0):.4f} | "
                   f"Trail: {'✅' if STATE.get('trail_activated',False) else '❌'} | "
                   f"TP1: {'✅' if STATE.get('tp1_hit',False) else '❌'} | "
                   f"Council: {council_action}")
...
STATE = {
    ...
    "continuation_probability": 0.5, "hold_quality": "UNKNOWN",
    "counter_pressure": 0.0, "reclaim_risk": 0.0, "trend_strength": 0.0,
    "continuation_reasons": [], "trade_thesis": None,
    "current_confidence": 50.0, "market_regime": "UNKNOWN",
    "continuation_pressure": 50, "thesis_failure_score": 0,
    "prev_di_spread": 0.0, "adx_live": 0.0, "di_plus_live": 0.0, "di_minus_live": 0.0,
...
DASHBOARD_STATE = {
    ...
    "position": None, "logs": [], "errors": [],
    "live_trade_mode": False, "lifecycle_state": "IDLE",
    "live_supervisor": {}, "institutional_flow": {}
}
...
# ========== FINALIZE TRADE (UNIFIED PnL) ==========
def finalize_trade_with_reality(symbol):
    """★ Unified: always uses ROE%"""
    mark_price, unrealized, initial_margin, roe = sync_position_state(symbol)
    if mark_price is None and not PAPER_MODE:
        mark_price = get_ticker_safe(symbol) or STATE.get("mark_price", STATE["entry"])

    entry = STATE["entry"]
    side = STATE["side"]
    leverage = STATE.get("leverage", LEVERAGE)

    # ★ UNIFIED: raw_move * leverage = ROE%
    if entry and entry > 0:
        if side == "BUY":
            raw_move_pct = (mark_price - entry) / entry * 100
        else:
            raw_move_pct = (entry - mark_price) / entry * 100
    else:
        raw_move_pct = 0.0

    pnl_pct = raw_move_pct * leverage  # ROE%
    ...
    if margin > 0:
        ...
        qty = STATE.get("qty", 0)
        pnl_usdt = raw_move_pct / 100 * entry * qty if entry else 0
    ...
    TRADE_STATE.update({
        "in_position": False, "symbol": None, "side": None, "entry": 0.0, "qty": 0.0,
        "tp1_hit": False, "tp2_hit": False, "trail_on": False,
        "zone": None, "location": None, "reason": []
    })
    DASHBOARD_STATE["live_trade_mode"] = False
    log_execution(f"Trade closed: {result} ROE={pnl_pct:.2f}% | USDT: {pnl_usdt:+.2f}",
    ...
    with _TRADE_LOCK:
        ...
        STATE["scale_ins"] = 0
        STATE["last_council_action"] = "N/A"
        STATE["last_council_reason"] = ""
        STATE["last_council_confidence"] = 0
        STATE["peak_roe"] = 0.0
...
def get_realized_pnl_for_symbol(symbol, lookback_seconds=30):
    ...
    try:
        ...
        for trade in trades:
            ...
            qty = trade['amount']; price = trade['price']
            cost = qty * price
...
def compute_atr(df, period=14):
    ...
    atr = rma(tr, period)
    atr = atr.bfill().ffill().fillna(tr.mean()).clip(lower=1e-8)
    return atr
...
def volume_pressure_real(df, window=20, threshold=1.2):
    ...
    return (vol.iloc[-1] - mean) / std > threshold
...
def flow_engine(df):
    ...
    if vol > avg_vol * 1.5:
        return "aggressive_buy" if body > 0 else "aggressive_sell"
...
def detect_walls(ob, depth=10, threshold=3.0):
    ...
    bid_wall = any(s > (sum(bid_sizes)/len(bid_sizes)) * threshold for s in bid_sizes) if bid_sizes else False
    ask_wall = any(s > (sum(ask_sizes)/len(ask_sizes)) * threshold for s in ask_sizes) if ask_sizes else False
...
class RFEngine:
    ...
    def compute(self, df, src="close"):
        ...
        return {"signal": signal, "triggered": triggered, "filt": filt.iloc[-1],
                "h_band": h.iloc[-1], "l_band": l.iloc[-1], "distance": distance}
...
def is_pinbar(candle, atr, side, body_atr_min=0.5, wick_body_ratio=2.5, wick_range_ratio=0.6):
    ...
    if side == "BUY":
        return (lower_wick >= wick_body_ratio * body and
                lower_wick / range_ >= wick_range_ratio and body / atr >= body_atr_min)
    else:
        return (upper_wick >= wick_body_ratio * body and
                upper_wick / range_ >= wick_range_ratio and body / atr >= body_atr_min)
...
def detect_location(df, price, supports, resistances, threshold=0.003):
    ...
    if supports:
        if min(abs(price - s) / price for s in supports) < threshold:
            near_support = True
    if resistances:
        if min(abs(price - r) / price for r in resistances) < threshold:
...
def advanced_decision_engine(scenario, adx, volume_state, location):
    ...
    if volume_state == "exhaustion": return "SKIP", None
...
def detect_bos(df, lookback=5):
    ...
    return current_close > recent_high, current_close < recent_low
...
def get_clustered_zones(df, lookback=120, cluster_pct=0.002):
    ...
    swing_highs = [highs[i] for i in range(2, len(highs)-2) if highs[i] == max(highs[i-2:i+3])]
    swing_lows = [lows[i] for i in range(2, len(lows)-2) if lows[i] == min(lows[i-2:i+3])]
    def cluster(points, pct):
        ...
        return clusters
    return cluster(swing_lows, cluster_pct), cluster(swing_highs, cluster_pct)
...
def detect_sweep_v2(df, side):
    ...
    if side == "BUY":
        ...
        return sweep_broke, (wick_ratio > 0.6 and last['close'] > last['low'])
    else:
        upper_wick = last['high'] - max(last['open'], last['close'])
        wick_ratio = upper_wick / range_
        high_cluster_max = df['high'].iloc[-21:-1].max()
        sweep_broke = eqh and (last['high'] > high_cluster_max)
        return sweep_broke, (wick_ratio > 0.6 and last['close'] < last['high'])
...
def build_liquidity_pools(df):
    ...
    pools_high = recent_highs if (len(recent_highs) >= 2 and equal_levels(recent_highs)) else ([sh[-1][1]] if sh else [])
    recent_lows = [p[1] for p in sl[-3:]] if len(sl) >= 3 else [sl[-1][1]] if sl else []
    pools_low = recent_lows if (len(recent_lows) >= 2 and equal_levels(recent_lows)) else ([sl[-1][1]] if sl else [])
...
def classify_sweep(df, side):
    ...
    if side == "BUY":
        lower_wick = min(last['open'], last['close']) - last['low']
        wick_ratio = lower_wick / range_
        if wick_ratio > 0.6 and last['close'] > last['low']: return "strong", 3
        elif wick_ratio > 0.3: return "weak", 1.5
        else: return "fake", -2
    else:
        upper_wick = last['high'] - max(last['open'], last['close'])
        wick_ratio = upper_wick / range_
        if wick_ratio > 0.6 and last['close'] < last['high']: return "strong", 3
        elif wick_ratio > 0.3: return "weak", 1.5
        else: return "fake", -2
...
def structure_engine(df):
    ...
    if len(sh) >= 2 and last_close > sh[-2][1]: bos = True
    if len(sl) >= 2 and last_close < sl[-2][1]: bos = True
...
def pre_rf_context_boost(df, side):
    ...
    if side == "BUY":
        if last2['close'].iloc[-2] < last2['close'].iloc[-1] and last2['close'].iloc[-1] < df['close'].iloc[-1]:
            boost += 1; reasons.append("consecutive_bullish")
        if (last2['close'].iloc[-1] - last2['low'].iloc[-1]) / (last2['high'].iloc[-1] - last2['low'].iloc[-1] + 1e-9) > 0.7:
            boost += 1; reasons.append("strong_bullish_candle")
    else:
        if last2['close'].iloc[-2] > last2['close'].iloc[-1] and last2['close'].iloc[-1] > df['close'].iloc[-1]:
            boost += 1; reasons.append("consecutive_bearish")
        if (last2['high'].iloc[-1] - last2['close'].iloc[-1]) / (last2['high'].iloc[-1] - last2['low'].iloc[-1] + 1e-9) > 0.7:
            boost += 1; reasons.append("strong_bearish_candle")
...
def council_decision(context):
    ...
    if context["location"] == "discount" and context["side"] == "BUY":
        score += 3; reasons.append("discount_location")
    elif context["location"] == "premium" and context["side"] == "SELL":
        score += 3; reasons.append("premium_location")
    ...
    return max(0, min(12, score)), reasons
...
# ========== EVENT BUS + MANAGERS INSTANTIATION ==========
_event_bus = EventBus()
...
_unified_manager = UnifiedPositionManager(_event_bus)   # ★ SINGLE TRADE CONTROLLER
_state_machine = TradeStateMachine()                     # ★ for trade_state updates
...
def sync_position_state(symbol=None):
    if PAPER_MODE:
        if STATE.get("open"):
            ...
            if price:
                if STATE["side"] == "BUY":
                    raw_pnl = (price - STATE["entry"])/STATE["entry"]*100
                else:
                    raw_pnl = (STATE["entry"]-price)/STATE["entry"]*100
                roe_pct = raw_pnl * LEVERAGE
                STATE["roe_pct"] = roe_pct
                STATE["mark_price"] = price
                if STATE["side"] == "BUY":
                    STATE["unrealized_pnl_usdt"] = (price - STATE["entry"]) * STATE["qty"]
                else:
                    STATE["unrealized_pnl_usdt"] = (STATE["entry"] - price) * STATE["qty"]
    ...
    with _TRADE_LOCK:
        if not STATE.get("open"):
            ...
            STATE["entry_time"] = time.time()
            TRADE_STATE.update({
                "in_position": True, "symbol": symbol, "side": snap.side,
...
def detect_reset(df, zone_price, zone_type):
    last_close = df['close'].iloc[-1]
    if zone_type == "TOP": return (zone_price - last_close) / zone_price >= 0.003
    else: return (last_close - zone_price) / zone_price >= 0.003
...
def confirm_reversal(df, ob, zone_type):
    ...
    return sum([wick_reject, macd_flip, flow_agree, obi_agree]) >= 2
...
def detect_stop_hunt(df):
    ...
    if swept_high and reclaim and volume_pressure_real(df): return True, "SELL"
    elif swept_low and reclaim and volume_pressure_real(df): return True, "BUY"
    return False, None
...
def smart_decision(df, ob, symbol):
    ...
    if is_zone and zone_price is not None: STATE["zone"][symbol] = (zone_price, zone_type)
...
def find_nearest_opposing_zone(df, side):
    ...
    if side == "BUY":
        ...
        if valid:
            return min(valid, key=lambda x: x - price), "RESISTANCE"
    else:
        valid = [s for s in supports if s < price]
        if valid:
            return max(valid, key=lambda x: x), "SUPPORT"
    return None, None
...
def compute_opposing_zone_strength(df, ob, atr, side, zone_price, zone_type):
    ...
    adx_series = compute_adx(df)
    if len(adx_series) >= 2 and adx_series.iloc[-1] < adx_series.iloc[-2]: score += 1
    return score
...
def get_vwap_narrative(df):
    ...
    return {"vwap": vwap_last, "distance": distance, "above": above, "below": below,
...
def compute_enhanced_zone_strength(df, level, zone_type, atr, ob, sweep_detected=False):
    price = df['close'].iloc[-1]
    touches = 0; rejection_count = 0; volume_at_touches = []
...
def classify_market_narrative(df, ob, atr, side, rf_signal):
    ...
    if (side == "BUY" and (bos_up or struct_shift == "bullish_shift")):
        score += 2.0; reasons.append("Bullish structure")
    elif (side == "SELL" and (bos_down or struct_shift == "bearish_shift")):
        score += 2.0; reasons.append("Bearish structure")
    vol_state = classify_volume(df)
    ...
    if detect_displacement(df, side, atr, vol_state, body_atr_threshold=0.8, volume_expansion_required=False):
        score += 1.5; reasons.append("Displacement")
    ...
    if adx is not None and adx < 18 and plus_di is not None and abs(plus_di - minus_di) < 6:
        score = 0; reasons = ["CHOP market (ADX<18 + DI tangled)"]
    if score >= 9.0:
        ...
        classification = "CHOP_NO_TRADE"; confidence = "NO_TRADE"
...
def detect_market_regime(df):
    ...
    try:
        ...
        atr_avg = atr_series.rolling(20).mean().iloc[-1] if len(atr_series)>=20 else atr
...
def adjust_narrative_confidence(narrative, regime, side, trend_direction):
    orig_conf = narrative["confidence"]; score = narrative["narrative_score"]
    side_aligned = (trend_direction == "BULLISH" and side == "BUY") or (trend_direction == "BEARISH" and side == "SELL")
...
def check_institutional_entry(symbol, side, df, ob, atr, price):
    ...
    if intent_score < 75: return False, None, f"Intent score {intent_score}"
    ...
    if not zone_ok:
        ...
        if side == "BUY" and fvg and fvg[0] == "bullish":
            if price >= fvg[1] and price <= fvg[2]: zone_ok = True
        elif side == "SELL" and fvg and fvg[0] == "bearish":
            if price >= fvg[1] and price <= fvg[2]: zone_ok = True
    if not zone_ok:
        ob_level = detect_order_block(df, side)
        if side == "BUY" and ob_level and abs(price - ob_level["low"]) / price < 0.003: zone_ok = True
        elif side == "SELL" and ob_level and abs(price - ob_level["high"]) / price < 0.003: zone_ok = True
    if not zone_ok: return False, None, "No strong zone tap"
    struct_shift = detect_structure_shift(df)
    bos_up, bos_down = detect_bos(df)
    choch_ok = (side == "BUY" and (struct_shift == "bullish_shift" or bos_up)) or \
               (side == "SELL" and (struct_shift == "bearish_shift" or bos_down))
    if sweep_ok and not choch_ok: return False, None, "Reversal requires MSS/CHoCH"
    ...
    if not (rejection_ok or displacement_ok): return False, None, "No rejection/displacement"
    if vol_state not in ("expansion", "spike"): return False, None, "No volume expansion"
    adx_series = compute_adx(df)
    ...
    if adx_now > 50:
        if adx_slope > 0 and di_spread > 8: pass
        else: return False, None, f"Exhaustion: ADX>50 slope={adx_slope:.1f}"
    elif adx_now > 35:
        if adx_slope > 0: pass
        else: return False, None, f"ADX high but falling"
    else:
        if adx_slope > 0: pass
        else: return False, None, f"ADX not rising"
    rf = RFEngine(20, 3.5).compute(df)
    if rf["signal"] != side: return False, None, f"RF signal {rf['signal']} != {side}"
    if abs(rf["distance"]) > 0.003: return False, None, f"RF distance too far"
    if zone_price:
        move_from_zone = abs(price - zone_price) / zone_price * 100
        if move_from_zone > 0.5: return False, None, f"Price moved too late"
    last_candle = df.iloc[-1]
    candle_range_pct = (last_candle['high'] - last_candle['low']) / last_candle['close'] * 100
    if candle_range_pct > 1.5 * (atr / price * 100): return False, None, "Large candle, too late"
...
def decision_score(df, ob, atr_val, side):
    ...
    return total, scenario, side, reasons
...
def compute_zone_strength(df, level, zone_type, atr, ob):
    ...
    if ob:
        obi = orderbook_imbalance(ob)
        if zone_type == "support" and obi > 0.1: liquidity_score = 2
        elif zone_type == "resistance" and obi < -0.1: liquidity_score = 2
        elif abs(obi) > 0.05: liquidity_score = 1
    inst_score = 0
    bos_up, bos_down = detect_bos(df, lookback=5)
    struct_shift = detect_structure_shift(df)
    if zone_type == "support" and (bos_up or struct_shift == "bullish_shift"): inst_score = 2
    elif zone_type == "resistance" and (bos_down or struct_shift == "bearish_shift"): inst_score = 2
    rejection_score = 0
    if len(df) >= 1:
        last = df.iloc[-1]
        body, range_, upper_wick, lower_wick = candle_metrics(last)
        if zone_type == "support" and lower_wick > body * 1.5 and abs(last['low'] - level) < atr: rejection_score = 2
        elif zone_type == "resistance" and upper_wick > body * 1.5 and abs(last['high'] - level) < atr: rejection_score = 2
    total = vol_strength + reaction_score + liquidity_score + inst_score + rejection_score
    strength = min(10.0, total * 10 / 10)
    return round(strength, 1), {"vol_strength": round(vol_strength, 1), "reaction_count": reaction_count,
                                 "liquidity_score": liquidity_score, "institutional_score": inst_score,
...
def equal_levels_points(highs, lows, tolerance=0.002):
    ...
    def cluster(points, tol):
        ...
        return clusters
    for cl in cluster(highs, tolerance):
        if len(cl) >= 2: eq_highs.append(sum(cl)/len(cl))
    for cl in cluster(lows, tolerance):
        if len(cl) >= 2: eq_lows.append(sum(cl)/len(cl))
...
def record_watchlist_entry(symbol, side, narrative, score, smart_money=None, momentum=None):
    ...
    entry = {"symbol": symbol, "side": side, "score": round(score, 2), "state": state,
             "reasons": reasons_list, "trade_type": trade_type, "strength": strength,
             "last_update": now}
    if smart_money:
...
def evaluate_liquidity_narrative(df, ob, atr, side):
    ...
    bos_up, bos_down = detect_bos(df)
    struct_shift = detect_structure_shift(df)
    if side == "BUY" and (bos_up or (struct_shift == "bullish_shift")): narrative["choch_bos"] = True
    elif side == "SELL" and (bos_down or (struct_shift == "bearish_shift")): narrative["choch_bos"] = True
...
def smart_opportunity_selection():
    ...
    best_setup = None; best_score = -1
    for cand in candidates[:15]:
        try:
            sym = cand["symbol"]; side = cand["side"]
            df = get_ohlcv_safe(sym, 100)
    ...
    if best_setup and best_score >= 9:
        ...
        return execute_entry(side, sym, price, sl, tp1, tp2, score, reason_str, atr,
    ...
    for c in watchlist:
        ...
        if ob is not None:
            ...
            if decision == "STOP_HUNT":
                ...
                reason_str = f"SMART_STOP_HUNT mode={dec_info.get('mode')}"
        ...
        if total_score >= 7:
            ...
            reason_str = f"UNIFIED_SNIPER ({scenario_name}) score={total_score}"
...
def fast_market_filter(df):
    ...
    atr = compute_atr(df).iloc[-1]
    if vol_usdt < 1_000_000: return False
...
def detect_sweep_simple(df):
    return detect_liquidity_context(df) is not None

def radar_score(df):
    score = 0
    if accumulation_v2(df): score += 3
    if volume_pressure_real(df): score += 2
    if detect_sweep_simple(df): score += 2
    if near_key_zone(df, df['close'].iloc[-1]): score += 2
    return score
...
def store_intent_for_symbol(symbol):
    try:
        ...
        intent_score, intent_status, intent_details = InstitutionalIntentEngine.detect(df, ob, symbol)
        if intent_score >= 0:
            MEMORY[f"intent_{symbol}"] = {"score": intent_score, "status": intent_status, "details": intent_details}
    except: pass
...
def radar_entry_scan():
    if not MEMORY.get("radar_top5"): return
    now = time.time()
    for entry in MEMORY["radar_top5"]:
        sym = entry["symbol"]
        last = LAST_ENTRY_PER_SYMBOL.get(sym, 0)
        if now - last < RADAR_COOLDOWN_SEC: continue
    ...
    def compute_liquidity_score(df):
        ...
        for i in range(-5, 0):
            ...
            if len(sub_df) >= 2:
                ...
                if swept_h or swept_l: sweep_count += 1
...
def get_sector(symbol):
    ...
    for sector, keywords in SECTOR_MAP.items():
        if any(kw in base for kw in keywords): return sector
    return "OTHER"
...
def get_volume_growth(sym):
    ...
    vol = df['volume']
    recent_avg = vol.iloc[-5:].mean(); older_avg = vol.iloc[-20:-5].mean()
...
def select_sector_leaders():
    sectors = set(SECTOR_MAP.keys()); leaders = []
    for sector in sectors:
        symbols_in_sector = [s for s in get_usdt_perp_symbols() if get_sector(s) == sector][:20]
        if not symbols_in_sector: continue
        best = None; best_score = -1e9
        for sym in symbols_in_sector:
            vol_growth = get_volume_growth(sym); momentum = get_price_momentum(sym)
            score = vol_growth * 10 + momentum
            if score > best_score: best_score = score; best = sym
        if best: leaders.append({"symbol": best, "score": round(best_score, 2), "sector": sector})
...
class WatchlistRotation:
    def __init__(self, symbols_40):
        self.symbols = symbols_40; self.batch_size = 6
        self.current_index = 0; self.last_rotate = time.time()
...
def build_40_symbol_universe():
    ...
    for c in MEMORY.get("radar_top5", []): strong_set.add(c["symbol"])
    for c in MEMORY.get("rf_watchlist", []): strong_set.add(c["symbol"])
    ...
    seen = set(); unique_universe = []
    for sym in universe:
        if sym not in seen: seen.add(sym); unique_universe.append(sym)
...
class OrderBlockQuality(Enum):
    FRESH = "FRESH"; TESTED = "TESTED"; WEAK = "WEAK"; BROKEN = "BROKEN"; FAKE = "FAKE"

class InstitutionalBehaviour(Enum):
    ACCUMULATION = "ACCUMULATION"; DISTRIBUTION = "DISTRIBUTION"
    RE_ACCUMULATION = "RE_ACCUMULATION"; RE_DISTRIBUTION = "RE_DISTRIBUTION"
    NEUTRAL = "NEUTRAL"

class MarketStructure(Enum):
    BOS = "BOS"; CHOCH = "CHOCH"; MSS = "MSS"; NONE = "NONE"
...
class ZoneMetrics:
    ...
    def final_zone_score(self) -> float:
        weights = {'order_block_quality': 0.20, 'zone_strength': 0.18, 'liquidity_quality': 0.15,
                   'institutional_confidence': 0.15, 'structure_alignment': 0.12, 'entry_timing': 0.10,
                   'trend_alignment': 0.05, 'risk_score': 0.05}
        score = 0.0
        for attr, w in weights.items(): score += getattr(self, attr, 50) * w
...
class ExecutionCandidate:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            'priority_score': self.priority_score, 'state': self.state.value,
...
class ExecutionQueue:
    ...
    def add_candidate(self, candidate: ExecutionCandidate) -> bool:
        with self._lock:
            if len(self._candidates) >= self._max_size:
                lowest = min([(s, c) for s, c in self._candidates.items() if c.state != ExecutionState.READY],
                             key=lambda x: x[1].priority_score, default=None)
                if lowest:
                    self._candidates.pop(lowest[0]); self.total_rejected += 1
                else: return False
    ...
    def re_evaluate_all(self, data_fetcher):
        ...
        with self._lock:
            for symbol, cand in list(self._candidates.items()):
                ...
                if df is None or len(df) < 30:
                    self._invalidate(symbol, "Insufficient data"); continue
                current_price = df['close'].iloc[-1]
                atr = compute_atr(df).iloc[-1] if len(df) > 14 else current_price * 0.01
                cand.atr = atr
                if self._is_extended(cand, current_price):
                    self._return_to_watchlist(symbol, "Price extended"); continue
                if self._is_order_block_broken(cand, current_price):
                    self._invalidate(symbol, "Order block broken"); continue
                ...
                metrics = ZoneMetrics(order_block_quality=ob_score, zone_strength=zone_score,
                ...
                cand.zone_metrics = metrics; cand.opportunity_type = opp_type
                cand.market_structure = struct_type; cand.institutional_behaviour = behaviour
                cand.last_evaluated = time.time(); cand.evaluation_count += 1
                cand.priority_score = metrics.final_zone_score
                self._update_state(cand, current_price)
                self.total_evaluations += 1
                if cand.priority_score < 30: self._return_to_watchlist(symbol, "Score too low")
    ...
    def _is_order_block_broken(self, cand, price):
        ...
        else: return price > cand.entry_price + cand.atr * 0.8
    ...
    def _evaluate_order_block(self, df, side, atr):
        ...
        body = abs(last['close'] - last['open'])
        range_ = last['high'] - last['low']
        if range_ == 0: return 30, OrderBlockQuality.WEAK
        if side == "BUY":
            lower_wick = min(last['open'], last['close']) - last['low']
            ...
            else: return 50, OrderBlockQuality.WEAK
        else:
            upper_wick = last['high'] - max(last['open'], last['close'])
    ...
    def _evaluate_zone_strength(self, df, side, atr, entry_price):
        touches = 0; rejections = 0; vol_sum = 0
            ...
            if side == "BUY":
                if abs(candle['low'] - entry_price) < atr * 0.5:
                    ...
                    if df['close'].iloc[i+1] > candle['close']:
                        rejections += 1; vol_sum += candle['volume']
            else:
    ...
    def _evaluate_liquidity(self, df, side, atr):
        pools = build_liquidity_pools(df)
        swept_high, swept_low = detect_sweep(df, pools)
        stop_hunt, hunt_side = detect_stop_hunt(df)
        eq_highs, eq_lows = detect_equal_highs_lows(df)
        score = 50
        if side == "BUY":
    ...
    def _evaluate_institutional(self, df, side):
        ...
        if smart.get('smart_money_dominant', False):
            ...
            if (side == "BUY" and smart['institutional_bias'] == "BUY") or \
               (side == "SELL" and smart['institutional_bias'] == "SELL"): score += 15
        dist = smart.get('distribution_risk', 0)
    ...
    def _evaluate_structure(self, df, side):
        bos_up, bos_down = detect_bos(df)
        struct_shift = detect_structure_shift(df)
        score = 50; struct_type = MarketStructure.NONE
        if side == "BUY":
            if struct_shift == "bullish_shift": score = 90; struct_type = MarketStructure.MSS
            elif bos_up: score = 70; struct_type = MarketStructure.BOS
        else:
            if struct_shift == "bearish_shift": score = 90; struct_type = MarketStructure.MSS
            elif bos_down: score = 70; struct_type = MarketStructure.BOS
    ...
    def _evaluate_timing(self, df, side, atr, current_price, entry_price):
        ...
        if range_ > 0:
            if side == "BUY":
                lower_wick = min(last['open'], last['close']) - last['low']
                if lower_wick / range_ > 0.5 and last['close'] > last['open']: score += 20
            else:
                upper_wick = last['high'] - max(last['open'], last['close'])
                if upper_wick / range_ > 0.5 and last['close'] < last['open']: score += 20
        vol_avg = df['volume'].iloc[-10:].mean()
        if vol_avg > 0 and df['volume'].iloc[-1] > 1.5 * vol_avg: score += 10
        return min(100, max(0, score))
    ...
    def _classify_opportunity(self, metrics, struct_type, side, df):
        ...
        if score >= 85 and struct_type != MarketStructure.NONE: return OpportunityType.INSTITUTIONAL_REVERSAL
        elif score >= 70 and struct_type == MarketStructure.BOS: return OpportunityType.BREAKOUT_RETEST
        elif score >= 60 and struct_type != MarketStructure.NONE: return OpportunityType.TREND_CONTINUATION
        elif side == "BUY" and metrics.institutional_confidence > 70: return OpportunityType.ACCUMULATION_ENTRY
        elif side == "SELL" and metrics.institutional_confidence > 70: return OpportunityType.DISTRIBUTION_ENTRY
        elif metrics.order_block_quality < 40: return OpportunityType.FAKE_BREAKOUT
        elif metrics.order_block_quality < 50: return OpportunityType.WEAK_ORDER_BLOCK
    ...
    def _detect_institutional_behaviour(self, df, side):
        ...
        dist = smart.get('distribution_risk', 0)
        acc = smart.get('accumulation_strength', 0)
        if side == "BUY" and banker > retail and dist < 30 and acc > 60: return InstitutionalBehaviour.ACCUMULATION
        if side == "SELL" and banker < retail and dist > 50: return InstitutionalBehaviour.DISTRIBUTION
        if side == "BUY" and dist > 50 and acc > 50: return InstitutionalBehaviour.RE_ACCUMULATION
        if side == "SELL" and dist < 30 and acc > 50: return InstitutionalBehaviour.RE_DISTRIBUTION
    ...
    def _detect_trigger_state(self, df, side, atr, entry_price):
        ...
        if sweep_ok and (bos_ok or choch_ok) and rejection_ok: return "MSS_CONFIRMED"
        elif sweep_ok and near_entry and rejection_ok: return "LIQUIDITY_SWEEP"
        elif bos_ok and displacement_ok: return "BOS_CONFIRMED"
        elif choch_ok and displacement_ok: return "CHOCH_CONFIRMED"
        elif sweep_ok and not (bos_ok or choch_ok): return "MITIGATION"
        elif near_entry and (bos_ok or choch_ok): return "WAITING_TRIGGER"
        elif near_entry: return "MITIGATION"
        elif displacement_ok: return "DISPLACEMENT"
    ...
    def _update_state(self, cand, price):
        ...
        elif score >= 70 and trigger == "MITIGATION": cand.state = ExecutionState.ENTRY_VALIDATION
        elif score >= 70: cand.state = ExecutionState.WAITING_TRIGGER
        elif score >= 55: cand.state = ExecutionState.GOOD_ZONE
        ...
        with self._lock:
            ...
            for symbol, cand in self._candidates.items():
                if cand.state in (ExecutionState.EXECUTED, ExecutionState.INVALIDATED, ExecutionState.RETURNED_WATCHLIST):
                    ...
                    to_remove.append(symbol)
            for sym in to_remove: self._candidates.pop(sym, None)
...
def global_discovery_scan():
    ...
    for item in top_candidates:
        ...
        if item["source"] == "scanner_v2": narrative["sweep"] = True
        elif item["source"] == "rf": narrative["rf_alignment"] = True
        elif item["source"] == "fresh": narrative["volume_confirmation"] = True
        record_watchlist_entry(sym, side, narrative, item["score"])
        store_intent_for_symbol(sym)
    MEMORY["radar_top5"] = [{"symbol": c["symbol"], "score": c["score"]} for c in top_candidates[:5]]
    log_execution(f"[DISCOVERY] Done in {time.time() - start_time:.1f}s", "INFO")

def promote_to_queue():
    if not USE_EXECUTION_QUEUE: return
    if STATE.get("open") or TRADE_STATE.get("in_position"): return
    watchlist = []
    for source in (MEMORY.get("watchlist", {}).values(), MEMORY.get("rf_watchlist", []),
                   MEMORY.get("scanner_v2_buy", []), MEMORY.get("scanner_v2_sell", [])):
        if isinstance(source, dict):
            for item in source.values():
                if isinstance(item, dict) and "symbol" in item: watchlist.append(item)
    ...
    for item in watchlist:
        sym = item.get('symbol')
        if not sym: continue
        score = item.get('score', 0); side = item.get('side', 'BUY')
    ...
    for sym, data in sorted_items[:30]:
        if sym in queue._candidates: continue
        df = get_ohlcv_safe(sym, 100)
        ...
        candidate = ExecutionCandidate(
            symbol=sym, side=side, price=price, entry_price=price, stop_loss=sl,
            take_profit_1=tp1, take_profit_2=tp2, atr=atr, df=df, ob=ob,
            zone_metrics=metrics, original_score=data.get('score', 0),
...
def process_queue_entry():
    ...
    success = execute_entry(
        best.side, best.symbol, best.price, best.stop_loss, best.take_profit_1, best.take_profit_2,
        best.original_score, f"QUEUE: {best.opportunity_type.value} (Zone Score: {best.zone_metrics.final_zone_score})",
...
MEMORY = {
    ...
    "radar_watchlist": [], "radar_top5": [], "log_debounce": {},
    "watchlist": {}, "no_entry_feed": [], "decision_log": []
}

# ========== EXECUTE ENTRY (FIXED - no DynamicTradeManager) ==========
def execute_entry(side, symbol, price, sl, tp1, tp2, score, reason, atr_val, trade_type, entry_type, classification):
    ...
    else: balance = usable_balance

    if classification in ("SNIPER", "INSTITUTIONAL_SNIPER"):
        margin_percent = 0.40; trade_type_label = "STRONG"
    elif classification == "TREND":
        margin_percent = 0.30; trade_type_label = "NORMAL"
    elif classification == "LOW":
        margin_percent = 0.15; trade_type_label = "LOW_CONF"
    else:
        margin_percent = 0.30; trade_type_label = "NORMAL"
    ...
    qty = notional / price
    log_execution(f"[SIZING] Free={free_bal:.2f} Usable={balance:.2f} Type={trade_type_label} Margin={margin:.2f} Qty={qty:.6f}", "INFO")
    ...
    if df is not None:
        last = df.iloc[-1]
        if side == "BUY":
            if last['close'] < last['open'] and abs(last['close'] - last['open']) < atr_val * 0.3: weak_pullback = True
        else:
            if last['close'] > last['open'] and abs(last['close'] - last['open']) < atr_val * 0.3: weak_pullback = True
    structure_aligned = False
    struct_shift = detect_structure_shift(df) if df is not None else None
    if side == "BUY" and struct_shift == "bullish_shift": structure_aligned = True
    elif side == "SELL" and struct_shift == "bearish_shift": structure_aligned = True
    counter_displacement = 0.0
    if df is not None:
        last = df.iloc[-1]
        if side == "SELL" and last['close'] > last['open']:
            body = abs(last['close'] - last['open'])
            if body > atr_val * 0.6: counter_displacement = body / atr_val
    ...
    market_state = {
        ...
        "regime": MEMORY.get("regime", "UNKNOWN"), "di_dominance": di_dominance,
        "weak_pullback": weak_pullback, "structure_aligned": structure_aligned,
    ...
    initial_conf = ConfidenceEngine.calculate_initial_confidence(score, 0, regime_class, market_state["adx"], di_spread, "mid")

    if df is not None:
        smart_money = SmartMoneyEngine.analyze_smart_money(df)
        momentum = MomentumFlowEngine.analyze_momentum_flow(df)
        initial_conf = ConfidenceEngine.apply_institutional_modifiers(initial_conf, smart_money, momentum,
                                                                       momentum.get("continuation_strength", 50))
    intent_info = MEMORY.get(f"intent_{symbol}", {})
    intent_score = intent_info.get("score", 0)
    if intent_score >= 85: initial_conf += 15
    elif intent_score >= 75: initial_conf += 10
    ...
    if PAPER_MODE:
        ...
        STATE.update({
            ...
            "trade_thesis": thesis.__dict__, "current_confidence": initial_conf,
            ...
            "peak_roe": 0.0, "peak_price": price, "peak_unrealized_pnl": 0.0,
            "drawdown_from_peak": 0.0, "tp1_hold_score": 10, "exit_warning": 0,
    ...
    if qty < min_qty:
        log_execution(f"SKIP: qty {qty:.6f} below min {min_qty}", "WARN")
        return False
...
def rf_proximity_score(rf, adx_val, vol_ok, rsi_val, atr_pct):
    ...
    return float((proximity * 0.35) + (trend * 0.25) + (vol_score * 0.15) + (rsi_score * 0.1) + (vol_boost * 0.05) + trigger_boost)
...
def scan_market_rf(top_n=40):
    ...
    for sym in symbols[:150]:
        try:
            ...
            status = "TRIGGERED" if rf["triggered"] else ("READY" if score >= 0.6 else "PROXIMITY")
            results.append({"symbol": sym, "score": round(score, 3), "rf_signal": rf["signal"],
    ...
    for sym in symbols:
        try:
            ...
            if final_score_buy >= 5:
                buy_candidates.append({"symbol": sym, "score": round(final_score_buy, 2),
                                        "rf_prox": round(rf_prox*100, 3), "liquidity": liquidity_ctx,
                                        "zone": zone_ctx, "structure": structure_ctx,
                                        "rejection": rejection_buy, "volume_spike": vol_spike_flag,
                                        "location": location,
                                        "smart_money": {"bias": smart_money["institutional_bias"],
                                        ...
                                        "momentum": {"expansion": momentum["trend_expansion"],
            ...
            if final_score_sell >= 5:
                sell_candidates.append({"symbol": sym, "score": round(final_score_sell, 2),
                                         "rf_prox": round(rf_prox*100, 3), "liquidity": liquidity_ctx,
                                         "zone": zone_ctx, "structure": structure_ctx,
                                         "rejection": rejection_sell, "volume_spike": vol_spike_flag,
...
def update_position_dashboard(symbol, side, entry, qty, pnl=0.0):
    DASHBOARD_STATE["position"] = {
        "symbol": symbol, "side": side, "entry": round(entry, 4), "qty": qty,
...
def dashboard():
    ...
    rf_html = "".join([f"<div>{item['icon']} {item['symbol']} | {item['status']} | score={item['score']:.2f}</div>" for item in rf_items])
    return f"""<!DOCTYPE html>
<html><head><title>RF v29 Council Edition</title>
...
.orange{{color:#f1c40f}} .grey{{color:#95a5a6}}
.log,.err{{max-height:220px;overflow:auto;white-space:pre-wrap;font-size:12px}}
.council-box{{background:linear-gradient(145deg,#0a1a0a 0%,#0a0f0a 100%);border:1px solid #2ecc71;border-radius:12px;padding:12px;margin-top:8px}}
</style></head><body>
<div class="header">🏛️ RF v29 Trade Council Edition</div>
...
<div class="section"><div class="title">🏛️ COUNCIL DECISION</div>
...
<button class="btn" onclick="manualTrade('SELL')">SELL</button>
<button class="btn" onclick="manualClose()">CLOSE</button>
</div>
<script>
let lastFetch = 0; let cachedData = null;
async function fetchData() {{
    const now = Date.now();
    if (cachedData && (now - lastFetch) < 5000) {{ updateUI(cachedData); return; }}
    lastFetch = now;
    try {{ const r = await fetch('/data'); const d = await r.json(); cachedData = d; updateUI(d); }}
...
function updateUI(d) {{
    ...
    if(d.position) {{
        ...
        document.getElementById("pos").innerHTML = `
            ...
            <div>Entry: ${{d.position.entry}} | ROE: <span class="${{pnlClass}}">${{d.position.pnl}}%</span></div>
...
async function manualTrade(side){{ await fetch('/trade',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{side:side}})}}); }}
...
def data():
    ...
    try:
        bal = get_balance_safe(); free_bal = get_free_balance_safe()
        ...
        if STATE["open"] and STATE.get("current_symbol"):
            pos = {
                ...
                "entry": round(STATE["entry"],4), "qty": STATE["qty"],
                "pnl": round(STATE.get("roe_pct",0), 2),
        ...
        payload = {
            "balance": bal, "free_balance": free_bal, "mode": mode,
            "stats": DASHBOARD_STATE["stats"], "position": pos,
            "logs": DASHBOARD_STATE["logs"][-30:], "errors": DASHBOARD_STATE["errors"][-10:],
            "last_scan": MEMORY["last_scan"], "regime": MEMORY["regime"], "health": health,
...
def get_dashboard_metrics():
    ...
    return {"winrate": f"{winrate:.1f}%", "total_pnl": f"{total_pnl:+.2f}%",
            "total_pnl_usdt": PERF["total_pnl_usdt"], "last_trade": last_txt,
            "trades": PERF["trades"], "wins": PERF["wins"], "losses": PERF["losses"]}
...
def print_snapshot():
    ...
    regime = MEMORY.get("regime", "RANGE")
    print(f"🧠 Regime: {color_text(regime, CYAN)}")
    if STATE["open"]:
        roe = STATE.get("roe_pct", 0.0)
        ...
        council = STATE.get("last_council_action", "N/A")
        print(f"   🏛️ Council: {color_text(council, MAGENTA)} | {STATE.get('last_council_reason','')[:80]}")
...
def build_rf_dashboard():
    ...
    for c in candidates:
        dashboard.append({"symbol": c["symbol"], "status": c.get("status", "PROXIMITY"),
...
def run_scanner_v2():
    try:
        ...
        log_execution(f"Scanner v2 error: {traceback.format_exc()}", "ERROR")
...
def update_institutional_flow_scanner():
    try:
        ...
        DASHBOARD_STATE["institutional_flow"] = {
            "banker_pressure": smart["banker_pressure"], "retailer_pressure": smart["retailer_pressure"],
            ...
            "flow_alignment": smart["flow_alignment"], "distribution_risk": smart["distribution_risk"],
            ...
            "greed_state": mom["greed_state"], "smart_money_dominant": smart["smart_money_dominant"]
        }
    except: pass
...
def main_loop_sniper():
    ...
    run_scanner_v2()
    updater_thread = threading.Thread(target=live_institutional_updater, daemon=True)
        ...
        try:
            ...
            if TRADE_STATE["in_position"] or STATE["open"]:
                ...
                if sym:
                    ...
                    if price and price > 0:
                        ...
                        if df is not None:
                            update_position_dashboard(sym, STATE["side"], STATE["entry"], STATE["qty"],
                                                       STATE.get("roe_pct", 0.0))
                print_snapshot()
                hourly_cleanup()
                time.sleep(BASE_SLEEP)
                continue  # ★ NO competing exit logic below
            ...
            if now - last_candidate_scan >= CANDIDATE_SCAN_INTERVAL:
                if watchlist_rotation and watchlist_rotation.should_rotate():
                    watchlist_rotation.get_next_batch()
