#!/usr/bin/env python3
# ====================================================================
# RF LIQUIDITY ENGINE v28 – INSTITUTIONAL INTENT + COUNCIL EDITION
# ====================================================================
...
import copy
from dataclasses import dataclass, field   # [FIX-1] Moved to top
...
# [COUNCIL-CONFIG]
# ====================================================================
USE_TRADE_COUNCIL = os.getenv("USE_TRADE_COUNCIL", "True") == "True"
COUNCIL_HYSTERESIS_CYCLES = int(os.getenv("COUNCIL_HYSTERESIS_CYCLES", "3"))
COUNCIL_EMERGENCY_ROE_DRAWDOWN = float(os.getenv("COUNCIL_EMERGENCY_ROE_DRAWDOWN", "40.0"))
COUNCIL_MIN_HOLD_SECONDS = int(os.getenv("COUNCIL_MIN_HOLD_SECONDS", "45"))
COUNCIL_PARTIAL_COOLDOWN_SEC = int(os.getenv("COUNCIL_PARTIAL_COOLDOWN_SEC", "60"))
...
class SmartMoneyEngine:
    ...
    def analyze_smart_money(df):
        ...
        return {
            "banker_pressure": sf(banker_pressure.iloc[-1]),
            "retailer_pressure": sf(retailer_pressure.iloc[-1]),
            "hot_money_pressure": sf(hot_money_pressure.iloc[-1]),
            "smart_money_dominant": bool(smart_money_dominant),
            "retail_euphoria": bool(retail_euphoria),
            "distribution_risk": sf(distribution_risk),
            "accumulation_strength": sf(accumulation_strength),
            "institutional_bias": institutional_bias,
            "institutional_bias_detailed": institutional_bias_detailed,
            "trend_quality": sf(trend_quality),
            "flow_alignment": sf(flow_alignment)
        }

    @staticmethod
    def _default_state():
        return {
            "banker_pressure": 50.0, "retailer_pressure": 50.0,
            "hot_money_pressure": 50.0, "smart_money_dominant": False,
            "retail_euphoria": False, "distribution_risk": 0.0,
            "accumulation_strength": 0.0, "institutional_bias": "NEUTRAL",
            "institutional_bias_detailed": "NEUTRAL", "trend_quality": 0.0,
            "flow_alignment": 25.0
        }
...
class MomentumFlowEngine:
    ...
    def analyze_momentum_flow(df):
        ...
        return {
            "continuation_strength": sf(continuation_strength),
            "momentum_health": sf(momentum_health),
            "trend_expansion": bool(trend_expansion),
            "momentum_decay": bool(momentum_decay),
            "climax_risk": sf(climax_risk),
            "exhaustion_risk": sf(exhaustion_risk),
...
class InstitutionalIntentEngine:
    ...
    def _detect_absorption_sequence(df, window=4):
        ...
        for i in range(window):
            ...
            if upper_wick > rng * 0.4: wick_score += 1
            if lower_wick > rng * 0.4: wick_score += 1
    ...
    def _get_regime_weights(regime):
        if regime == "RANGE":
            ...
            return {k: 12.5 for k in ['liquidity','absorption','volatility',
                                     'institutional_flow','structure','momentum','volume','narrative']}


# ============================================================
# DYNAMIC TRADE MANAGER — PRESERVED (bypassed when Council active)
# ============================================================
...
# ============================================================
# [COUNCIL-CORE] Enums, Evidence, Decision
# ============================================================
...
class CouncilEvidence:
    ...
    mss_up: bool = False
    mss_down: bool = False
    liquidity_sweep_supportive: bool = False
    liquidity_sweep_against: bool = False
    liquidity_health: float = 50.0
    entry_zone_score: float = 50.0
    zone_still_valid: bool = True
    zone_fresh: bool = True
    momentum_health: float = 50.0
    ...
    di_minus: float = 20.0
    di_spread: float = 0.0
    volume_state: str = "neutral"
    volume_supportive: bool = False
    volume_against: bool = False
    smart_money_bias: str = "NEUTRAL"
    smart_money_bias_detailed: str = "NEUTRAL"
    smart_money_dominant: bool = False
    distribution_risk: float = 0.0
    accumulation_strength: float = 50.0
    trend_direction: str = "NEUTRAL"
    trend_strength: float = 0.5
    continuation_probability: float = 0.5
    counter_pressure: float = 0.0
    hold_quality: str = "UNKNOWN"
    atr: float = 0.0
    atr_ratio: float = 1.0
    ...
    thesis_failure_score: float = 0.0
    thesis_failure_reasons: list = None
    has_confirmed_distribution: bool = False
    has_confirmed_thesis_failure: bool = False


@dataclass
class CouncilDecision:
    action: CouncilAction = CouncilAction.HOLD
    new_state: CouncilState = CouncilState.INITIAL
    reason: str = ""
    sl_adjust: float = 0.0
    trail_mult: float = 0.0
    partial_ratio: float = 0.0
    evidence: Any = None
    levels_hit: list = field(default_factory=list)
    ...
    def to_dict(self):
        ...
        return {
            ...
            "reason": self.reason,
            "sl_adjust": self.sl_adjust,
            "trail_mult": self.trail_mult,
            "partial_ratio": self.partial_ratio,
            "levels_hit": self.levels_hit,
            "timestamp": self.timestamp,
            "evidence": {
                "structure_bias": ev.structure_bias if ev else "NEUTRAL",
                "structure_health": ev.structure_health if ev else 50.0,
                "liquidity_health": ev.liquidity_health if ev else 50.0,
                "entry_zone_score": ev.entry_zone_score if ev else 50.0,
                "momentum_health": ev.momentum_health if ev else 50.0,
                "adx": ev.adx if ev else 20.0,
                "di_spread": ev.di_spread if ev else 0.0,
                "volume_state": ev.volume_state if ev else "neutral",
                "smart_money_bias": ev.smart_money_bias if ev else "NEUTRAL",
                "distribution_risk": ev.distribution_risk if ev else 0.0,
                "trend_direction": ev.trend_direction if ev else "NEUTRAL",
                "trend_strength": ev.trend_strength if ev else 0.5,
                "continuation_probability": ev.continuation_probability if ev else 0.5,
                "counter_pressure": ev.counter_pressure if ev else 0.0,
                "pullback_type": ev.pullback_type if ev else "NONE",
                "thesis_valid": ev.thesis_valid if ev else True,
                "thesis_failure_score": ev.thesis_failure_score if ev else 0.0,
                "thesis_failure_reasons": ev.thesis_failure_reasons if ev else [],
                "roe": ev.roe if ev else 0.0,
                "peak_roe": ev.peak_roe if ev else 0.0,
                "drawdown_from_peak": ev.drawdown_from_peak if ev else 0.0,
                "tp1_reached": ev.tp1_reached if ev else False,
                "tp2_reached": ev.tp2_reached if ev else False,
                "runner_mode": ev.runner_mode if ev else False,
            } if ev else {}
        }


class TradeManagementCouncil:
    HARD_EXIT_ROE_DRAWDOWN = COUNCIL_EMERGENCY_ROE_DRAWDOWN

    def __init__(self):
        self.state = CouncilState.INITIAL
        self.cycles_in_state = 0
        self.state_started_at = time.time()
        self._confirmation_buffer = {}
        self._last_partial_ts = 0.0
        self._has_partially_closed = False
        self._last_decision = None
        self.decision_log = deque(maxlen=200)
        self.trail_mult_current = 1.5
    ...
    def _confirm(self, key, required=None):
        required = required or COUNCIL_HYSTERESIS_CYCLES
        self._confirmation_buffer[key] = self._confirmation_buffer.get(key, 0) + 1
        for k in list(self._confirmation_buffer.keys()):
            if k != key:
                self._confirmation_buffer[k] = max(0, self._confirmation_buffer[k] - 1)
        return self._confirmation_buffer[key] >= required

    def _transition(self, new_state, reason=""):
        if new_state != self.state:
            log_execution(f"[COUNCIL] State {self.state.value} -> {new_state.value} | {reason}", "INFO")
    ...
    def evaluate(self, side, entry, mark_price, state_dict, df_live, atr, ob=None,
                 thesis_dict=None, smart_money=None, momentum=None,
                 trend_engine=None, continuation_engine=None, continuation_eval=None,
                 plus_di=None, minus_di=None, adx_now=None, adx_slope=None):
        ev = CouncilEvidence()
        ev.atr = atr or 0.0
        ...
        if entry > 0:
            if side == "BUY":
                ev.roe = ((mark_price - entry) / entry) * 100 * LEVERAGE
            else:
                ev.roe = ((entry - mark_price) / entry) * 100 * LEVERAGE
        ev.peak_roe = state_dict.get("peak_roe", ev.roe) if state_dict else ev.roe
        ev.drawdown_from_peak = max(0.0, ev.peak_roe - ev.roe)

        synthetic_tp1 = state_dict.get("synthetic_tp1", 0.0) if state_dict else 0.0
        tp2_price = state_dict.get("tp2_price", 0.0) if state_dict else 0.0
        if synthetic_tp1 and mark_price > 0:
            if side == "BUY":
                ev.tp1_reached = mark_price >= synthetic_tp1
            else:
                ev.tp1_reached = mark_price <= synthetic_tp1
        if tp2_price and mark_price > 0:
            if side == "BUY":
                ev.tp2_reached = mark_price >= tp2_price
            else:
                ev.tp2_reached = mark_price <= tp2_price
        ev.runner_mode = state_dict.get("runner_mode", False) if state_dict else False

        smart_money = smart_money or {}
        momentum = momentum or {}
        ev.smart_money_bias = smart_money.get("institutional_bias", "NEUTRAL")
        ev.smart_money_bias_detailed = smart_money.get("institutional_bias_detailed", "NEUTRAL")
        ev.smart_money_dominant = bool(smart_money.get("smart_money_dominant", False))
        ev.distribution_risk = float(smart_money.get("distribution_risk", 0.0) or 0.0)
        ev.accumulation_strength = float(smart_money.get("accumulation_strength", 50.0) or 50.0)

        ev.momentum_health = float(momentum.get("momentum_health", 50.0) or 50.0)
        ev.momentum_decay = bool(momentum.get("momentum_decay", False))
        ev.volume_state = classify_volume(df_live) if df_live is not None and len(df_live) >= 5 else "neutral"
        ev.volume_supportive = ev.volume_state in ("expansion", "spike", "absorption")
        ev.volume_against = ev.volume_state == "exhaustion"

        try:
            ev.rsi = float(compute_rsi(df_live).iloc[-1])
        except Exception:
            ev.rsi = 50.0

        if adx_now is not None:
            ev.adx = float(adx_now)
        if adx_slope is not None:
            ev.adx_slope = float(adx_slope)
        if plus_di is not None:
            ev.di_plus = float(plus_di)
        if minus_di is not None:
            ev.di_minus = float(minus_di)
        ev.di_spread = ev.di_plus - ev.di_minus

        if side == "BUY":
            ev.momentum_direction = "BULLISH" if ev.di_spread > 3 else ("BEARISH" if ev.di_spread < -3 else "NEUTRAL")
        else:
            ev.momentum_direction = "BEARISH" if ev.di_spread < -3 else ("BULLISH" if ev.di_spread > 3 else "NEUTRAL")

        try:
            bos_up, bos_down = detect_bos(df_live, lookback=5) if df_live is not None else (False, False)
            ev.bos_up, ev.bos_down = bool(bos_up), bool(bos_down)
        except Exception:
            pass
        try:
            struct_shift = detect_structure_shift(df_live) if df_live is not None else None
            ev.choch_up = (struct_shift == "bullish_shift")
            ev.choch_down = (struct_shift == "bearish_shift")
            ev.mss_up = ev.choch_up
            ev.mss_down = ev.choch_down
        except Exception:
            pass

        if side == "BUY":
            if ev.bos_up or ev.choch_up:
                ev.structure_bias = "BULLISH"
                ev.structure_health = 80.0
            elif ev.bos_down or ev.choch_down:
                ev.structure_bias = "BEARISH"
                ev.structure_health = 25.0
            else:
                ev.structure_bias = "NEUTRAL"
                ev.structure_health = 55.0
        else:
            if ev.bos_down or ev.choch_down:
                ev.structure_bias = "BEARISH"
                ev.structure_health = 80.0
            elif ev.bos_up or ev.choch_up:
                ev.structure_bias = "BULLISH"
                ev.structure_health = 25.0
            else:
                ev.structure_bias = "NEUTRAL"
        ...
        try:
            pools = build_liquidity_pools(df_live) if df_live is not None else {"high_pools": [], "low_pools": []}
            swept_high, swept_low = detect_sweep(df_live, pools) if df_live is not None else (False, False)
            if side == "BUY":
                ev.liquidity_sweep_supportive = bool(swept_low)
                ev.liquidity_sweep_against = bool(swept_high)
            else:
                ev.liquidity_sweep_supportive = bool(swept_high)
                ev.liquidity_sweep_against = bool(swept_low)
            ev.liquidity_health = 70.0 if ev.liquidity_sweep_supportive else (30.0 if ev.liquidity_sweep_against else 55.0)
        except Exception:
            ev.liquidity_health = 50.0

        try:
            if df_live is not None and len(df_live) >= 5:
                supports, resistances = get_clustered_zones(df_live, lookback=80, cluster_pct=0.002)
                nearest_level = None
                if side == "BUY" and supports:
                    nearest_level = max([s for s in supports if s <= entry], default=None)
                elif side == "SELL" and resistances:
                    nearest_level = min([r for r in resistances if r >= entry], default=None)
                if nearest_level:
                    zscore, _ = compute_zone_strength(df_live, nearest_level,
                                                       "support" if side == "BUY" else "resistance",
                                                       atr, ob)
                    ev.entry_zone_score = float(zscore)
                else:
                    ev.entry_zone_score = 55.0
        except Exception:
            ev.entry_zone_score = 50.0

        if trend_engine is not None and df_live is not None:
            try:
                ev.trend_direction = trend_engine.trend_state.value if hasattr(trend_engine.trend_state, "value") else str(trend_engine.trend_state)
            except Exception:
                pass

        if continuation_eval is not None:
            ev.continuation_probability = float(getattr(continuation_eval, "continuation_probability", 0.5) or 0.5)
            ev.trend_strength = float(getattr(continuation_eval, "trend_strength", 0.5) or 0.5)
            ev.counter_pressure = float(getattr(continuation_eval, "counter_pressure", 0.0) or 0.0)
            ev.hold_quality = getattr(continuation_eval, "hold_quality", "UNKNOWN") or "UNKNOWN"

        thesis_dict = thesis_dict or {}
        if state_dict is not None:
            ev.thesis_failure_score = float(state_dict.get("thesis_failure_score", 0.0) or 0.0)
            ev.thesis_failure_reasons = state_dict.get("thesis_failure_reasons", []) or []
        ev.thesis_valid = (ev.thesis_failure_score < 50)
        ev.has_confirmed_thesis_failure = ev.thesis_failure_score >= 60

        ev.pullback_type = self._classify_pullback(side, ev, df_live, entry, mark_price, atr)

        if df_live is not None and len(df_live) >= 20:
            try:
                atr_series = compute_atr(df_live)
                atr_ma = atr_series.rolling(20).mean().iloc[-1]
                if atr_ma and atr_ma > 0:
                    ev.atr_ratio = float(atr_series.iloc[-1] / atr_ma)
            except Exception:
                ev.atr_ratio = 1.0

        if entry > 0:
            if side == "BUY":
                worst = state_dict.get("min_price", entry) if state_dict else entry
                ev.adverse_excursion_pct = max(0.0, (entry - worst) / entry * 100)
            else:
                worst = state_dict.get("max_price", entry) if state_dict else entry
                ev.adverse_excursion_pct = max(0.0, (worst - entry) / entry * 100)

        decision = self._decide(side, ev, state_dict, df_live, entry, mark_price, atr)
        self._last_decision = decision
        self.decision_log.append(decision.to_dict())
        return decision

    def _classify_pullback(self, side, ev, df, entry, mark_price, atr):
        if df is None or len(df) < 5 or entry <= 0 or atr <= 0:
            return PullbackType.NONE.value
        if side == "BUY":
            adverse = mark_price < entry
            if not adverse:
                return PullbackType.NONE.value
        else:
            adverse = mark_price > entry
            if not adverse:
                return PullbackType.NONE.value
        structure_intact = (ev.structure_bias == "BULLISH" and side == "BUY") or \
                           (ev.structure_bias == "BEARISH" and side == "SELL")
        structure_broken = (ev.structure_bias == "BEARISH" and side == "BUY") or \
                           (ev.structure_bias == "BULLISH" and side == "SELL")
        dist_atr = abs(mark_price - entry) / atr if atr > 0 else 0.0
        strong_counter = ev.counter_pressure > 0.8
        volume_against = ev.volume_against
        if ev.has_confirmed_thesis_failure:
            return PullbackType.THESIS_FAILURE.value
        if volume_against and structure_broken and ev.momentum_decay:
            return PullbackType.DISTRIBUTION.value
        if structure_broken and strong_counter:
            return PullbackType.REVERSAL.value
        if dist_atr <= 0.3:
            return PullbackType.FILL_TEST.value
        if dist_atr <= 1.0 and structure_intact:
            return PullbackType.MICRO_PULLBACK.value
        if dist_atr <= 2.5 and structure_intact and not volume_against:
            return PullbackType.HEALTHY_PULLBACK.value
        if dist_atr > 2.5:
            return PullbackType.STRONG_PULLBACK.value
        return PullbackType.NONE.value

    def _decide(self, side, ev, state_dict, df, entry, mark_price, atr):
        now = time.time()
        entry_time = float(state_dict.get("entry_time", now) or now) if state_dict else now
        seconds_held = max(0.0, now - entry_time)

        if ev.drawdown_from_peak >= self.HARD_EXIT_ROE_DRAWDOWN and ev.peak_roe > 20:
            self._transition(CouncilState.EXIT, "Hard drawdown from peak")
            return CouncilDecision(
                action=CouncilAction.EXIT, new_state=CouncilState.EXIT,
                reason=f"HARD_RISK: drawdown_from_peak={ev.drawdown_from_peak:.1f}%",
                evidence=ev, levels_hit=["HARD_RISK_EMERGENCY"]
            )

        sl_price = state_dict.get("synthetic_sl", 0.0) if state_dict else 0.0
        if entry > 0 and sl_price > 0:
            if side == "BUY":
                sl_dist = (entry - sl_price) / entry * 100
            else:
                sl_dist = (sl_price - entry) / entry * 100
            if sl_dist > 0 and ev.adverse_excursion_pct >= sl_dist * 0.95:
                self._transition(CouncilState.EXIT, "Adverse excursion at SL boundary")
                return CouncilDecision(
                    action=CouncilAction.EXIT, new_state=CouncilState.EXIT,
                    reason=f"HARD_RISK: near SL ({ev.adverse_excursion_pct:.2f}%)",
                    evidence=ev, levels_hit=["NEAR_SL"]
                )

        thesis_failed_strong = ev.thesis_failure_score >= 75
        if ev.has_confirmed_thesis_failure:
            if thesis_failed_strong or self._confirm("thesis_failure"):
                self._transition(CouncilState.THESIS_FAILURE, "Thesis failure confirmed")
                return CouncilDecision(
                    action=CouncilAction.EXIT, new_state=CouncilState.THESIS_FAILURE,
                    reason=f"THESIS_FAILURE score={ev.thesis_failure_score:.1f}",
                    evidence=ev, levels_hit=["THESIS_FAILURE"]
                )

        confirmed_distribution = (
            ev.distribution_risk >= 55
            and (ev.momentum_decay or ev.volume_against or ev.momentum_health < 35)
            and not ev.smart_money_dominant
        )
        if confirmed_distribution and self._confirm("distribution", required=2):
            ev.has_confirmed_distribution = True
            if (ev.roe > 5.0
                and not self._has_partially_closed
                and (now - self._last_partial_ts) > COUNCIL_PARTIAL_COOLDOWN_SEC
                and seconds_held >= COUNCIL_MIN_HOLD_SECONDS):
                self._last_partial_ts = now
                self._has_partially_closed = True
                self._transition(CouncilState.DISTRIBUTION, "Institutional distribution – partial")
                return CouncilDecision(
                    action=CouncilAction.PARTIAL, new_state=CouncilState.DISTRIBUTION,
                    reason=f"DISTRIBUTION dist_risk={ev.distribution_risk:.1f} roe={ev.roe:.1f}%",
                    partial_ratio=0.40, evidence=ev, levels_hit=["DISTRIBUTION_PARTIAL"]
                )
            else:
                self._transition(CouncilState.PROFIT_PROTECTION, "Distribution evidence – protect")
                return CouncilDecision(
                    action=CouncilAction.PROTECT, new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"PROTECT: distribution (roe={ev.roe:.1f}%)",
                    trail_mult=1.0, evidence=ev, levels_hit=["PROTECT_DISTRIBUTION"]
                )

        opposite_bias = "BEARISH" if side == "BUY" else "BULLISH"
        if ev.structure_bias == opposite_bias and ev.counter_pressure > 0.5:
            if self._confirm("struct_weaken", required=2):
                self._transition(CouncilState.PROFIT_PROTECTION, "Structure weakening")
                return CouncilDecision(
                    action=CouncilAction.TIGHTEN_TRAIL, new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"TIGHTEN: struct weak, counter={ev.counter_pressure:.2f}",
                    trail_mult=1.0, evidence=ev, levels_hit=["STRUCTURE_WEAKEN"]
                )

        tp1_already_hit = state_dict.get("tp1_hit", False) if state_dict else False
        if not tp1_already_hit:
            if ev.tp1_reached:
                trend_strong = (
                    ev.continuation_probability >= 0.7
                    and ev.structure_health >= 65
                    and ev.momentum_health >= 55
                    and ev.distribution_risk <= 45
                )
                if trend_strong:
                    new_tp1 = entry * (1 + 0.06) if side == "BUY" else entry * (1 - 0.06)
                    self._transition(CouncilState.TREND_HEALTHY, "TP1 reached – trend strong, hold runner")
                    return CouncilDecision(
                        action=CouncilAction.HOLD, new_state=CouncilState.TREND_HEALTHY,
                        reason=f"TP1_REACHED held (cont={ev.continuation_probability:.2f})",
                        trail_mult=2.0, evidence=ev, levels_hit=["TP1_REACHED_HELD"]
                    )
                else:
                    if seconds_held >= COUNCIL_MIN_HOLD_SECONDS:
                        self._last_partial_ts = now
                        self._has_partially_closed = True
                        self._transition(CouncilState.PROFIT_FORMING, "TP1 event-based partial")
                        return CouncilDecision(
                            action=CouncilAction.PARTIAL, new_state=CouncilState.PROFIT_FORMING,
                            reason=f"TP1_REACHED partial (cont={ev.continuation_probability:.2f})",
                            partial_ratio=0.50, trail_mult=1.5,
                            evidence=ev, levels_hit=["TP1_EVENT_PARTIAL"]
                        )

        pb = ev.pullback_type
        if pb in ("MICRO_PULLBACK", "HEALTHY_PULLBACK", "FILL_TEST"):
            if ev.continuation_probability >= 0.65 and ev.structure_health >= 60:
                self._transition(CouncilState.TREND_HEALTHY, f"Healthy pullback ({pb})")
                return CouncilDecision(
                    action=CouncilAction.HOLD, new_state=CouncilState.TREND_HEALTHY,
                    reason=f"HOLD: pullback={pb} cont={ev.continuation_probability:.2f}",
                    trail_mult=2.5, evidence=ev, levels_hit=["HEALTHY_PULLBACK_HOLD"]
                )
            else:
                self._transition(CouncilState.PROFIT_PROTECTION, f"Pullback {pb} weak cont")
                return CouncilDecision(
                    action=CouncilAction.PROTECT, new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"PROTECT: pullback={pb}",
                    trail_mult=1.2, evidence=ev, levels_hit=["PULLBACK_PROTECT"]
                )

        if pb in ("STRONG_PULLBACK", "REVERSAL"):
            if self._confirm(f"pb_{pb}", required=2):
                self._transition(CouncilState.PROFIT_PROTECTION, f"Strong pullback: {pb}")
                return CouncilDecision(
                    action=CouncilAction.TIGHTEN_TRAIL, new_state=CouncilState.PROFIT_PROTECTION,
                    reason=f"TIGHTEN: pullback={pb} counter={ev.counter_pressure:.2f}",
                    trail_mult=0.9, evidence=ev, levels_hit=[f"PULLBACK_{pb}"]
                )

        trend_rider_conditions = (
            ev.continuation_probability >= 0.75
            and ev.structure_health >= 70
            and ev.momentum_health >= 60
            and ev.distribution_risk < 40
            and ev.adx >= 25
            and ev.adx_slope >= 0
        )
        if trend_rider_conditions:
            self._transition(CouncilState.TREND_RIDER, "Strong trend rider conditions")
            return CouncilDecision(
                action=CouncilAction.TRAIL, new_state=CouncilState.TREND_RIDER,
                reason=f"TREND_RIDER cont={ev.continuation_probability:.2f} adx={ev.adx:.1f}",
                trail_mult=3.5, evidence=ev, levels_hit=["TREND_RIDER"]
            )

        if ev.continuation_probability >= 0.65 and ev.structure_health >= 55:
            self._transition(CouncilState.TREND_HEALTHY, "Trend healthy")
            return CouncilDecision(
                action=CouncilAction.TRAIL, new_state=CouncilState.TREND_HEALTHY,
                reason=f"TRAIL: cont={ev.continuation_probability:.2f}",
                trail_mult=2.0, evidence=ev, levels_hit=["TREND_HEALTHY"]
            )

        if ev.roe > 0.5:
            self._transition(CouncilState.PROFIT_FORMING, "Profit forming")
            return CouncilDecision(
                action=CouncilAction.HOLD, new_state=CouncilState.PROFIT_FORMING,
                reason=f"HOLD: profit forming (roe={ev.roe:.2f}%)",
                trail_mult=1.5, evidence=ev, levels_hit=["PROFIT_FORMING"]
            )

        self._transition(CouncilState.INITIAL, "Initial state")
        return CouncilDecision(
            action=CouncilAction.HOLD, new_state=CouncilState.INITIAL,
            reason=f"HOLD: initial (roe={ev.roe:.2f}%)",
            trail_mult=0.0, evidence=ev, levels_hit=["INITIAL_HOLD"]
        )


_trade_council = TradeManagementCouncil()


# ========== TRADE STATE MACHINE (LEGACY) ==========
class TradeStateMachine:
    STATES = {
        "ACCUMULATION": 0, "EXPANSION": 1, "TREND_RIDE": 2, "DISTRIBUTION": 3,
        "EXHAUSTION": 4, "FAKE_BREAKOUT": 5, "MOMENTUM_COLLAPSE": 6,
        "PANIC_EXIT": 7, "RANGE_CHOP": 8, "HEALTHY_PULLBACK": 9,
        "PROFIT_DEFENSE": 10, "LIQUIDITY_EXHAUSTION": 11
    }
    ...
    def get_trail_multiplier(self) -> float:
        mult_map = {
            ...
            "HEALTHY_PULLBACK": 2.8, "PROFIT_DEFENSE": 1.2, "DISTRIBUTION": 1.2,
            "EXHAUSTION": 1.0, "LIQUIDITY_EXHAUSTION": 0.8, "FAKE_BREAKOUT": 0.8,
            "MOMENTUM_COLLAPSE": 0.6, "PANIC_EXIT": 0.5, "RANGE_CHOP": 1.5
        }
...
PERF = {
    "total_pnl_pct": 0.0, "total_pnl_usdt": 0.0, "trades": 0, "wins": 0,
    "losses": 0, "last_trade": None
}
...
def tg_start(balance, mode):
    send_once(f"🚀 <b>RF v28 COUNCIL Edition</b>\nBalance: {balance:.2f} USDT\nMode: {mode}", "startup", 86400)
...
def tg_council(symbol, action, state, reason):
    icon_map = {"HOLD": "🧊", "PROTECT": "🛡️", "PARTIAL": "✂️", "TRAIL": "📈", "TIGHTEN_TRAIL": "🔒", "EXIT": "🚪"}
    icon = icon_map.get(action, "🧠")
    send_once(f"{icon} <b>COUNCIL {action}</b> on {symbol}\nState: {state}\nReason: {reason[:150]}", f"council_{symbol}_{action}", 30)
...
class TradeThesisEngine:
    def build_thesis(self, symbol, side, trade_type, market_state, narrative, entry_context):
        ...
        return TradeThesis(
            ...
            created_at=time.time(), entry_reason=reasons,
            continuation_factors=continuation, invalidation_factors=invalidation,
            risk_factors=risks, market_context=market_state,
...
class ConfidenceEngine:
    ...
    def calculate_initial_confidence(entry_score, narrative_score, regime, adx, di_spread, location_quality):
        ...
        regime_map = {"STRONG_TREND": 20, "WEAK_TREND": 10, "EXPANSION": 25,
                      "COMPRESSION": 5, "CHOPPY": 0, "ACCUMULATION": 15,
                      "DISTRIBUTION": 10, "TRANSITION": 10}
...
class ContinuationProbabilityEngine:
    ...
    def evaluate(self, side, df, market_state, thesis):
        ...
        if di_spread > 8:
            score += 2.5; reasons.append("strong_di_pressure")
        elif di_spread > 4:
            score += 1.5; reasons.append("moderate_di_pressure")
        else:
            score -= 2.0; reasons.append("weak_di_pressure")

        if adx > 25:
            score += 2.5; reasons.append("healthy_adx")
        elif adx > 18:
            score += 1.0; reasons.append("developing_adx")
        else:
            score -= 2.5; reasons.append("dead_adx")
        if adx_slope > 0:
            score += 1.5; reasons.append("adx_expanding")
        else:
            score -= 1.0; reasons.append("adx_fading")

        if trend_health >= 8:
            score += 3.0; reasons.append("excellent_trend_health")
        elif trend_health >= 6:
            score += 2.0; reasons.append("healthy_trend")
        elif trend_health <= 3:
            score -= 3.0; reasons.append("trend_breakdown")

        if weak_pullback:
            score += 2.0; reasons.append("weak_pullback_detected")

        if counter_displacement > 1.5:
            counter_pressure += 0.5; score -= 3.0; reasons.append("strong_counter_pressure")
        elif counter_displacement > 0.8:
            counter_pressure += 0.25; score -= 1.5; reasons.append("moderate_counter_pressure")

        if side == "BUY":
            if close > ema20:
                score += 1.5; reasons.append("holding_ema20")
            if close > ema50:
                score += 2.0; reasons.append("holding_ema50")
            if close < ema20:
                reclaim_risk += 0.2
            if close < ema50:
                reclaim_risk += 0.4
        ...
        if volume_ratio > 1.2:
            score += 1.5; reasons.append("volume_confirmation")
        elif volume_ratio < 0.7:
            score -= 1.5; reasons.append("weak_volume")
...
class PositionSnapshot:
    def __init__(self):
        ...
        self.roe_pct = 0.0; self.leverage = LEVERAGE
        self.margin = 0.0; self.liquidation_price = 0.0
        self.tp1_hit = False; self.tp2_hit = False
        self.trailing_active = False; self.trailing_stop = 0.0
        self.sl_price = 0.0; self.partial_closed = False
        self.stale = False; self.updated_at = 0.0; self.source = "unknown"
...
class ExchangeSyncService:
    ...
    def reconcile(self, symbol, local_state):
        ...
        with _TRADE_LOCK:
            ...
            TRADE_STATE.update({
                "symbol": symbol, "side": snap.side,
                "entry": snap.entry_price, "qty": snap.qty,
...
class RecoveryGuard:
    ...
    def check_and_recover(self, symbol):
        ...
        if not success:
            ...
            self.last_recovery = now
        return success# ========== INSTITUTIONAL TREND ENGINE ==========
...
class InstitutionalTradeBrain:
    ...
    def should_delay_tp1(self):
        return self.state_machine.should_delay_tp1()

    def should_aggressive_profit_lock(self):
...
# ========== STATE DEFINITIONS (must come before methods that use them) ==========
STATE = {
    ...
    # Council fields
    "council_decision": {},
    "council_state": "INITIAL",
    "council_last_partial_ts": 0.0,
}
...
def close_partial(ratio):
    ...
    try:
        ...
        if filled:
            ...
            if abs(current_qty - expected_remaining) < 0.0001 * max(expected_remaining, 1e-9):
                STATE["remaining_qty"] = current_qty
                TRADE_STATE["qty"] = current_qty
                log_execution(f"[CLOSE_PARTIAL] Partial close confirmed, remaining: {current_qty:.6f}", "SUCCESS")
            else:
                log_execution(f"[CLOSE_PARTIAL] Position mismatch. Using exchange value.", "WARN")
                STATE["remaining_qty"] = current_qty
                TRADE_STATE["qty"] = current_qty
                if current_qty <= 0:
                    STATE["open"] = False
...
# ====================================================================
# LIVE TRADE MANAGER — COUNCIL INTEGRATED
# ====================================================================
class LiveTradeManager:
    ...
    def set_entry_atr(self, entry_atr):
        ...
        if STATE["side"] == "BUY":
            ...
            STATE["synthetic_sl"] = STATE["entry"] + entry_atr * base_sl_mult
        log_execution(f"[SL_FIXED] Initial SL set to {STATE['synthetic_sl']:.4f} (entry ATR={entry_atr:.4f})", "INFO")

    # ---------- Legacy helpers ----------
    def _compute_tp1_hold_score(self, smart, momentum, adx, adx_slope, trade_state,
                                 continuation_eval, distribution_risk, rejection_detected,
                                 failed_breakout, roe):
        score = 0
    ...
    def _compute_institutional_exit_warning(self, smart, momentum, distribution_risk,
                                             continuation_prob, rejection_detected,
    ...
    def _apply_runner_defense(self, roe, peak_roe, drawdown, exit_warning, continuation_prob, trail_mult):
        mult = trail_mult
    ...
    def _update_peak_profit(self, roe, price):
        ...
        if peak > 0:
            STATE["drawdown_from_peak"] = max(0.0, peak - roe)
        else:
    ...
    def _log_live_status(self):
        ...
        if USE_TRADE_COUNCIL:
            state_str = f"COUNCIL:{_trade_council.state.value}"
        else:
        ...
        log_msg = (f"{BLUE}[LIVE_MGMT]{RESET} {direction_icon} {STATE['current_symbol']} {side} | "
                   ...
                   f"PnL: {pnl_color}{pnl_usdt:.2f} USDT{RESET} | SL: {sl:.2f} | TP1: {tp1:.2f} | "
                   f"Trail: {'✅' if trail else '❌'} | TP1 Hit: {'✅' if tp1_hit else '❌'} | State: {state_str}")
    ...
    def _apply_management(self, symbol, now):
        ...
        if now - self.last_heavy_calc_ts >= 5:
            ...
            market_state = {
                ...
                "trend_health": trend_health, "weak_pullback": weak_pullback,
                "counter_displacement": counter_displacement,
                "volume_ratio": volume_ratio, "df": df_live,
                "last_candle": last_candle, "structure_aligned": structure_aligned,
                "continuation_pressure": 50
            }
            ...
            cont_pressure_score, _ = self.continuation_pressure_engine.calculate_pressure(df_live, side, entry, atr, STATE.get("entry_time", time.time()))
            ...
            STATE["thesis_failure_reasons"] = failure_reasons
            STATE["thesis_failed"] = failed

            if not USE_TRADE_COUNCIL and failed:
                if roe > 0:
                    close_partial(0.5)
        ...
        if now - self.last_live_debug_ts >= 5:
            ...
            council_state = _trade_council.state.value if USE_TRADE_COUNCIL else "-"
            log_execution(
                ...
                f"ROE={roe:.2f}% | Council={council_state}", "INFO"
            )
        ...
        # ====================================================================
        if USE_TRADE_COUNCIL:
            decision = _trade_council.evaluate(
                side=side, entry=entry, mark_price=mark_price,
                state_dict=STATE, df_live=df_live, atr=atr, ob=ob,
                thesis_dict=STATE.get("trade_thesis", {}),
                smart_money=smart_money, momentum=momentum,
                trend_engine=trend_engine,
                continuation_engine=_continuation_engine,
                continuation_eval=continuation_eval,
                plus_di=plus_di, minus_di=minus_di,
                adx_now=adx_now, adx_slope=adx_slope
            )
            STATE["council_decision"] = decision.to_dict()
            STATE["council_state"] = decision.new_state.value

            action = decision.action
            log_execution(
                f"[COUNCIL_DECISION] {symbol} {side} -> {action.value} | "
                f"state={decision.new_state.value} | reason={decision.reason}",
                "INFO", debounce_key=f"council_{symbol}_{action.value}_{decision.new_state.value}",
                debounce_sec=20
            )
            if action in (CouncilAction.PARTIAL, CouncilAction.EXIT, CouncilAction.TIGHTEN_TRAIL):
                tg_council(symbol, action.value, decision.new_state.value, decision.reason)

            if action == CouncilAction.EXIT:
                log_execution(f"[COUNCIL-EXIT] {decision.reason}", "WARN")
                close_position_full()
                self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                DASHBOARD_STATE["live_trade_mode"] = False
                return

            if action == CouncilAction.PARTIAL:
                ratio = decision.partial_ratio or 0.50
                log_execution(f"[COUNCIL-PARTIAL] Closing {ratio*100:.0f}% | {decision.reason}", "SUCCESS")
                close_partial(ratio)
                STATE["tp1_hit"] = True
                STATE["runner_mode"] = True
                STATE["trail_activated"] = True
                if side == "BUY":
                    STATE["synthetic_sl"] = max(STATE.get("synthetic_sl", 0.0), entry)
                else:
                    STATE["synthetic_sl"] = min(STATE.get("synthetic_sl", float('inf')), entry)
                STATE["trail_stop"] = STATE.get("synthetic_sl", 0.0)
                tg_tp_hit(symbol, 1, roe)
                STATE["council_last_partial_ts"] = time.time()
                return

            if action in (CouncilAction.TIGHTEN_TRAIL, CouncilAction.TRAIL):
                council_mult = decision.trail_mult if decision.trail_mult > 0 else 1.5
                STATE["smart_trail_mult"] = council_mult
                if not STATE.get("trail_activated", False):
                    STATE["trail_activated"] = True
                    STATE["trail_stop"] = STATE.get("synthetic_sl", 0.0)
                if side == "BUY":
                    proposed_trail = mark_price - council_mult * atr
                    if proposed_trail > STATE.get("trail_stop", 0.0):
                        STATE["trail_stop"] = proposed_trail
                    if mark_price <= STATE["trail_stop"]:
                        log_execution(f"[COUNCIL-TRAIL-HIT] {decision.reason}", "WARN")
                        close_position_full()
                        self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                        DASHBOARD_STATE["live_trade_mode"] = False
                        return
                    ...
                    proposed_trail = mark_price + council_mult * atr
                    if proposed_trail < STATE.get("trail_stop", float('inf')):
                        STATE["trail_stop"] = proposed_trail
                    if mark_price >= STATE["trail_stop"]:
                        log_execution(f"[COUNCIL-TRAIL-HIT] {decision.reason}", "WARN")
                        close_position_full()
                ...
                TRADE_STATE["tp1_hit"] = STATE.get("tp1_hit", False)
                return

            if action == CouncilAction.PROTECT:
                council_mult = decision.trail_mult if decision.trail_mult > 0 else 1.0
                STATE["smart_trail_mult"] = council_mult
                if not STATE.get("trail_activated", False):
                    STATE["trail_activated"] = True
                    STATE["trail_stop"] = STATE.get("synthetic_sl", 0.0)
            ...
            # HOLD: continue to safety net below
        else:
            # [LEGACY FALLBACK]
        ...
        # ---------- SAFETY NET: synthetic SL ----------
        current_sl = STATE.get("synthetic_sl", 0.0)
        if current_sl > 0:
            if (side == "BUY" and mark_price <= current_sl) or (side == "SELL" and mark_price >= current_sl):
                log_execution(f"[SYNTHETIC_SL] Hit at {mark_price:.4f} (SL={current_sl:.4f})", "WARN")
                close_position_full()
                self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                DASHBOARD_STATE["live_trade_mode"] = False
                return
...
def detect_sweep_v2(df, side):
    ...
    if side == "BUY":
        ...
        return sweep_broke, (wick_ratio > 0.6 and reclaim)
    else:
        upper_wick = last['high'] - max(last['open'], last['close'])
...
def structure_engine(df):
    ...
    bos = False
    choch = False
    if len(sh) >= 2 and last_close > sh[-2][1]:
        bos = True
    if len(sl) >= 2 and last_close < sl[-2][1]:
        bos = True
...
def get_vwap_narrative(df):
    ...
    return {"vwap": vwap_last, "distance": distance, "above": above, "below": (not above),
    ...
    for i in range(max(0, len(df)-60), len(df)):
        ...
        if zone_type == "support":
            if abs(candle_low - level) < atr * 0.5:
                touches += 1
                if i < len(df)-1 and df['close'].iloc[i+1] > df['close'].iloc[i]:
                    rejection_count += 1
...
def classify_market_narrative(df, ob, atr, side, rf_signal):
    ...
    if plus_di is not None:
        if side == "BUY" and plus_di > minus_di:
            score += 2.0; reasons.append("DI+ dominance")
        elif side == "SELL" and minus_di > plus_di:
            score += 2.0; reasons.append("DI- dominance")
        elif abs(plus_di - minus_di) < 5:
            reasons.append("DI tangled")
    if adx_slope > 1.5:
        score += 1.5; reasons.append(f"ADX rising ({adx_slope:.1f})")
    elif adx_slope < -1.5:
        score -= 1.0; reasons.append("ADX falling")
    ...
    if side == "BUY":
        ...
        if not vwap_n["above"]: score += 1.5; reasons.append("VWAP below")
    ...
    if sweep_detected:
        score += 2.5; reasons.append("Liquidity sweep")
    ...
    if vol_state in ("expansion", "spike"):
        score += 1.5; reasons.append("Volume expansion")
    elif vol_state == "exhaustion":
        score -= 1.0; reasons.append("Volume exhaustion")
    if candle_rejection(df, side):
        score += 1.5; reasons.append("Rejection candle")
    ...
    if rf_signal == side:
        score += 1.5; reasons.append("RF aligned")
...
def adjust_narrative_confidence(narrative, regime, side, trend_direction):
    ...
    if regime == "TREND":
        if side_aligned:
            if orig_conf == "HIGH": final_conf, final_class = "HIGH", "SNIPER"
            elif orig_conf == "MEDIUM": final_conf, final_class = "MEDIUM", "TREND"
            elif orig_conf == "LOW":
                if score >= 5.0: final_conf, final_class = "MEDIUM", "TREND"
                else: final_conf, final_class = "NO_TRADE", "NO_TRADE"
        else:
            if orig_conf == "HIGH": final_conf, final_class = "HIGH", "SNIPER"
            else: final_conf, final_class = "NO_TRADE", "NO_TRADE"
    ...
    for key, val in MEMORY.items():
        if key.startswith("last_narrative_"):
            debug_data.append({
                ...
                "side": val.get("side"), "classification": val.get("classification"),
                "confidence": val.get("confidence"), "score": val.get("narrative_score"),
                "reasons": val.get("reasons"), "timestamp": val.get("timestamp")
            })
    return jsonify({"narrative_debug": debug_data})


# ========== SMART INSTITUTIONAL ENTRY ==========
def check_institutional_entry(symbol, side, df, ob, atr, price):
    ...
    if intent_score < 75:
        log_execution(f"[INTENT] {symbol} {side} intent {intent_score} < 75 – abort.", "WARN")
    ...
    if side == "BUY" and (struct_shift == "bullish_shift" or bos_up):
        choch_ok = True; reasons.append("Bullish MSS/CHoCH")
    elif side == "SELL" and (struct_shift == "bearish_shift" or bos_down):
        choch_ok = True; reasons.append("Bearish MSS/CHoCH")
    ...
    if adx_now > 50:
        if adx_slope > 0 and di_spread > 8:
            ...
            return False, None, f"Exhaustion risk"
    elif adx_now > 35:
        if adx_slope > 0:
            reasons.append(f"Strong trend ADX={adx_now:.1f}")
        else:
            return False, None, f"ADX high but falling slope"
    else:
        if adx_slope > 0:
            reasons.append(f"Healthy ADX={adx_now:.1f}")
        else:
            return False, None, f"ADX not rising"

    rf = RFEngine(20, 3.5).compute(df)
    if rf["signal"] != side:
        return False, None, f"RF signal {rf['signal']} mismatch"
    ...
    if candle_range_pct > 1.5 * (atr / price * 100):
        return False, None, "Large displacement candle already"
...
def decision_score_v1(df, ob, atr_val, side):
    ...
    return total_score, scenario, side, reasons
...
# ========== SMART DECISION (SMART STOP HUNT / EXHAUSTION) ==========
...
def detect_reset(df, zone_price, zone_type):
    last_close = df['close'].iloc[-1]
    if zone_type == "TOP":
        return (zone_price - last_close) / zone_price >= 0.003
    else:
        return (last_close - zone_price) / zone_price >= 0.003
...
def clear_position_dashboard():
    DASHBOARD_STATE["position"] = None#!/usr/bin/env python3
"""
em_replay.py — Three-track replay / validation harness for Early-Move v3.
Tracks are NOT combined. Synthetic L2 never counts as historical evidence.
"""
...
TRACK_A = "OHLCV_ONLY"
TRACK_B = "REAL_L2"
TRACK_C = "SYNTHETIC_L2_STRESS"

# ------------------------------------------------------------------
# Synthetic L2 generator (Track C only — labeled explicitly)
# ------------------------------------------------------------------
def synth_orderbook_from_bar(bar, seed):
    """Approximate orderbook from volume profile. NEVER claim as history."""
    import random
    ...
    bids, asks = [], []
    for i in range(20):
        b_px = price - atr * 0.1 * (i + 1) - rng.random() * atr * 0.02
        a_px = price + atr * 0.1 * (i + 1) + rng.random() * atr * 0.02
        b_sz = rng.random() * 1000 + 100
        a_sz = rng.random() * 1000 + 100
        bids.append([b_px, b_sz])
        asks.append([a_px, a_sz])
    return {"bids": bids, "asks": asks,
            "synthetic": True,
            "note": "SYNTHETIC — not evidence of historical L2 performance"}

# ------------------------------------------------------------------
# Metrics
# ------------------------------------------------------------------
def find_displacements(bars, atr_mult=3.0, window=20):
    """Return indices of bars where forward-window displacement > atr_mult*ATR."""
    events = []
    for i in range(len(bars) - window):
        w = bars[i:i+window]
        atr = w[0].get("atr") or (w[0]["high"] - w[0]["low"])
        max_high = max(b["high"] for b in w)
        min_low = min(b["low"] for b in w)
        if (max_high - min_low) > atr_mult * atr:
            events.append(i + window)   # approximate ignition index
    return events

# ------------------------------------------------------------------
# Replay core
# ------------------------------------------------------------------
def replay_symbol(symbol, bars, orderbooks=None, track=TRACK_A):
    """
    bars: list of dicts {ts,o,h,l,c,v,atr,rsi,adx,bb_width}
    orderbooks: list of dicts {ts, bids, asks} or None
    returns list of per-bar records with lifecycle / score / conf
    """
    # This function imports EarlyMoveDetector via min.py in your real run.
    # The skeleton intentionally stays decoupled so you can run it offline.
    records = []
    history = deque(maxlen=20)
    for i, b in enumerate(bars):
        # 1. build pseudo-snapshot
        snap = {
            "ts": b["ts"], "price": b["c"],
            "volume": b["v"], "atr": b.get("atr", 0),
            "rsi": b.get("rsi", 50), "adx": b.get("adx", 20),
            "bb_width": b.get("bb_width", 0.02),
        }
        ob = None
        if orderbooks and i < len(orderbooks):
            ob = orderbooks[i]
            if ob and not ob.get("synthetic"):
                snap["ob_valid"] = True
                snap["bid_depth_top15"] = sum(x[1] for x in ob["bids"][:15])
                snap["ask_depth_top15"] = sum(x[1] for x in ob["asks"][:15])
            elif ob and ob.get("synthetic"):
                snap["ob_valid"] = False   # EXCLUDE synthetic from scoring
                snap["_synthetic_l2"] = True
        history.append(snap)
        # 2. call detector (loaded from min.py at runtime in your real run)
        # 3. record result
        records.append({
            "ts": b["ts"], "price": b["c"],
            "lifecycle": "N/A",       # populated by real detector
            "score": 0.0,
            "confidence": 0.0,
            "track": track,
        })
...
# ------------------------------------------------------------------
def compute_metrics(records, displacement_indices):
    """Compute lead-time, precision, false-positive, late-entry on one symbol."""
    first_early = None
    late_count = 0
    early_count = 0
    for r in records:
        if r["lifecycle"] in ("PRE_MOVE", "EARLY_WATCH", "EARLY_IGNITION"):
            if first_early is None:
                first_early = r["ts"]
            early_count += 1
        if r["lifecycle"] in ("IGNITION", "CONFIRMED_MOVE", "LATE_MOVE", "EXHAUSTION"):
            late_count += 1
    lead_times = []
    for idx in displacement_indices:
        ig_ts = records[idx]["ts"] if idx < len(records) else None
        if ig_ts and first_early:
            lead_times.append(ig_ts - first_early)
    return {
        "early_signals": early_count,
        "late_signals": late_count,
        "median_lead_sec": statistics.median(lead_times) if lead_times else 0,
        "false_positive_ratio": (
            late_count / (early_count + late_count)
            if (early_count + late_count) else 1.0
        ),
        "late_entry_ratio": (
            late_count / max(early_count + late_count, 1)
        ),
    }
...
def main():
    ...
    ap.add_argument("--track", choices=[TRACK_A, TRACK_B, TRACK_C], required=True)
    ap.add_argument("--csv", required=True, help="OHLCV CSV path")
    ap.add_argument("--ob", help="Real L2 CSV path (Track B only)")
    ...
    with open(args.csv, "r") as f:
        for line in f:
            ...
            try:
                bars.append({
                    "ts": float(parts[0]), "o": float(parts[1]),
                    "h": float(parts[2]), "l": float(parts[3]),
    ...
    orderbooks = None
    if args.track == TRACK_B:
        if not args.ob:
            print("ERROR: Track B requires --ob", file=sys.stderr)
            sys.exit(2)
        orderbooks = []
        with open(args.ob, "r") as f:
            for line in f:
                if not line.strip():
                    continue
                try:
                    ...
                    continue
    elif args.track == TRACK_C:
        orderbooks = []
        for i, b in enumerate(bars):
            orderbooks.append(synth_orderbook_from_bar(b, seed=i))

    # -------- Detector must be run with min.py loaded in your environment
    disp = find_displacements(bars)
    records = replay_symbol(args.symbol, bars, orderbooks, track=args.track)
    metrics = compute_metrics(records, disp)

    report = {
        "symbol": args.symbol,
        "track": args.track,
        "total_bars": len(bars),
        "displacement_events": len(disp),
        "metrics": metrics,
        "note_l2": (
            "SYNTHETIC — NOT EVIDENCE OF HISTORICAL PERFORMANCE"
            if args.track == TRACK_C else
            "REAL L2 — evidence of historical performance"
            if args.track == TRACK_B else
            "NO L2 EVIDENCE — L2_HISTORICAL_VALIDATED remains False"
        ),
    }
