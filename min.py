...
# + CRITICAL FIXES: lifecycle_state → LIVE, SL safety net, qty sync
# ====================================================================
...
# - FIXED: lifecycle_state stuck at OPEN_PENDING_CONFIRMATION
# - FIXED: SL never enforced when lifecycle is stuck
# - FIXED: STATE["qty"] not updated after partial close
# - FIXED: distribution_risk elif branch unreachable
# ====================================================================
...
class ExchangeSyncService:
    ...
    def reconcile(self, symbol, local_state):
        ...
        # ===== FIX: force LIVE state if still pending =====
        try:
            if STATE.get("open") and _live_manager.lifecycle_state in (
                TradeLifecycleState.OPEN_PENDING_CONFIRMATION,
                TradeLifecycleState.OPEN_REQUESTED,
                TradeLifecycleState.RECOVERING,
            ):
                _live_manager.lifecycle_state = TradeLifecycleState.LIVE
                log_execution("[LIFECYCLE_FIX] Forced → LIVE after reconcile", "SUCCESS")
        except Exception:
            pass
    ...
    try:
        ...
        if filled:
            ...
            if abs(current_qty - expected_remaining) < 0.0001 * expected_remaining:
                ...
                STATE["qty"] = current_qty   # FIX: keep qty in sync
...
def apply_50_50_profit_engine(df, idx, price, atr, side, entry, state, roe_pct, trade_state=None):
    ...
    if climax_risk > 50 and state.get("trail_active", False):
        if not state.get("trail_tightened", False):
            ...
            log_execution(f"[PPE] Climax risk {climax_risk:.1f} > 50 – tightened trail", "WARN")

    if momentum_health < -8 and roe_pct > 2 and not state.get("tp1_hit", False):
        log_execution("[PPE] Negative momentum health – exiting partial", "WARN")
        close_partial(0.5)
        state["sl"] = entry
    ...
    if not state.get("trail_active", False) and roe_pct >= 1.5:
        ...
        log_execution(f"[PPE] {state['symbol']} Stage1: BE SL, trail_active", "INFO")
    ...
    if not state.get("runner_mode", False) and len(adx_series) > idx:
        ...
        if adx_val >= 25:
            ...
            log_execution(f"[PPE] {state['symbol']} Stage3: Runner mode activated", "INFO")
    ...
    if state.get("trail_active", False) and state.get("trail_stop", 0):
        if (side == "BUY" and price <= state["trail_stop"]) or (side == "SELL" and price >= state["trail_stop"]):
            log_execution(f"[PPE] {state['symbol']} Exit: trailing stop hit", "WARN")
    ...
    if state.get("runner_mode", False) and len(adx_series) >= 2:
        ...
        if (side == "BUY" and adx_now < adx_prev and is_bearish) or (side == "SELL" and adx_now < adx_prev and is_bullish):
            log_execution(f"[PPE] {state['symbol']} Exit: momentum weakness", "WARN")
            return "EXIT", state["sl"], state["trail_stop"]
...
class LiveTradeManager:
    ...
    def _on_reconciled(self, snapshot):
        ...
        # ===== FIX: Force LIVE from any pending state =====
        if self.lifecycle_state in (
            ...
            log_execution("[LIFECYCLE_FIX] Forced → LIVE (from reconciled event)", "SUCCESS")
    ...
    def set_entry_atr(self, entry_atr):
        ...
        if STATE["side"] == "BUY":
            ...
            STATE["synthetic_sl"] = STATE["entry"] + entry_atr * base_sl_mult
        # ===== CRITICAL FIX #1: Force transition to LIVE immediately =====
        self.lifecycle_state = TradeLifecycleState.LIVE
        log_execution(f"[SL_FIXED] Initial SL set to {STATE['synthetic_sl']:.4f} based on entry ATR={entry_atr:.4f}", "INFO")
        log_execution("[LIFECYCLE_FIX] Forced → LIVE (entry ATR set)", "SUCCESS")
    ...
    # ===== EMERGENCY SL SAFETY NET =====
    def _emergency_sl_check(self) -> bool:
        """
        Runs regardless of lifecycle_state. Guards against stuck SL.
        ...
        try:
            symbol = STATE["current_symbol"]
            price = STATE.get("mark_price") or get_ticker_safe(symbol)
            if not price or price <= 0:
                return False
            side = STATE.get("side")
            syn_sl = STATE.get("synthetic_sl", 0) or 0
            trail = STATE.get("trail_stop", 0) or 0
            trail_on = STATE.get("trail_activated", False)
            roe = STATE.get("roe_pct", 0) or 0

            # Hard SL check
            if syn_sl > 0:
                if (side == "BUY" and price <= syn_sl) or (side == "SELL" and price >= syn_sl):
                    log_execution(
                        f"[SAFETY_SL] {symbol} price={price:.4f} SL={syn_sl:.4f} → EMERGENCY CLOSE",
                        "ERROR",
                    )
                    close_position_full()
            ...
            if trail_on and trail > 0:
                if (side == "BUY" and price <= trail) or (side == "SELL" and price >= trail):
                    log_execution(
                        f"[SAFETY_TRAIL] {symbol} price={price:.4f} trail={trail:.4f} → EXIT",
                        "WARN",
                    )
                    close_position_full()
                    self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
            ...
            # Extreme loss net (SL should have fired long ago)
            if roe < -8.0:
                log_execution(
                    f"[SAFETY_LOSS] {symbol} ROE={roe:.2f}% past SL — FORCE EXIT",
                    "ERROR",
                )
                close_position_full()
                self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
            ...
            log_execution(f"[SAFETY] Error: {e}", "WARN")
        return False

    def manage_live_trade(self):
        # ===== CRITICAL FIX #2: Emergency safety net (runs before lifecycle check) =====
        if STATE.get("open") and STATE.get("current_symbol"):
            if self._emergency_sl_check():
                return
            # Force LIVE if stuck in pending state while position is open
            if self.lifecycle_state in (
                TradeLifecycleState.OPEN_PENDING_CONFIRMATION,
                TradeLifecycleState.OPEN_REQUESTED,
            ):
                self.lifecycle_state = TradeLifecycleState.LIVE
                log_execution("[LIFECYCLE_FIX] Forced → LIVE (manage_live_trade entry)", "SUCCESS")
    ...
    def _apply_management(self, symbol, now):
        ...
        # ===== FIX: distribution_risk elif branch order =====
        dr = smart_money.get("distribution_risk", 0)
        if dr > 65:
            base_sl_mult = 0.8
        elif dr > 45:
            base_sl_mult = 1.2
...
# ⬇️⬇️⬇️ [الجزء 3 يبدأ من هنا] ⬇️⬇️⬇️# ⬆️⬆️⬆️ [الجزء 2 ينتهي عند dynamic_spread_tolerance] ⬆️⬆️⬆️
