...
# - Execution Queue Dynamic Priority + TRIGGER PERSISTENCE (2/2)
...
class SmartMoneyEngine:
    ...
    def _default_state():
        return {
            "banker_pressure": 50.0, "retailer_pressure": 50.0, "hot_money_pressure": 50.0,
...
class MomentumFlowEngine:
    ...
    def _default_state():
        return {
            "continuation_strength": 0.0, "momentum_health": 50.0, "trend_expansion": False,
...
class TradeStateMachine:
    STATES = {
        "ACCUMULATION": 0, "EXPANSION": 1, "TREND_RIDE": 2, "DISTRIBUTION": 3,
        "EXHAUSTION": 4, "FAKE_BREAKOUT": 5, "MOMENTUM_COLLAPSE": 6, "PANIC_EXIT": 7,
        "RANGE_CHOP": 8, "HEALTHY_PULLBACK": 9, "PROFIT_DEFENSE": 10, "LIQUIDITY_EXHAUSTION": 11
    }
    ...
    def get_trail_multiplier(self) -> float:
        mult_map = {
            "ACCUMULATION": 3.0, "EXPANSION": 3.5, "TREND_RIDE": 4.0, "HEALTHY_PULLBACK": 2.8,
            "PROFIT_DEFENSE": 1.2, "DISTRIBUTION": 1.2, "EXHAUSTION": 1.0, "LIQUIDITY_EXHAUSTION": 0.8,
            "FAKE_BREAKOUT": 0.8, "MOMENTUM_COLLAPSE": 0.6, "PANIC_EXIT": 0.5, "RANGE_CHOP": 1.5
        }
...
TRADE_STATE = {
    "in_position": False, "symbol": None, "side": None, "entry": 0.0, "qty": 0.0,
    "tp1_hit": False, "tp2_hit": False, "trail_on": False, "zone": None,
    "location": None, "reason": [], "last_update_ts": 0
}
PERF = {
    "total_pnl_pct": 0.0, "total_pnl_usdt": 0.0, "trades": 0, "wins": 0, "losses": 0, "last_trade": None
}
...
# ========== CACHE & RATE LIMIT ==========
CACHE = {
    "balance": {"value": 0.0, "ts": 0}, "free_balance": {"value": 0.0, "ts": 0},
    "ohlcv": {"value": {}, "ts": 0}, "ticker": {"value": {}, "ts": 0},
    "orderbook": {"value": {}, "ts": 0}, "dashboard": {"value": None, "ts": 0},
...
def get_live_hybrid_df(symbol, base_df: pd.DataFrame, live_price: float) -> pd.DataFrame:
    ...
    df = base_df.copy()
    last_idx = df.index[-1]
    if 'timestamp' in df.columns: current_ts = df.loc[last_idx, 'timestamp']
    else: current_ts = last_idx
...
def validate_dataframe(df, min_length=100):
    ...
    if not all(c in df.columns for c in required): return False
    if df[required].iloc[-min_length:].isna().any().any(): return False
...
def get_ohlcv_safe(symbol, limit=120, htf=False):
    ...
    if cached is not None:
        if len(cached) >= 100: return cached
    if htf: df = fetch_ohlcv_htf(symbol, '1h', limit)
    else: df = fetch_ohlcv(symbol, limit)
...
class TradeThesis:
    ...
    confidence: float = 0.0; continuation_probability: float = 0.5
    exhaustion_probability: float = 0.0; thesis_strength: float = 0.0
    current_status: str = "ACTIVE"; last_update: float = field(default_factory=time.time)
...
class TradeThesisEngine:
    def build_thesis(self, symbol: str, side: str, trade_type: str, market_state: Dict,
        ...
        reasons = []; continuation = []; invalidation = []; risks = []
        adx = market_state.get("adx", 0); regime = market_state.get("regime", "UNKNOWN")
        continuation_probability = 0.5
        if adx > 25:
            reasons.append("strong_trend_environment"); continuation.append("adx_expansion"); continuation_probability += 0.1
        if market_state.get("di_dominance", False):
            reasons.append("di_dominance"); continuation.append("persistent_pressure"); continuation_probability += 0.1
        if market_state.get("weak_pullback", False):
            reasons.append("weak_pullback"); continuation.append("counter_move_weakness"); continuation_probability += 0.1
        if market_state.get("structure_aligned", False):
            reasons.append("market_structure_alignment"); continuation_probability += 0.1
        narrative_class = narrative.get("classification", "NEUTRAL")
        if narrative_class in ("TREND_CONTINUATION", "INSTITUTIONAL_CONTINUATION"):
            reasons.append("institutional_narrative_alignment"); continuation_probability += 0.1
        ...
        if regime == "CHOP": risks.append("choppy_environment")
        ...
        thesis = TradeThesis(
            thesis_id=f"{symbol}_{int(time.time())}", symbol=symbol, side=side, trade_type=trade_type,
            created_at=time.time(), entry_reason=reasons, continuation_factors=continuation,
            invalidation_factors=invalidation, risk_factors=risks, market_context=market_state,
            confidence=round(confidence, 2), continuation_probability=round(continuation_probability, 2),
            exhaustion_probability=0.0, thesis_strength=round(thesis_strength, 2)
        )
        return thesis
    ...
    def update_thesis(self, thesis: TradeThesis, market_state: Dict) -> TradeThesis:
        ...
        if trend_health >= 7: continuation_prob += 0.05
        elif trend_health <= 3: continuation_prob -= 0.1
        adx_slope = market_state.get("adx_slope", 0)
        if adx_slope > 0: continuation_prob += 0.05
        else: continuation_prob -= 0.03
        counter_displacement = market_state.get("counter_displacement", 0)
        if counter_displacement > 1.2:
            continuation_prob -= 0.15; exhaustion_prob += 0.2
        if market_state.get("weak_pullback", False): continuation_prob += 0.08
...
class RejectionIntelligence:
    ...
    def is_bearish_rejection(df, atr, zone_price=None):
        ...
        zone_failure = False
        if zone_price is not None: zone_failure = last['close'] < zone_price
        if len(df) >= 3:
            prev2 = df.iloc[-2]; prev3 = df.iloc[-3]
            weak_continuation = (prev2['close'] < prev2['open'] or prev3['close'] < prev3['open'])
        else: weak_continuation = False
        ...
        reasons = []; score = 0
        if wick_condition: score += 2; reasons.append("long_upper_wick")
        if close_near_low: score += 1; reasons.append("close_near_low")
        if zone_failure: score += 2; reasons.append("zone_failure")
        if weak_continuation: score += 1; reasons.append("weak_continuation")
        if volume_ok: score += 1; reasons.append("volume_absorption")
        if di_ok: score += 2; reasons.append("di_dominance_sell")
        if is_shooting_star: score += 1.5; reasons.append("shooting_star")
        if is_bearish_engulfing: score += 2; reasons.append("bearish_engulfing")
        is_valid = score >= 5
    ...
    def is_bullish_rejection(df, atr, zone_price=None):
        ...
        if len(df) >= 2:
            ...
            is_bullish_engulfing = (prev['close'] < prev['open'] and last['close'] > last['open'] and
                                    last['high'] > prev['high'] and last['low'] < prev['low'])
        reasons = []; score = 0
        if wick_condition: score += 2; reasons.append("long_lower_wick")
        if close_near_high: score += 1; reasons.append("close_near_high")
        if zone_failure: score += 2; reasons.append("zone_failure")
        if weak_continuation: score += 1; reasons.append("weak_continuation")
        if volume_ok: score += 1; reasons.append("volume_absorption")
        if di_ok: score += 2; reasons.append("di_dominance_buy")
        if is_hammer: score += 1.5; reasons.append("hammer")
        if is_bullish_engulfing: score += 2; reasons.append("bullish_engulfing")
        is_valid = score >= 5
        return is_valid, reasons
...
class MSSValidator:
    ...
    def validate_structure_shift(df, side, atr):
        ...
        body = abs(last['close'] - last['open']); prev_body = abs(prev['close'] - prev['open'])
        ...
        if len(df) >= 3:
            ...
            if side == "BUY": follow_through = next_candle['close'] > next_candle['open'] and body > prev_body
            else: follow_through = next_candle['close'] < next_candle['open'] and body > prev_body
        else: follow_through = True
        ...
        vol_state = classify_volume(df); volume_ok = vol_state in ("expansion", "spike")
        adx_acc = adx_slope > 0 and adx > 25
        score = 0; reasons = []
        if body_expansion: score += 2; reasons.append("body_expansion")
        if follow_through: score += 2; reasons.append("follow_through")
        if di_spread > 8: score += 2; reasons.append("di_spread_strong")
        if volume_ok: score += 1; reasons.append("volume_confirm")
        if adx_acc: score += 2; reasons.append("adx_accelerating")
        is_valid = score >= 5
        if adx < 20: is_valid = False; reasons.append("adx_too_low")
        if not body_expansion and not follow_through: is_valid = False; reasons.append("weak_displacement")
...
class ADXDIIntelligence:
    ...
    def get_adx_state(df):
        adx_series = compute_adx(df)
        if adx_series is None or len(adx_series) < 3: return {"state": "UNKNOWN", "value": 20, "slope": 0, "acceleration": 0}
        adx_val = adx_series.iloc[-1]; adx_prev = adx_series.iloc[-2]
        adx_prev2 = adx_series.iloc[-3] if len(adx_series) >= 3 else adx_prev
        slope = adx_val - adx_prev; accel = slope - (adx_prev - adx_prev2)
        if adx_val < 18: state = "CHOP"
        elif 18 <= adx_val < 22: state = "EMERGING"
        elif 22 <= adx_val < 35: state = "STRONG_TREND"
        elif 35 <= adx_val < 45: state = "VERY_STRONG"
    ...
    def get_di_state(df):
        ...
        if plus_di is None or minus_di is None: return {"dominant": "NEUTRAL", "spread": 0, "trend": "NEUTRAL"}
        spread = plus_di - minus_di
        if spread > 5: dominant = "BUY"; trend = "BULLISH"
        elif spread < -5: dominant = "SELL"; trend = "BEARISH"
        else: dominant = "NEUTRAL"; trend = "CHOP"
        return {"dominant": dominant, "spread": spread, "trend": trend}

    @staticmethod
    def is_healthy_trend(df, side):
        adx_state = ADXDIIntelligence.get_adx_state(df); di_state = ADXDIIntelligence.get_di_state(df)
        if side == "BUY": return adx_state["state"] in ("STRONG_TREND", "VERY_STRONG") and di_state["dominant"] == "BUY" and adx_state["slope"] > 0
        else: return adx_state["state"] in ("STRONG_TREND", "VERY_STRONG") and di_state["dominant"] == "SELL" and adx_state["slope"] > 0
...
class ContinuationPressureEngine:
    ...
    def calculate_pressure(df, side, entry_price, atr, entry_time):
        ...
        if len(bodies) >= 2:
            ...
            if growth > 1.2: score += 10; reasons.append("body_expansion")
            elif growth < 0.8: score -= 10; reasons.append("body_contraction")
        adx_state = ADXDIIntelligence.get_adx_state(df); di_state = ADXDIIntelligence.get_di_state(df)
        if side == "BUY" and adx_state["slope"] > 0 and di_state["dominant"] == "BUY":
            score += 15; reasons.append("adx_rising_di_bullish")
        elif side == "SELL" and adx_state["slope"] > 0 and di_state["dominant"] == "SELL":
            score += 15; reasons.append("adx_rising_di_bearish")
        elif adx_state["slope"] <= 0: score -= 10; reasons.append("adx_falling")
        if di_state["spread"] > 10: score += 10; reasons.append("di_spread_wide")
        elif abs(di_state["spread"]) < 4: score -= 10; reasons.append("di_tangled")
        vol_state = classify_volume(df)
        if vol_state == "expansion": score += 15; reasons.append("volume_expansion")
        elif vol_state == "exhaustion": score -= 15; reasons.append("volume_exhaustion")
        last_close = df['close'].iloc[-1]
        if side == "BUY" and last_close < entry_price: score -= 10; reasons.append("price_below_entry")
        elif side == "SELL" and last_close > entry_price: score -= 10; reasons.append("price_above_entry")
        consecutive = 0
        for i in range(1, min(5, len(df))):
            if side == "BUY" and df['close'].iloc[-i] > df['open'].iloc[-i]: consecutive += 1
            elif side == "SELL" and df['close'].iloc[-i] < df['open'].iloc[-i]: consecutive += 1
            else: break
        if consecutive >= 3: score += 10; reasons.append(f"consecutive_{consecutive}")
        elif consecutive == 0: score -= 5; reasons.append("no_follow_through")
...
class ThesisFailureEngine:
    ...
    def evaluate_failure(thesis: Dict, market_state: Dict, current_price, entry_price, side):
        ...
        failure_score = 0; reasons = []
        if market_state.get("strong_reclaim", False): failure_score += 30; reasons.append("strong_reclaim")
        di_state = ADXDIIntelligence.get_di_state(market_state.get("df", None))
        if side == "BUY" and di_state.get("dominant") == "SELL": failure_score += 25; reasons.append("di_flip_bearish")
        elif side == "SELL" and di_state.get("dominant") == "BUY": failure_score += 25; reasons.append("di_flip_bullish")
        adx_state = ADXDIIntelligence.get_adx_state(market_state.get("df", None))
        if adx_state.get("state") == "CHOP" and adx_state.get("value") < 18: failure_score += 20; reasons.append("adx_collapse")
        ...
        if side == "SELL" and last_candle.get("close", 0) > last_candle.get("open", 0):
            ...
            if body > market_state.get("atr", 0) * 0.6: failure_score += 20; reasons.append("strong_bullish_candle")
        elif side == "BUY" and last_candle.get("close", 0) < last_candle.get("open", 0):
            body = abs(last_candle.get("close",0)-last_candle.get("open",0))
            if body > market_state.get("atr", 0) * 0.6: failure_score += 20; reasons.append("strong_bearish_candle")
        continuation_pressure = market_state.get("continuation_pressure", 50)
        if continuation_pressure < 30: failure_score += 25; reasons.append("low_continuation_pressure")
        sl_distance = abs(current_price - thesis.get("sl", entry_price)) / entry_price
        if sl_distance < 0.005: failure_score += 15; reasons.append("sl_too_close")
...
class MarketRegimeClassifier:
    ...
    def classify(df, ob=None):
        ...
        atr_pct = (atr / price) * 100; vol_state = classify_volume(df)
        ...
        bos_up, bos_down = detect_bos(df, lookback=5); struct_shift = detect_structure_shift(df)
        if adx_state["state"] in ("STRONG_TREND", "VERY_STRONG") and di_state["dominant"] != "NEUTRAL":
            if (price_above_ema and di_state["dominant"] == "BUY") or (price_below_ema and di_state["dominant"] == "SELL"):
                if atr_pct > 2.0: return "EXPANSION"
                else: return "STRONG_TREND"
        if adx_state["state"] == "EMERGING" and adx_state["slope"] > 0: return "WEAK_TREND"
        if adx_state["value"] < 18 or di_state["dominant"] == "NEUTRAL":
            if range_pct < 1.5: return "COMPRESSION"
            else: return "CHOPPY"
        if vol_state == "expansion" and adx_state["value"] > 25: return "EXPANSION"
        if vol_state == "exhaustion" and adx_state["value"] > 30: return "DISTRIBUTION"
        if (struct_shift == "bullish_shift" and bos_up) or (struct_shift == "bearish_shift" and bos_down): return "TRANSITION"
        if vol_state == "absorption": return "ACCUMULATION"
        return "RANGE"
...
class ConfidenceEngine:
    ...
    def apply_institutional_modifiers(base_confidence, smart_money, momentum, continuation_strength):
        ...
        if smart_money.get("smart_money_dominant", False): conf += 10; log_execution("[CONF] Smart money dominant: +10", "INFO", debounce_key="conf_smart", debounce_sec=30)
        else: conf -= 8
        cont = min(100, max(0, continuation_strength))
        if cont > 20: conf += 8
        elif cont < 5: conf -= 10
        mom_health = momentum.get("momentum_health", 50)
        if mom_health > 15: conf += 6
        elif mom_health < 0: conf -= 8
        banker = smart_money.get("banker_pressure", 50); retail = smart_money.get("retailer_pressure", 50)
        if banker > retail: conf += 5
        else: conf -= 6
        dist = smart_money.get("distribution_risk", 0)
        if dist > 45: conf -= 12
        climax = momentum.get("climax_risk", 0)
        if climax > 50: conf -= 10
...
class PrecisionSafety:
    ...
    def normalize_price(symbol, price):
        try:
            market = ex.market(normalize_symbol(symbol)); prec = market['precision']['price']
    ...
    def normalize_amount(symbol, amount):
        try:
            market = ex.market(normalize_symbol(symbol)); prec = market['precision']['amount']
...
class ContinuationProbabilityEngine:
    ...
    def evaluate(self, side: str, df, market_state: Dict, thesis: Dict) -> ContinuationEvaluation:
        score = 0.0; reasons = []
        close = df["close"].iloc[-1]; atr = market_state.get("atr", 0)
        adx = market_state.get("adx", 0); adx_slope = market_state.get("adx_slope", 0)
        di_plus = market_state.get("di_plus", 0); di_minus = market_state.get("di_minus", 0)
        trend_health = market_state.get("trend_health", 5)
        ...
        exhaustion_probability = 0.0; reclaim_risk = 0.0; counter_pressure = 0.0
        if side == "BUY": di_spread = di_plus - di_minus
        else: di_spread = di_minus - di_plus
        if di_spread > 8: score += 2.5; reasons.append("strong_di_pressure")
        elif di_spread > 4: score += 1.5; reasons.append("moderate_di_pressure")
        else: score -= 2.0; reasons.append("weak_di_pressure")
        if adx > 25: score += 2.5; reasons.append("healthy_adx")
        elif adx > 18: score += 1.0; reasons.append("developing_adx")
        else: score -= 2.5; reasons.append("dead_adx")
        if adx_slope > 0: score += 1.5; reasons.append("adx_expanding")
        else: score -= 1.0; reasons.append("adx_fading")
        if trend_health >= 8: score += 3.0; reasons.append("excellent_trend_health")
        elif trend_health >= 6: score += 2.0; reasons.append("healthy_trend")
        elif trend_health <= 3: score -= 3.0; reasons.append("trend_breakdown")
        if weak_pullback: score += 2.0; reasons.append("weak_pullback_detected")
        if counter_displacement > 1.5: counter_pressure += 0.5; score -= 3.0; reasons.append("strong_counter_pressure")
        elif counter_displacement > 0.8: counter_pressure += 0.25; score -= 1.5; reasons.append("moderate_counter_pressure")
        if side == "BUY":
            if close > ema20: score += 1.5; reasons.append("holding_ema20")
            if close > ema50: score += 2.0; reasons.append("holding_ema50")
            if close < ema20: reclaim_risk += 0.2
            if close < ema50: reclaim_risk += 0.4
        else:
            if close < ema20: score += 1.5; reasons.append("holding_ema20")
            if close < ema50: score += 2.0; reasons.append("holding_ema50")
            if close > ema20: reclaim_risk += 0.2
        ...
        if atr > 0:
            ...
            if extension > 3: exhaustion_probability += 0.5; score -= 1.5; reasons.append("overextended")
            elif extension > 2: exhaustion_probability += 0.25; reasons.append("extended_move")
        if volume_ratio > 1.2: score += 1.5; reasons.append("volume_confirmation")
        elif volume_ratio < 0.7: score -= 1.5; reasons.append("weak_volume")
        ...
        if momentum["trend_expansion"]: score += 2.0; reasons.append("momentum_expansion")
        if momentum["momentum_decay"]: score -= 2.5; reasons.append("momentum_decay")
        dist_risk = smart_money["distribution_risk"] / 100.0
        score -= dist_risk * 3.0
        if dist_risk > 0.7: reasons.append("high_distribution_risk")
        if smart_money["retail_euphoria"]: score -= 1.5; reasons.append("retail_euphoria")
        probability = (score + 15) / 30
        probability = max(0.0, min(1.0, probability))
        confidence = min(abs(score) / 15, 1.0)
        should_hold = probability >= self.HOLD_THRESHOLD
        if probability >= 0.8: hold_quality = "STRONG"
        elif probability >= 0.65: hold_quality = "HEALTHY"
        elif probability >= 0.5: hold_quality = "NEUTRAL"
        else: hold_quality = "WEAK"
        return ContinuationEvaluation(
            continuation_probability=round(probability, 2), trend_strength=round(trend_health / 10, 2),
            exhaustion_probability=round(exhaustion_probability, 2), reclaim_risk=round(reclaim_risk, 2),
            counter_pressure=round(counter_pressure, 2), confidence=round(confidence, 2),
            reasons=reasons, should_hold=should_hold, hold_quality=hold_quality
        )
...
class TradeLifecycleState(Enum):
    ...
    LIVE = "LIVE"; PARTIALLY_CLOSED = "PARTIALLY_CLOSED"; CLOSING = "CLOSING"
    CLOSED = "CLOSED"; RECOVERING = "RECOVERING"; ERROR_DEGRADED = "ERROR_DEGRADED"

class PositionSnapshot:
    def __init__(self):
        self.symbol = None; self.side = None; self.qty = 0.0; self.entry_price = 0.0
        self.mark_price = 0.0; self.unrealized_pnl = 0.0; self.realized_pnl = 0.0
        self.roe_pct = 0.0; self.leverage = LEVERAGE; self.margin = 0.0
        self.liquidation_price = 0.0; self.tp1_hit = False; self.tp2_hit = False
    ...
    def to_dict(self):
        return {
            "symbol": self.symbol, "side": self.side, "qty": self.qty, "entry_price": self.entry_price,
            ...
            "trailing_active": self.trailing_active, "trailing_stop": self.trailing_stop, "sl_price": self.sl_price,
            "partial_closed": self.partial_closed, "stale": self.stale, "updated_at": self.updated_at, "source": self.source
        }
...
class ExchangeSyncService:
    def __init__(self, event_bus):
        self.event_bus = event_bus; self._last_snapshot = PositionSnapshot()
        self._reconcile_count = 0; self._last_reconcile = 0
    def fetch_live_snapshot(self, symbol):
        ...
        try:
            ...
            if snapshot.margin > 0: snapshot.roe_pct = (snapshot.unrealized_pnl / snapshot.margin) * 100
            else:
            ...
            snapshot.updated_at = time.time(); snapshot.source = "rest_sync"
    ...
    def _paper_snapshot(self, symbol):
        ...
        snap.symbol = symbol; snap.side = STATE["side"]; snap.qty = STATE["qty"]
    ...
    def reconcile(self, symbol, local_state):
        ...
        if snap is None:
            ...
            return
        with _TRADE_LOCK:
            STATE["entry"] = snap.entry_price; STATE["qty"] = snap.qty
            STATE["remaining_qty"] = snap.qty; STATE["side"] = snap.side
            STATE["mark_price"] = snap.mark_price; STATE["unrealized_pnl_usdt"] = snap.unrealized_pnl
            STATE["roe_pct"] = snap.roe_pct; STATE["margin"] = snap.margin
            STATE["liquidation_price"] = snap.liquidation_price
            TRADE_STATE.update({"symbol": symbol, "side": snap.side, "entry": snap.entry_price, "qty": snap.qty, "last_update_ts": time.time()})
...
class RecoveryGuard:
    ...
    def check_and_recover(self, symbol):
        ...
        if self.recovery_attempts > 5 and now - self.last_recovery < 300:
            log_execution("[RECOVERY] Too many attempts, cooling down", "WARN"); return False
...
class InstitutionalTrendEngine:
    ...
    def analyze_adx_momentum(self, adx_series):
        if adx_series is None or len(adx_series) < 10: return {"value": 20, "slope": 0, "acceleration": 0, "state": "UNKNOWN", "rising": False}
        ...
        if current < 18: state = "CHOP"
        elif 18 <= current < 25: state = "EMERGING"
        elif 25 <= current < 35: state = "HEALTHY"
        elif 35 <= current < 45: state = "STRONG"
        else: state = "EXHAUSTION"
        return {"value": current, "slope": slope, "acceleration": accel, "state": state, "rising": slope > 0}
    def analyze_di_pressure(self, df):
        plus_di, minus_di, _, _ = get_di_components(df)
        if plus_di is None or minus_di is None: return {"dominant": "NEUTRAL", "spread": 0, "persistent": False}
        spread = plus_di - minus_di
        ...
        if len(df) >= 6:
            ...
            for i in range(-5, 0):
                ...
                if p is not None and m is not None:
                    if p > m: buy_count += 1
                    elif m > p: sell_count += 1
            if dominant == "BUY" and buy_count >= 4: persistent = True
            elif dominant == "SELL" and sell_count >= 4: persistent = True
    ...
    def analyze_pullback(self, df, side, atr):
        ...
        last = df.iloc[-1]; prev_candles = df.iloc[-5:-1]
        if side == "SELL":
            bullish_candles = [c for i, c in prev_candles.iterrows() if c['close'] > c['open']]
            if not bullish_candles and last['close'] <= last['open']: return "NO_PULLBACK"
            ...
            vol = df['volume'].iloc[-1]; avg_vol = df['volume'].iloc[-10:-1].mean()
            di = self.analyze_di_pressure(df); adx_mom = self.analyze_adx_momentum(compute_adx(df))
            ...
            if weak_conditions: return "WEAK_PULLBACK"
            if last['close'] > last['open'] and last['close'] > prev_candles['close'].max():
                if vol > avg_vol * 1.5 and di["dominant"] == "BUY" and not adx_mom["rising"]: return "REVERSAL"
            return "STRONG_PULLBACK"
        else:
            bearish_candles = [c for i, c in prev_candles.iterrows() if c['close'] < c['open']]
            if not bearish_candles and last['close'] >= last['open']: return "NO_PULLBACK"
            avg_body = sum(abs(c['close'] - c['open']) for _, c in prev_candles.iterrows()) / len(prev_candles)
            ...
            if last['close'] < last['open'] and last['close'] < prev_candles['close'].min():
                if vol > avg_vol * 1.5 and di["dominant"] == "SELL" and not adx_mom["rising"]: return "REVERSAL"
            return "STRONG_PULLBACK"
    ...
    def is_chop(self, df):
        ...
        adx_val = adx.iloc[-1]
        plus_di, minus_di, _, _ = get_di_components(df)
        if plus_di is None or minus_di is None: return True
    ...
    def update_trend_state(self, df, ob):
        adx_series = compute_adx(df); adx_mom = self.analyze_adx_momentum(adx_series)
        di_pressure = self.analyze_di_pressure(df); chop = self.is_chop(df); now = time.time()
        if chop:
        ...
        if bullish and not bearish: target_state = TrendState.BULLISH
        elif bearish and not bullish: target_state = TrendState.BEARISH
        else: target_state = TrendState.CHOP
        if target_state != self.trend_state:
            if self.trend_state in (TrendState.BULLISH, TrendState.BEARISH):
                if self.trend_state == TrendState.BULLISH and target_state == TrendState.BEARISH: self.trend_state = TrendState.PROBATION_BEARISH
                elif self.trend_state == TrendState.BEARISH and target_state == TrendState.BULLISH: self.trend_state = TrendState.PROBATION_BULLISH
            else:
                self.trend_state = target_state; self.last_state_change = now
                self.trend_persistence = 0; self.state_confidence = 0.3
        elif self.trend_state in (TrendState.PROBATION_BULLISH, TrendState.PROBATION_BEARISH):
            if now - self.last_state_change > 3600: self.trend_state = target_state; self.state_confidence = 0.6
        else:
            self.trend_state = target_state; self.last_state_change = now
            self.trend_persistence = 0; self.state_confidence = 0.5
        else:
            self.trend_persistence += 1; self.state_confidence = min(1.0, self.state_confidence + 0.02)
    def should_enter(self, side, df, ob, atr, price):
        if self.is_chop(df): return False, "CHOP market"
        if side == "BUY" and self.trend_state not in (TrendState.BULLISH, TrendState.PROBATION_BULLISH): return False, f"Trend not bullish ({self.trend_state.value})"
        if side == "SELL" and self.trend_state not in (TrendState.BEARISH, TrendState.PROBATION_BEARISH): return False, f"Trend not bearish ({self.trend_state.value})"
        pullback = self.analyze_pullback(df, side, atr)
        if pullback == "REVERSAL": return False, "Strong reversal detected"
        if pullback == "STRONG_PULLBACK": return False, "Pullback too strong"
        adx_series = compute_adx(df); adx_mom = self.analyze_adx_momentum(adx_series)
        if adx_mom["state"] not in ("HEALTHY", "STRONG", "EMERGING"): return False, f"ADX weak ({adx_mom['state']})"
        di = self.analyze_di_pressure(df)
        if not di["persistent"] and di["spread"] < 3: return False, "DI not persistent"
        return True, f"Trend {self.trend_state.value}, Pullback={pullback}"
    def get_trend_health(self, df, side):
        adx_mom = self.analyze_adx_momentum(compute_adx(df)); di = self.analyze_di_pressure(df)
        health = 5
    ...
    def should_hold(self, df, side, atr, current_roe):
        pullback = self.analyze_pullback(df, side, atr); adx_mom = self.analyze_adx_momentum(compute_adx(df))
        di = self.analyze_di_pressure(df)
        if pullback == "WEAK_PULLBACK" and adx_mom["rising"] and di["persistent"]: return True, "Weak pullback, trend healthy"
        if adx_mom["value"] > 25 and di["dominant"] == side: return True, f"ADX {adx_mom['value']:.1f} still strong"
        if current_roe > 3.0 and pullback != "REVERSAL": return True, "High profit, allowing pullback"
        return False, "Trend weakening"
    def compute_exit_score_live(self, df, side, entry_price, current_price, atr):
        adx_series = compute_adx(df); adx_mom = self.analyze_adx_momentum(adx_series)
        di = self.analyze_di_pressure(df); last = df.iloc[-1]
        ...
        if side == "SELL": strong_reclaim = current_price > entry_price and current_price > last['open']
        else: strong_reclaim = current_price < entry_price and current_price < last['open']
        ...
        if side == "SELL" and last['close'] > last['open']:
            if last['high'] - last['low'] > atr * 1.2: failed_cont = True
...
class InstitutionalTradeBrain:
    def __init__(self):
        self.state_machine = TradeStateMachine(); self.last_update = 0; self.current_trade_state = "RANGE_CHOP"
    def update(self, smart: dict, momentum: dict, adx: float, regime: str):
        ...
        self.last_update = time.time(); return self.current_trade_state
    def get_trail_multiplier(self) -> float: return self.state_machine.get_trail_multiplier()
    def should_delay_tp1(self) -> bool: return self.state_machine.should_delay_tp1()
    def should_aggressive_profit_lock(self) -> bool: return self.state_machine.should_aggressive_profit_lock()
    def should_hard_exit(self) -> bool: return self.state_machine.should_hard_exit()
    def get_patience_level(self) -> str: return self.state_machine.get_patience_level()
    ...
    while time.time() - start < timeout:
        try:
            ...
            if order:
                ...
                elif status == 'open' or status == 'partial': time.sleep(0.5); continue
                pos = fetch_position(symbol)
                if pos is not None: pass
                time.sleep(0.5)
        except Exception as e:
...
def close_partial(ratio):
    ...
    if _closing_in_progress: log_execution("[CLOSE_PARTIAL] Already closing, skipping", "WARN"); return
    _closing_in_progress = True
    try:
        if PAPER_MODE:
            if paper["position"]:
                paper["position"]["remaining_qty"] *= (1-ratio)
                STATE["remaining_qty"] *= (1-ratio); TRADE_STATE["qty"] = STATE["remaining_qty"]
                log_execution(f"[CLOSE_PARTIAL] Paper partial close {ratio*100:.0f}%", "SUCCESS")
            return
        symbol = STATE["current_symbol"]; qty_to_close = STATE["remaining_qty"] * ratio
        if qty_to_close <= 0: log_execution("[CLOSE_PARTIAL] No quantity to close", "WARN"); return
        side = "sell" if STATE["side"] == "BUY" else "buy"; sym = normalize_symbol(symbol)
        ...
        if order is None: log_execution("[CLOSE_PARTIAL] Order creation failed (None)", "ERROR"); return
        order_id = order.get('id')
        if not order_id: log_execution("[CLOSE_PARTIAL] No order ID returned", "ERROR"); return
        filled, filled_qty = verify_order_filled(symbol, order_id, side, qty_precise, timeout=10)
        if filled:
            time.sleep(1); pos = fetch_position(symbol)
            if pos is None:
                log_execution("[CLOSE_PARTIAL] Position vanished unexpectedly", "WARN")
                STATE["open"] = False; TRADE_STATE["in_position"] = False
                DASHBOARD_STATE["live_trade_mode"] = False; finalize_trade_with_reality(symbol); return
            ...
            if abs(current_qty - expected_remaining) < 0.0001 * expected_remaining:
                STATE["remaining_qty"] = current_qty; TRADE_STATE["qty"] = current_qty
            ...
            if current_qty <= 0:
                STATE["open"] = False; TRADE_STATE["in_position"] = False
                DASHBOARD_STATE["live_trade_mode"] = False; finalize_trade_with_reality(symbol)
...
def close_position_full():
    global _closing_in_progress
    if _closing_in_progress: log_execution("[CLOSE] Already closing, skipping", "WARN"); return False
    _closing_in_progress = True
    try:
        if PAPER_MODE:
        ...
        if not STATE["open"]: log_execution("[CLOSE] No position to close", "WARN"); return False
        symbol = STATE["current_symbol"]; qty_to_close = STATE["remaining_qty"]
        if qty_to_close <= 0: log_execution("[CLOSE] No quantity to close", "WARN"); return False
        side = "sell" if STATE["side"] == "BUY" else "buy"; sym = normalize_symbol(symbol)
        qty_precise = float(ex.amount_to_precision(sym, qty_to_close))
        for attempt in range(3):
            order = safe_api_call(ex.create_order, sym, "market", side, qty_precise, params={"reduceOnly": True})
            if order is None: log_execution(f"[CLOSE] Order creation failed (attempt {attempt+1})", "ERROR"); time.sleep(1); continue
            order_id = order.get('id')
            if not order_id: log_execution(f"[CLOSE] No order ID returned (attempt {attempt+1})", "ERROR"); time.sleep(1); continue
            filled, filled_qty = verify_order_filled(symbol, order_id, side, qty_precise, timeout=10)
            if filled:
                time.sleep(1); pos = fetch_position(symbol)
                if pos is None:
                    ...
                    STATE["open"] = False; TRADE_STATE["in_position"] = False
                    DASHBOARD_STATE["live_trade_mode"] = False; finalize_trade_with_reality(symbol); return True
                else:
                    current_qty = float(pos.get('contracts', 0))
                    if current_qty <= 0:
                        ...
                        qty_to_close = current_qty; qty_precise = float(ex.amount_to_precision(sym, qty_to_close)); continue
            else:
                log_execution(f"[CLOSE] Order did not fill (attempt {attempt+1})", "ERROR"); time.sleep(1); continue
        log_execution("[CLOSE] All close attempts failed. Attempting emergency close via position close.", "ERROR")
        try:
            order = safe_api_call(ex.create_order, sym, "market", side, qty_precise, params={"reduceOnly": True})
            if order:
                time.sleep(2); pos = fetch_position(symbol)
                if pos is None or float(pos.get('contracts', 0)) <= 0:
                    STATE["open"] = False; TRADE_STATE["in_position"] = False
                    DASHBOARD_STATE["live_trade_mode"] = False; finalize_trade_with_reality(symbol)
                    log_execution("[CLOSE] Emergency close succeeded", "SUCCESS"); return True
        except Exception as e:
...
def apply_50_50_profit_engine(df, idx, price, atr, side, entry, state, roe_pct, trade_state=None):
    if roe_pct is None: return "HOLD", state.get("sl", 0.0), state.get("trail_stop", 0.0)
    high = df['high'].iloc[-1]; low = df['low'].iloc[-1]
    state["max_price"] = max(state["max_price"], high); state["min_price"] = min(state["min_price"], low)
    smart = state.get("smart_money", {}); mom = state.get("momentum_flow", {})
    dist_risk = smart.get("distribution_risk", 0)
    ...
    if dist_risk > 45 and not state.get("profit_lock_activated", False):
        ...
        if not state.get("tp1_done", False): close_partial(0.5); state["profit_lock_activated"] = True
    ...
    if momentum_health < -8 and roe_pct > 2 and not state.get("tp1_hit", False):
        ...
        close_partial(0.5); state["sl"] = entry
        return "HOLD", state["sl"], state.get("trail_stop", 0.0)
    if not state.get("trail_active", False) and roe_pct >= 1.5:
        state["sl"] = entry
        if side == "BUY": state["trail_stop"] = price - 1.2 * atr
        else: state["trail_stop"] = price + 1.2 * atr
    ...
    remaining = state.get("remaining_qty", 0); adx_series = compute_adx(df)
    if not state.get("runner_mode", False) and len(adx_series) > idx:
        adx_val = adx_series.iloc[idx]
        if adx_val >= 25: state["runner_mode"] = True; log_execution(f"[PPE] {state['symbol']} Stage3: Runner mode activated (ADX={adx_val:.1f})", "INFO")
    trail_mult = state.get("smart_trail_mult", 1.5)
    if state.get("trail_active", False):
        if side == "BUY":
            new_stop = state["max_price"] - trail_mult * atr
            if new_stop > state.get("trail_stop", 0): state["trail_stop"] = new_stop
        else:
            new_stop = state["min_price"] + trail_mult * atr
            if new_stop < state.get("trail_stop", float('inf')): state["trail_stop"] = new_stop
    ...
    if state.get("runner_mode", False) and len(adx_series) >= 2:
        adx_now = adx_series.iloc[idx]; adx_prev = adx_series.iloc[idx-1]; last_candle = df.iloc[-1]
        is_bearish = last_candle['close'] < last_candle['open']; is_bullish = last_candle['close'] > last_candle['open']
...
class LiveTradeManager:
    def __init__(self, event_bus, exchange_sync, recovery_guard):
        ...
        self.lifecycle_state = TradeLifecycleState.IDLE; self.current_snapshot = None
        self.last_management_ts = 0; self.last_log_ts = 0; self.last_live_debug_ts = 0
        self.last_heavy_calc_ts = 0; self.last_position_sync_ts = 0
    ...
    def _set_lifecycle(self, state):
        self.lifecycle_state = state; log_execution(f"[LIFECYCLE] New state: {state.value}", "INFO")
        DASHBOARD_STATE["lifecycle_state"] = state.value
    def _on_reconciled(self, snapshot):
        self.current_snapshot = snapshot; DASHBOARD_STATE["live_trade_mode"] = True
        if self.lifecycle_state == TradeLifecycleState.RECOVERING: self.lifecycle_state = TradeLifecycleState.LIVE
    def _force_close(self, _):
        if STATE["open"]: close_position_full()
        self.lifecycle_state = TradeLifecycleState.CLOSED; DASHBOARD_STATE["live_trade_mode"] = False
    ...
    def set_entry_atr(self, entry_atr):
        STATE["entry_atr"] = entry_atr; base_sl_mult = 1.6
    ...
    def _compute_tp1_hold_score(self, smart: dict, momentum: dict, adx: float, adx_slope: float,
        ...
        score = 0
        if smart.get("smart_money_dominant", False): score += 4
        banker = smart.get("banker_pressure", 50); retail = smart.get("retailer_pressure", 50)
        if banker > retail + 10: score += 3
        cont_strength = momentum.get("continuation_strength", 0)
        if cont_strength > 70: score += 4
        mom_health = momentum.get("momentum_health", 50)
        if mom_health > 65: score += 2
        if momentum.get("trend_expansion", False): score += 3
        if continuation_eval.continuation_probability > 0.75: score += 3
        if trade_state in ("TREND_RIDE", "EXPANSION", "ACCUMULATION", "HEALTHY_PULLBACK"): score += 3
        if adx > 25 and adx_slope > 0: score += 2
        if momentum.get("exhaustion_risk", 0) > 50: score -= 5
        if momentum.get("climax_risk", 0) > 60: score -= 4
        if distribution_risk > 45: score -= 6
        if smart.get("retail_euphoria", False): score -= 3
        if momentum.get("momentum_decay", False): score -= 4
        if trade_state in ("PROFIT_DEFENSE", "DISTRIBUTION", "LIQUIDITY_EXHAUSTION", "MOMENTUM_COLLAPSE"): score -= 5
        if failed_breakout: score -= 5
        if rejection_detected: score -= 4
        if continuation_eval.continuation_probability < 0.45: score -= 5
        if roe > 80: score -= 8
        elif roe > 50: score -= 4
        elif roe > 20: score -= 2
    ...
    def _compute_institutional_exit_warning(self, smart: dict, momentum: dict,
        ...
        warning = 0
        if smart.get("banker_pressure", 50) < 45: warning += 1
        if smart.get("retailer_pressure", 50) > 60: warning += 1
        if distribution_risk > 50: warning += 2
        if momentum.get("exhaustion_risk", 0) > 60: warning += 1
        if momentum.get("climax_risk", 0) > 70: warning += 1
        if momentum.get("momentum_decay", False): warning += 2
        if continuation_prob < 0.5: warning += 1
        if rejection_detected: warning += 1
        if adx_slope < -2: warning += 1
        if di_spread_change < -5: warning += 1
    ...
    def _apply_runner_defense(self, roe: float, peak_roe: float, drawdown: float,
        ...
        mult = trail_mult
        if STATE.get("tp1_hit", False): mult *= 0.9
        if roe > 50: mult *= 0.85
        if drawdown > 12: mult *= 0.7
        if exit_warning >= 3: mult *= 0.6
        if continuation_prob < 0.55: mult *= 0.8
        return max(0.5, min(4.0, mult))
    def _update_peak_profit(self, roe: float, price: float):
        if roe > STATE.get("peak_roe", 0.0):
            STATE["peak_roe"] = roe; STATE["peak_price"] = price
            STATE["peak_unrealized_pnl"] = STATE.get("unrealized_pnl_usdt", 0.0)
        peak = STATE.get("peak_roe", roe)
        if peak > 0: drawdown = peak - roe; STATE["drawdown_from_peak"] = max(0.0, drawdown)
        else: STATE["drawdown_from_peak"] = 0.0
    def manage_live_trade(self):
        if not (STATE.get("open") and STATE.get("current_symbol")):
            if self.lifecycle_state not in (TradeLifecycleState.IDLE, TradeLifecycleState.CLOSED):
                self.lifecycle_state = TradeLifecycleState.IDLE; DASHBOARD_STATE["live_trade_mode"] = False
            return
        now = time.time(); roe = STATE.get("roe_pct", 0.0); adx = STATE.get("adx_live", 20.0)
        calm_conditions = abs(roe) < 1.5 and 18 < adx < 30
        target_interval = 5 if calm_conditions else 2
        if now - self.last_management_ts < target_interval: return
        self.last_management_ts = now; symbol = STATE["current_symbol"]
        with _TRADE_LOCK:
            if now - self.last_position_sync_ts >= 10:
                self.exchange_sync.reconcile(symbol, STATE); self.last_position_sync_ts = now
            self._apply_management(symbol, now); self._log_live_status()
    def _log_live_status(self):
        now = time.time()
        if now - self.last_log_ts < 5: return
        if not STATE.get("open"): return
        roe = STATE.get("roe_pct", 0.0); side = STATE.get("side", "?"); entry = STATE.get("entry", 0.0)
        mark = STATE.get("mark_price", 0.0); pnl_usdt = STATE.get("unrealized_pnl_usdt", 0.0)
        margin = STATE.get("margin", 0.0); sl = STATE.get("synthetic_sl", 0.0)
        tp1 = STATE.get("synthetic_tp1", 0.0); trail = STATE.get("trail_activated", False)
        tp1_hit = STATE.get("tp1_hit", False)
        direction_icon = "🟢" if side == "BUY" else "🔴"; roe_color = color_pnl(roe)
        pnl_color = GREEN if pnl_usdt >= 0 else RED; state_str = self.brain.current_trade_state
    ...
    def _apply_management(self, symbol, now):
        if not STATE.get("open"): return
        if self.lifecycle_state != TradeLifecycleState.LIVE: return
        df_closed = get_ohlcv_safe(symbol, 50)
        if df_closed is None: return
        mark_price = STATE.get("mark_price", get_ticker_safe(symbol))
        if not mark_price: return
        ...
        side = STATE["side"]; entry = STATE["entry"]; roe = STATE.get("roe_pct", 0.0)
        self._update_peak_profit(roe, mark_price); ob = None
        if now - self.last_heavy_calc_ts >= 5:
            ...
            counter_displacement = 0.0; last_candle = df_live.iloc[-1]
            if side == "SELL" and last_candle['close'] > last_candle['open']:
                body = abs(last_candle['close'] - last_candle['open'])
                if body > atr * 0.6: counter_displacement = body / atr
            ...
            market_state = {
                "atr": atr, "adx": adx_now, "adx_slope": adx_slope, "di_plus": plus_di, "di_minus": minus_di,
                "trend_health": trend_health, "weak_pullback": weak_pullback, "counter_displacement": counter_displacement,
                "volume_ratio": volume_ratio, "df": df_live, "last_candle": last_candle,
                "structure_aligned": structure_aligned, "continuation_pressure": 50
            }
            ...
            STATE["trade_state"] = trade_state; STATE["smart_trail_mult"] = self.brain.get_trail_multiplier()
            STATE["delay_tp1"] = self.brain.should_delay_tp1(); STATE["adx_live"] = adx_now
            STATE["di_plus_live"] = plus_di; STATE["di_minus_live"] = minus_di
            STATE["smart_money"] = smart_money; STATE["momentum_flow"] = momentum; STATE["market_regime"] = regime
            ...
            if failed:
                ...
                if roe > 0: close_partial(0.5); STATE["synthetic_sl"] = entry
                else: close_position_full()
                self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                DASHBOARD_STATE["live_trade_mode"] = False; return
            ...
            STATE["current_confidence"] = new_conf; STATE["prev_di_spread"] = plus_di - minus_di
            ...
            adx_now = STATE.get("adx_live", 20.0); plus_di = STATE.get("di_plus_live", 20.0)
            minus_di = STATE.get("di_minus_live", 20.0); smart_money = STATE.get("smart_money", {})
            momentum = STATE.get("momentum_flow", {}); trade_state = STATE.get("trade_state", "RANGE_CHOP")
            continuation_eval = ContinuationEvaluation(
                continuation_probability=STATE.get("continuation_probability", 0.5),
                trend_strength=STATE.get("trend_strength", 0.5), exhaustion_probability=0.0,
                reclaim_risk=STATE.get("reclaim_risk", 0.0), counter_pressure=STATE.get("counter_pressure", 0.0),
                confidence=0.5, reasons=STATE.get("continuation_reasons", []),
                should_hold=STATE.get("continuation_probability", 0.5) >= 0.62,
                hold_quality=STATE.get("hold_quality", "UNKNOWN")
            )
            tp1_hold_score = STATE.get("tp1_hold_score", 10); exit_warning = STATE.get("exit_warning", 0)
            if now - self.last_live_debug_ts >= 5:
                ...
                log_execution(
                    ...
                    f"TP1HoldScore={tp1_hold_score} | ExitWarning={exit_warning}", "INFO"
                )
        if self.brain.should_aggressive_profit_lock() and not STATE.get("profit_lock_activated", False):
            ...
            if not STATE.get("tp1_hit", False):
                close_partial(0.5); STATE["tp1_hit"] = True; STATE["runner_mode"] = True; STATE["profit_lock_activated"] = True
        if self.brain.should_hard_exit():
            log_execution(f"[HARD_EXIT] Hard exit triggered (state={trade_state})", "ERROR")
            close_position_full(); self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
            DASHBOARD_STATE["live_trade_mode"] = False; return
        entry_atr = STATE.get("entry_atr", atr); base_sl_mult = 1.6
        if smart_money.get("distribution_risk", 0) > 45: base_sl_mult = 1.2
        elif smart_money.get("distribution_risk", 0) > 65: base_sl_mult = 0.8
        if side == "BUY":
            synthetic_sl = entry - entry_atr * base_sl_mult
            if STATE.get("tp1_hit", False): synthetic_sl = max(synthetic_sl, entry)
        else:
            synthetic_sl = entry + entry_atr * base_sl_mult
            if STATE.get("tp1_hit", False): synthetic_sl = min(synthetic_sl, entry)
        ...
        if not STATE.get("tp1_hit", False):
            if tp1_hold_score >= 8:
                ...
                close_partial(0.5); STATE["tp1_hit"] = True; STATE["synthetic_sl"] = entry
                tg_tp_hit(symbol, 1, roe); STATE["runner_mode"] = True; STATE["trail_activated"] = True
                self._update_peak_profit(roe, mark_price)
        if STATE.get("tp1_hit", False):
            peak_roe = STATE.get("peak_roe", roe); drawdown = STATE.get("drawdown_from_peak", 0.0)
            ...
            adjusted_mult = self._apply_runner_defense(roe, peak_roe, drawdown, exit_warning,
                                                       continuation_eval.continuation_probability, base_trail_mult)
            STATE["smart_trail_mult"] = adjusted_mult
            tp2_pct = 0.05
            if continuation_eval.continuation_probability > 0.8: tp2_pct = 0.08
            ...
            if not STATE.get("tp2_hit", False):
                if (side == "BUY" and mark_price >= tp2_price) or (side == "SELL" and mark_price <= tp2_price):
                    ...
                    close_position_full(); STATE["tp2_hit"] = True
                    self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                    DASHBOARD_STATE["live_trade_mode"] = False; return
        trail_mult = STATE.get("smart_trail_mult", 1.5)
        if smart_money.get("distribution_risk", 0) > 45: trail_mult *= 0.7
        if momentum.get("momentum_health", 50) < 30: trail_mult *= 0.8
        if continuation_eval.continuation_probability > 0.8: trail_mult *= 1.2
        trail_mult = max(0.5, min(4.5, trail_mult))
        if roe > 1.5:
            if not STATE.get("trail_activated", False):
                STATE["trail_activated"] = True; STATE["trail_stop"] = synthetic_sl
                log_execution(f"[TRAIL] Activated with multiplier {trail_mult}", "INFO")
            if side == "BUY":
                new_trail = mark_price - trail_mult * atr
                if new_trail > STATE.get("trail_stop", 0): STATE["trail_stop"] = new_trail
            else:
                new_trail = mark_price + trail_mult * atr
                if new_trail < STATE.get("trail_stop", float('inf')): STATE["trail_stop"] = new_trail
        ...
        if USE_PPE:
            state_ppe = {
                "symbol": symbol, "trail_active": STATE.get("trail_activated", False),
                "tp1_done": STATE.get("tp1_hit", False), "tp1_hit": STATE.get("tp1_hit", False),
                "runner_mode": STATE.get("runner_mode", False), "max_price": STATE.get("max_price", entry),
                "min_price": STATE.get("min_price", entry), "sl": STATE.get("synthetic_sl", 0.0),
                "trail_stop": STATE.get("trail_stop", 0.0), "remaining_qty": STATE.get("remaining_qty", STATE["qty"]),
                "smart_trail_mult": trail_mult, "smart_money": smart_money, "momentum_flow": momentum,
            ...
            STATE["synthetic_sl"] = new_sl; STATE["trail_stop"] = new_trail
            if state_ppe.get("trail_active", False): STATE["trail_activated"] = state_ppe["trail_active"]
            if state_ppe.get("tp1_hit", False): STATE["tp1_hit"] = state_ppe["tp1_hit"]
            if state_ppe.get("runner_mode", False): STATE["runner_mode"] = state_ppe["runner_mode"]
            STATE["max_price"] = state_ppe["max_price"]; STATE["min_price"] = state_ppe["min_price"]
            if state_ppe.get("profit_lock_activated", False): STATE["profit_lock_activated"] = state_ppe["profit_lock_activated"]
            STATE["trail_tightened"] = state_ppe["trail_tightened"]
            TRADE_STATE["trail_on"] = STATE["trail_activated"]; TRADE_STATE["tp1_hit"] = STATE["tp1_hit"]
            if action == "EXIT":
                close_position_full(); self.event_bus.emit("lifecycle_change", TradeLifecycleState.CLOSED)
                DASHBOARD_STATE["live_trade_mode"] = False; return
...
def sync_position_state(symbol=None):
    if PAPER_MODE:
        if STATE.get("open"):
            ...
            if price:
                ...
                roe_pct = raw_pnl * LEVERAGE; STATE["roe_pct"] = roe_pct; STATE["mark_price"] = price
    ...
    if snap is None:
        if STATE.get("open"):
            ...
            with _TRADE_LOCK:
                STATE["open"] = False; TRADE_STATE["in_position"] = False
                _live_manager.lifecycle_state = TradeLifecycleState.CLOSED; DASHBOARD_STATE["live_trade_mode"] = False
        return None, None, None, None
    with _TRADE_LOCK:
        if not STATE.get("open"):
            ...
            TRADE_STATE.update({"in_position": True, "symbol": symbol, "side": snap.side, "entry": snap.entry_price, "qty": snap.qty, "last_update_ts": time.time()})
            _live_manager.start_trade(symbol, snap.side, snap.entry_price, snap.qty, 0.0, 0.0, 0.0)
        else:
            STATE["entry"] = snap.entry_price; STATE["qty"] = snap.qty; STATE["remaining_qty"] = snap.qty; STATE["side"] = snap.side
...
def get_realized_pnl_for_symbol(symbol, lookback_seconds=30):
    ...
    try:
        sym = normalize_symbol(symbol); since = int((time.time() - lookback_seconds) * 1000)
        trades = safe_api_call(ex.fetch_my_trades, sym, limit=100, params={'since': since})
        if not trades: return 0.0, 0.0
        pnl_usdt = 0.0
        for trade in trades:
            side = trade['side'].lower(); qty = trade['amount']; price = trade['price']; cost = qty * price
...
def compute_atr(df, period=14):
    ...
    atr = rma(tr, period); atr = atr.bfill().ffill().fillna(tr.mean()); atr = atr.clip(lower=1e-8)
    return atr
...
def compute_adx(df, period=14):
    ...
    high = df['high']; low = df['low']; close = df['close']
    tr1 = high - low; tr2 = (high - close.shift(1)).abs(); tr3 = (low - close.shift(1)).abs()
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1); atr = rma(tr, period) + 1e-9
    ...
    adx = rma(dx, period); adx = adx.bfill().ffill().fillna(0).clip(0, 100)
    return adx
...
def compute_rsi(df, period=14):
    ...
    avg_gain = rma(gain, period) + 1e-9; avg_loss = rma(loss, period) + 1e-9
...
def volume_pressure_real(df, window=20, threshold=1.2):
    ...
    vol = df['volume']; mean = vol.rolling(window).mean().iloc[-1]; std = vol.rolling(window).std().iloc[-1]
    if std == 0: return False
    z = (vol.iloc[-1] - mean) / std; return z > threshold
...
def flow_engine(df):
    ...
    if vol > avg_vol * 1.5:
        if body > 0: return "aggressive_buy"
        else: return "aggressive_sell"
    if vol > avg_vol and abs(body) < (last['high'] - last['low']) * 0.3: return "absorption"
    return "neutral"
...
def detect_walls(ob, depth=10, threshold=3.0):
    ...
    bid_sizes = [b[1] for b in ob['bids'][:depth]]; ask_sizes = [a[1] for a in ob['asks'][:depth]]
    if bid_sizes:
        avg_bid = sum(bid_sizes) / len(bid_sizes); bid_wall = any(s > avg_bid * threshold for s in bid_sizes)
    else: bid_wall = False
    if ask_sizes:
        avg_ask = sum(ask_sizes) / len(ask_sizes); ask_wall = any(s > avg_ask * threshold for s in ask_sizes)
    else: ask_wall = False
    return bid_wall, ask_wall

def is_late_move(df, atr, multiplier=1.5):
    if len(df) < 1 or atr <= 0: return False
    last = df.iloc[-1]; candle_range = last['high'] - last['low']
    return candle_range > multiplier * atr

def early_score(df, ob, atr, side):
    score = 0; reasons = []
    macd, signal, hist = compute_macd(df)
    if macd_first_flip(hist): score += 2; reasons.append("macd_flip")
    if volume_pressure_real(df): score += 2; reasons.append("volume_pressure")
    flow = flow_engine(df)
    if side == "BUY" and flow == "aggressive_buy": score += 2; reasons.append("flow_buy")
    elif side == "SELL" and flow == "aggressive_sell": score += 2; reasons.append("flow_sell")
    elif flow == "absorption": reasons.append("absorption")
    obi = orderbook_imbalance(ob, depth=10)
    if side == "BUY" and obi > 0.2: score += 2; reasons.append(f"obi_bullish_{obi:.2f}")
    elif side == "SELL" and obi < -0.2: score += 2; reasons.append(f"obi_bearish_{obi:.2f}")
    bid_wall, ask_wall = detect_walls(ob, depth=10, threshold=3.0)
    if side == "BUY" and bid_wall: score += 1; reasons.append("bid_wall")
    elif side == "SELL" and ask_wall: score += 1; reasons.append("ask_wall")
    if is_late_move(df, atr, multiplier=1.5): score -= 3; reasons.append("late_move_penalty")
    return score, reasons

# ========== RF ENGINE ==========
class RFEngine:
    def __init__(self, period=20, multiplier=3.5): self.period = period; self.multiplier = multiplier
    def ema(self, s, length): return s.ewm(span=length, adjust=False).mean()
    def rng_size(self, x):
        n = self.period; qty = self.multiplier; wper = (n * 2) - 1
        avrng = self.ema((x - x.shift(1)).abs(), n); return self.ema(avrng, wper) * qty
    ...
    def compute(self, df, src="close"):
        x = df[src]; rng = self.rng_size(x); h, l, filt = self.rng_filt(x, rng)
        ...
        longCond = (x > filt) & (pd.Series(fdir) == 1); shortCond = (x < filt) & (pd.Series(fdir) == -1)
        CondIni = np.zeros(len(x))
        for i in range(1, len(x)):
            if longCond.iloc[i]: CondIni[i] = 1
            elif shortCond.iloc[i]: CondIni[i] = -1
        ...
        signal = None
        if longSignal.iloc[-1]: signal = "BUY"
        elif shortSignal.iloc[-1]: signal = "SELL"
        triggered = bool(longSignal.iloc[-1] or shortSignal.iloc[-1])
        distance = (x.iloc[-1] - filt.iloc[-1]) / x.iloc[-1] if x.iloc[-1] != 0 else 0
        return {"signal": signal, "triggered": triggered, "filt": filt.iloc[-1], "h_band": h.iloc[-1], "l_band": l.iloc[-1], "distance": distance}
...
def is_pinbar(candle, atr, side, body_atr_min=0.5, wick_body_ratio=2.5, wick_range_ratio=0.6):
    ...
    if range_ == 0 or atr <= 0: return False
    if side == "BUY": return (lower_wick >= wick_body_ratio * body and lower_wick / range_ >= wick_range_ratio and body / atr >= body_atr_min)
    else: return (upper_wick >= wick_body_ratio * body and upper_wick / range_ >= wick_range_ratio and body / atr >= body_atr_min)
...
def classify_volume(df, period=20, expansion_threshold=1.8, normal_threshold=1.3, exhaustion_threshold=0.7):
    ...
    vol = df['volume']; avg_vol = vol.rolling(period).mean().iloc[-1]
    if avg_vol == 0: return "neutral"
    ratio = vol.iloc[-1] / avg_vol
    if ratio > expansion_threshold: return "expansion"
    elif ratio > normal_threshold: return "normal"
    elif ratio < exhaustion_threshold: return "exhaustion"
    else: return "neutral"

def detect_displacement(df, side, atr, volume_state, body_atr_threshold=0.8, volume_expansion_required=False):
    if len(df) < 2: return False
    last = df.iloc[-1]; body, range_, _, _ = candle_metrics(last)
    if body / atr < body_atr_threshold: return False
    if side == "BUY" and last['close'] <= last['open']: return False
    if side == "SELL" and last['close'] >= last['open']: return False
    if volume_expansion_required and volume_state != "expansion": return False
    return True
...
def detect_location(df, price, supports, resistances, threshold=0.003):
    ...
    if supports:
        ...
        if min_dist_sup < threshold: near_support = True
    if resistances:
        min_dist_res = min(abs(price - r) / price for r in resistances)
        if min_dist_res < threshold: near_resistance = True
    if near_support and not near_resistance: return "LOW"
    elif near_resistance and not near_support: return "HIGH"
    else: return "MID"
...
def get_rejection_pinbar(df, side, atr):
    if len(df) < 1: return False
    candle = df.iloc[-1]; return is_pinbar(candle, atr, side)

def advanced_detect_scenario(df, side, atr, volume_state):
    if len(df) < 3: return "NONE"
    sweep = get_liquidity_sweep_for_side(df, side); rejection = get_rejection_pinbar(df, side, atr)
    displacement = detect_displacement(df, side, atr, volume_state, body_atr_threshold=0.8, volume_expansion_required=False)
    if sweep and rejection: return "TRAP_REVERSAL"
    elif displacement and not rejection: return "TREND_CONTINUATION"
    else: return "NONE"
...
def advanced_decision_engine(scenario, adx, volume_state, location):
    ...
    if volume_state == "exhaustion":
        log_execution(f"[ADV_DECISION] Volume exhaustion -> SKIP", "INFO", debounce_key=f"adv_vol_exhaustion", debounce_sec=120); return "SKIP", None
    adx = float(adx) if adx is not None else 20.0
    if adx < 18:
        log_execution(f"[ADV_DECISION] ADX too low ({adx:.1f}) -> SKIP", "INFO", debounce_key=f"adv_adx_low", debounce_sec=120); return "SKIP", None
    if scenario == "TRAP_REVERSAL":
        if adx < 35: return "ENTER", "STRONG"
        else:
            log_execution(f"[ADV_DECISION] TRAP_REVERSAL but ADX >=35 ({adx:.1f}) -> SKIP", "INFO", debounce_key=f"adv_trap_adx_high", debounce_sec=120); return "SKIP", None
    elif scenario == "TREND_CONTINUATION":
        if 20 < adx < 45: return "ENTER", "MEDIUM"
        else:
            log_execution(f"[ADV_DECISION] TREND_CONTINUATION but ADX out of range (20-45) -> {adx:.1f} SKIP", "INFO", debounce_key=f"adv_trend_adx_range", debounce_sec=120); return "SKIP", None
    return "SKIP", None

# ========== LEGACY SMC FUNCTIONS ==========
def detect_bos(df, lookback=5):
    if len(df) < lookback+2: return False, False
    recent_high = df['high'].iloc[-lookback-1:-1].max(); recent_low = df['low'].iloc[-lookback-1:-1].min()
    current_close = df['close'].iloc[-1]
    bos_up = current_close > recent_high; bos_down = current_close < recent_low
    return bos_up, bos_down

def detect_scenario(df):
    if len(df) < 30: return "NONE"
    row = df.iloc[-1]; liquidity_ctx = detect_liquidity_context(df, lookback=10)
    sweep_up = (liquidity_ctx == "buy_side_taken"); sweep_down = (liquidity_ctx == "sell_side_taken")
    bos_up, bos_down = detect_bos(df); vol_spike_flag = volume_spike(df)
    ...
    if range_ == 0: rejection_buy = False; rejection_sell = False
    else:
    ...
    if sweep_down and rejection_buy: return "REVERSAL_BUY"
    if sweep_up and rejection_sell: return "REVERSAL_SELL"
    if bos_up and volume_ok: return "TREND_BUY"
    if bos_down and volume_ok: return "TREND_SELL"
    if sweep_down and not rejection_buy: return "TRAP_SELL"
    if sweep_up and not rejection_sell: return "TRAP_BUY"
    return "NONE"
...
def apply_profit_engine(symbol, current_price, df, idx, position_state):
    if not position_state["open"]: return "HOLD"
    side = position_state["side"]; entry = position_state["entry"]; remaining = position_state["remaining_qty"]
    ...
    if not position_state.get("tp1_hit", False) and pnl_pct >= 0.5:
        close_ratio = 0.3; close_qty = remaining * close_ratio
        if close_qty > 0:
            if PAPER_MODE: position_state["remaining_qty"] -= close_qty; TRADE_STATE["qty"] = position_state["remaining_qty"]
            else: close_partial(close_ratio)
            log_execution(f"TP1 hit at {pnl_pct:.2f}% - closed {close_ratio*100:.0f}%", "SUCCESS")
            tg_tp_hit(symbol, 1, pnl_pct); position_state["tp1_hit"] = True
            position_state["sl"] = entry; position_state["trail_activated"] = True
            atr_val = compute_atr(df).iloc[-1] if len(df) > 14 else current_price * 0.01
            if side == "BUY": position_state["trail_stop"] = current_price - TRAIL_ATR_MULT * atr_val
            else: position_state["trail_stop"] = current_price + TRAIL_ATR_MULT * atr_val
        return "TP1"
    if position_state.get("tp1_hit", False) and not position_state.get("tp2_hit", False) and pnl_pct >= 1.2:
        close_ratio = 0.3; close_qty = position_state["remaining_qty"] * close_ratio
        if close_qty > 0:
            if PAPER_MODE: position_state["remaining_qty"] -= close_qty; TRADE_STATE["qty"] = position_state["remaining_qty"]
            else: close_partial(close_ratio)
            log_execution(f"TP2 hit at {pnl_pct:.2f}% - closed {close_ratio*100:.0f}%", "SUCCESS")
            tg_tp_hit(symbol, 2, pnl_pct); position_state["tp2_hit"] = True
        return "TP2"
    if len(df) >= 2 and idx >= 1:
        adx_series = compute_adx(df)
        if adx_series is not None and len(adx_series) > idx:
            adx_now = adx_series.iloc[idx]; adx_prev = adx_series.iloc[idx-1]
            if adx_now < adx_prev:
                candle_close = df['close'].iloc[idx]; candle_open = df['open'].iloc[idx]
                if side == "BUY" and candle_close < candle_open:
                    log_execution(f"Exhaustion exit: ADX weakening and bearish candle", "WARN"); close_position_full(); return "EXIT"
                if side == "SELL" and candle_close > candle_open:
                    log_execution(f"Exhaustion exit: ADX weakening and bullish candle", "WARN"); close_position_full(); return "EXIT"
    if position_state.get("trail_activated", False) and position_state.get("trail_stop", 0) > 0:
        if side == "BUY" and current_price <= position_state["trail_stop"]:
            log_execution(f"Trailing stop hit at {current_price:.4f}", "WARN"); close_position_full(); return "EXIT"
        if side == "SELL" and current_price >= position_state["trail_stop"]:
            log_execution(f"Trailing stop hit at {current_price:.4f}", "WARN"); close_position_full(); return "EXIT"
    atr_val = compute_atr(df).iloc[-1] if len(df) > 14 else current_price * 0.01
    if side == "BUY":
        new_stop = current_price - TRAIL_ATR_MULT * atr_val
        if new_stop > position_state["trail_stop"]: position_state["trail_stop"] = new_stop
    else:
        new_stop = current_price + TRAIL_ATR_MULT * atr_val
        if new_stop < position_state["trail_stop"]: position_state["trail_stop"] = new_stop
    return "HOLD"
...
def detect_liquidity_context(df, lookback=10):
    ...
    for i in range(-lookback, 0):
        if i == -1: continue
        prev_low = df['low'].iloc[i-1]; curr_low = df['low'].iloc[i]
        lower_wick = min(df['open'].iloc[i], df['close'].iloc[i]) - curr_low
        if curr_low < prev_low and lower_wick > 0.0001: sweeps.append("sell_side_taken")
        prev_high = df['high'].iloc[i-1]; curr_high = df['high'].iloc[i]
        upper_wick = curr_high - max(df['open'].iloc[i], df['close'].iloc[i])
        if curr_high > prev_high and upper_wick > 0.0001: sweeps.append("buy_side_taken")
...
def detect_structure_shift(df):
    ...
    last_high = df['high'].iloc[-3]; prev_high = df['high'].iloc[-6]
    last_low = df['low'].iloc[-3]; prev_low = df['low'].iloc[-6]
    if last_high > prev_high and last_low > prev_low: return "bullish_shift"
    elif last_high < prev_high and last_low < prev_low: return "bearish_shift"
    return None
...
def get_clustered_zones(df, lookback=120, cluster_pct=0.002):
    ...
    for i in range(2, len(highs)-2):
        if highs[i] == max(highs[i-2:i+3]): swing_highs.append(highs[i])
    swing_lows = []
    for i in range(2, len(lows)-2):
        if lows[i] == min(lows[i-2:i+3]): swing_lows.append(lows[i])
    def cluster(points, pct):
        if not points: return []
        points = sorted(points); clusters = []; current = [points[0]]
        for p in points[1:]:
            if abs(p - current[-1]) / p < pct: current.append(p)
            else: clusters.append(sum(current)/len(current)); current = [p]
        clusters.append(sum(current)/len(current)); return clusters
    res_levels = cluster(swing_highs, cluster_pct); sup_levels = cluster(swing_lows, cluster_pct)
    return sup_levels, res_levels

def detect_liquidity_cluster(df, lb=20, tol=0.001):
    highs = df['high'].iloc[-lb:]; lows  = df['low'].iloc[-lb:]
    if highs.max() == highs.min() or lows.max() == lows.min(): return False, False
    eqh = (highs.max() - highs.min()) / highs.mean() < tol
    eql = (lows.max() - lows.min()) / lows.mean() < tol
    return eqh, eql

def detect_sweep_v2(df, side):
    if len(df) < 22: return False, False
    last = df.iloc[-1]; prev = df.iloc[-2]
    eqh, eql = detect_liquidity_cluster(df, lb=20, tol=0.001)
    range_ = last['high'] - last['low']
    if range_ == 0: return False, False
    if side == "BUY":
        lower_wick = min(last['open'], last['close']) - last['low']; wick_ratio = lower_wick / range_
        low_cluster_min = df['low'].iloc[-21:-1].min()
        sweep_broke = eql and (last['low'] < low_cluster_min); reclaim = last['close'] > last['low']
        valid = (wick_ratio > 0.6 and reclaim); return sweep_broke, valid
    else:
        upper_wick = last['high'] - max(last['open'], last['close']); wick_ratio = upper_wick / range_
        high_cluster_max = df['high'].iloc[-21:-1].max()
        sweep_broke = eqh and (last['high'] > high_cluster_max); reclaim = last['close'] < last['high']
        valid = (wick_ratio > 0.6 and reclaim); return sweep_broke, valid

def candle_rejection(df, side):
    if len(df) < 1: return False
    last = df.iloc[-1]; range_ = last['high'] - last['low']
    if range_ == 0: return False
...
def volume_spike(df):
    ...
    avg_vol = df['volume'].iloc[-21:-1].mean(); last_vol = df['volume'].iloc[-1]
...
def compute_location(df, price, side):
    low50 = df['low'].iloc[-50:].min(); high50 = df['high'].iloc[-50:].max()
    if high50 == low50: return "mid"
    relative = (price - low50) / (high50 - low50)
    if side == "BUY":
        if relative <= 0.3: return "discount"
        elif relative >= 0.7: return "premium"
        else: return "mid"
    else:
        if relative >= 0.7: return "premium"
        elif relative <= 0.3: return "discount"
        else: return "mid"

def swing_points(df, lb=5):
    highs = df['high'].values; lows = df['low'].values; swing_highs = []; swing_lows = []
    for i in range(lb, len(df)-lb):
        if highs[i] == max(highs[i-lb:i+lb+1]): swing_highs.append((i, highs[i]))
        if lows[i] == min(lows[i-lb:i+lb+1]): swing_lows.append((i, lows[i]))
...
def build_liquidity_pools(df):
    ...
    if len(recent_highs) >= 2 and equal_levels(recent_highs): pools_high = recent_highs
    else: pools_high = [sh[-1][1]] if sh else []
    recent_lows = [p[1] for p in sl[-3:]] if len(sl) >= 3 else [sl[-1][1]] if sl else []
    if len(recent_lows) >= 2 and equal_levels(recent_lows): pools_low = recent_lows
    else: pools_low = [sl[-1][1]] if sl else []
    return {"high_pools": pools_high, "low_pools": pools_low}

def detect_sweep(df, pools):
    if len(df) < 2: return False, False
    last = df.iloc[-1]; prev = df.iloc[-2]; swept_high = False; swept_low = False
    for h in pools["high_pools"]:
        if last['high'] > h and prev['high'] <= h and last['close'] < last['high']: swept_high = True; break
    for l in pools["low_pools"]:
        if last['low'] < l and prev['low'] >= l and last['close'] > last['low']: swept_low = True; break
    return swept_high, swept_low

def classify_sweep(df, side):
    if len(df) < 2: return "fake", -2
    last = df.iloc[-1]; range_ = last['high'] - last['low']
    if range_ == 0: return "weak", 1
    if side == "BUY":
        lower_wick = min(last['open'], last['close']) - last['low']; wick_ratio = lower_wick / range_
        reclaimed = last['close'] > last['low']
        if wick_ratio > 0.6 and reclaimed: return "strong", 3
        elif wick_ratio > 0.3: return "weak", 1.5
        else: return "fake", -2
    else:
        upper_wick = last['high'] - max(last['open'], last['close']); wick_ratio = upper_wick / range_
...
def volume_engine(df):
    ...
    if last_vol >= 1.5 * avg_vol: return "spike", 2
    elif last_vol < 0.7 * avg_vol: return "exhaustion", -1
    else:
        last = df.iloc[-1]; body = abs(last['close'] - last['open']); range_ = last['high'] - last['low']
        if range_ > 0 and body / range_ < 0.4 and last_vol > avg_vol: return "absorption", 1
        else: return "normal", 0

def structure_engine(df):
    sh, sl = swing_points(df, lb=5); last_close = df['close'].iloc[-1]; bos = False; choch = False
    if len(sh) >= 2:
        if last_close > sh[-2][1]: bos = True
    if len(sl) >= 2:
        if last_close < sl[-2][1]: bos = True
    if len(sh) >= 2 and len(sl) >= 2:
        if sh[-1][1] > sh[-2][1] and sl[-1][1] > sl[-2][1]: choch = True
        elif sh[-1][1] < sh[-2][1] and sl[-1][1] < sl[-2][1]: choch = True
    return bos, choch

def pre_rf_context_boost(df, side):
    if len(df) < 3: return 0, []
    last2 = df.iloc[-3:-1]; boost = 0; reasons = []
    if side == "BUY":
        if last2['close'].iloc[-2] < last2['close'].iloc[-1] and last2['close'].iloc[-1] < df['close'].iloc[-1]: boost += 1; reasons.append("consecutive_bullish")
        if (last2['close'].iloc[-1] - last2['low'].iloc[-1]) / (last2['high'].iloc[-1] - last2['low'].iloc[-1] + 1e-9) > 0.7: boost += 1; reasons.append("strong_bullish_candle")
    else:
        if last2['close'].iloc[-2] > last2['close'].iloc[-1] and last2['close'].iloc[-1] > df['close'].iloc[-1]: boost += 1; reasons.append("consecutive_bearish")
        if (last2['high'].iloc[-1] - last2['close'].iloc[-1]) / (last2['high'].iloc[-1] - last2['low'].iloc[-1] + 1e-9) > 0.7: boost += 1; reasons.append("strong_bearish_candle")
...
def market_intent(df):
    ...
    if absorption and vol_state == "absorption": return "accumulation", 1
    last = df.iloc[-1]; prev = df.iloc[-2]
    if vol_state == "spike" and last['close'] < prev['high'] and last['high'] > prev['high']: return "distribution", 1
    if len(df) >= 3:
...
def council_decision(context):
    score = 0; reasons = []
    if context["location"] == "discount" and context["side"] == "BUY": score += 3; reasons.append("discount_location")
    elif context["location"] == "premium" and context["side"] == "SELL": score += 3; reasons.append("premium_location")
    else: score -= 4; reasons.append("bad_location")
    if context["sweep"] == "strong": score += 3; reasons.append("strong_sweep")
    elif context["sweep"] == "weak": score += 1; reasons.append("weak_sweep")
    elif context["sweep"] == "fake": score -= 2; reasons.append("fake_sweep")
    if context["in_zone"]: score += 2.5; reasons.append("zone_hit")
    if context["volume"] == "spike": score += 2; reasons.append("volume_spike")
    elif context["volume"] == "absorption": score += 1; reasons.append("absorption")
    elif context["volume"] == "exhaustion": score -= 1; reasons.append("exhaustion")
    if context["bos"] or context["choch"]: score += 2; reasons.append("structure_shift")
    intent = context.get("intent")
    if intent == "trap": score += 2; reasons.append("trap_intent")
    elif intent == "accumulation": score += 1; reasons.append("accumulation")
    elif intent == "distribution": score += 1; reasons.append("distribution")
    score += context.get("pre_rf_boost", 0)
    if context.get("pre_rf_reasons"): reasons.extend(context["pre_rf_reasons"])
    if context.get("distance_penalty", 0) == -100: return -100, ["far_from_zone_skip"]
    score += context.get("distance_penalty", 0)
    if context.get("distance_reasons"): reasons.extend(context["distance_reasons"])
    adx = context["adx"]
    if adx < 18: score -= 1; reasons.append("weak_trend")
    elif 20 <= adx <= 30: score += 2; reasons.append("ideal_trend_phase")
    elif 30 < adx <= 35: score += 0.5; reasons.append("mid_trend")
    elif adx > 40: score -= 2; reasons.append("late_trend_no_entry")
...
def compute_sl_tp(entry_price, side, classification, atr, df):
    if classification == "REVERSAL":
        ...
        min_sl_dist = 1.2 * atr
        if abs(entry_price - sl) < min_sl_dist: sl = entry_price - min_sl_dist if side == "BUY" else entry_price + min_sl_dist
...
STATE = {
    ...
    "delay_tp1": False, "smart_trail_mult": 1.5, "synthetic_sl": 0.0, "synthetic_tp1": 0.0,
    "max_price": 0.0, "min_price": 0.0, "peak_roe": 0.0, "peak_price": 0.0,
    "peak_unrealized_pnl": 0.0, "drawdown_from_peak": 0.0, "tp1_hold_score": 10,
    "exit_warning": 0, "runner_mode": False, "entry_atr": 0.0
}
...
def log_execution(msg, level="INFO", debounce_key=None, debounce_sec=60):
    if debounce_key:
        now = time.time(); last = MEMORY.get("log_debounce", {}).get(debounce_key, 0)
...
def update_stats(pnl_pct):
    ...
    if pnl_pct >= 0: DASHBOARD_STATE["stats"]["wins"] += 1
    else: DASHBOARD_STATE["stats"]["losses"] += 1
...
def open_position(side, amount, symbol):
    ...
    sym = normalize_symbol(symbol)
    with _TRADE_LOCK:
        if _ACTIVE_TRADE: log_execution("[OPEN] Another trade already in progress, skipping", "WARN"); return None
        _ACTIVE_TRADE = True
    try:
        set_leverage(symbol, LEVERAGE); amount = float(ex.amount_to_precision(sym, amount))
        bal = safe_api_call(ex.fetch_balance)
        if bal is None: log_execution("[OPEN] Failed to fetch balance", "ERROR"); with _TRADE_LOCK: _ACTIVE_TRADE = False; return None
        usdt = bal.get("free", {}).get("USDT", 0.0)
        ticker = safe_api_call(ex.fetch_ticker, sym)
        if ticker is None: log_execution("[OPEN] Failed to fetch ticker", "ERROR"); with _TRADE_LOCK: _ACTIVE_TRADE = False; return None
        price = ticker["last"]; required_margin = (amount * price) / LEVERAGE
        if usdt < required_margin * 1.01:
            ...
            INSUFFICIENT_MARGIN_COOLDOWN_UNTIL = time.time() + INSUFFICIENT_MARGIN_COOLDOWN_SEC; return None
        max_spread = dynamic_spread_tolerance(symbol); spread = get_spread_bps(symbol)
        if spread > max_spread: log_execution(f"[OPEN] Spread {spread:.2f}% > {max_spread}%", "WARN"); with _TRADE_LOCK: _ACTIVE_TRADE = False; return None
        order = safe_api_call(ex.create_order, sym, "market", side.lower(), amount, params={"leverage": LEVERAGE})
        if order: log_execution(f"[OPEN] Order filled: {side} {amount} {symbol} @ {price}", "SUCCESS"); return order
    except Exception as e:
...
def close_position(amount, symbol):
    global _ACTIVE_TRADE
    sym = normalize_symbol(symbol); side = STATE["side"]; close_side = "sell" if side == "BUY" else "buy"
    try:
        ...
        with _TRADE_LOCK: _ACTIVE_TRADE = False
        log_execution(f"[CLOSE] Closed {amount} {symbol} (reduceOnly)", "SUCCESS"); return order
    except Exception as e:
...
def finalize_trade_with_reality(symbol):
    ...
    if mark_price is None and not PAPER_MODE: mark_price = get_ticker_safe(symbol)
    pnl_usdt = 0.0; pnl_pct = 0.0
    if PAPER_MODE:
        entry = STATE["entry"]; side = STATE["side"]
        if side == "BUY": pnl_pct = (mark_price - entry) / entry * 100
        else: pnl_pct = (entry - mark_price) / entry * 100
        pnl_usdt = pnl_pct / 100 * entry * STATE["qty"]
    else:
        realized_usdt, realized_pct = get_realized_pnl_for_symbol(symbol, lookback_seconds=30)
        if realized_usdt != 0.0: pnl_usdt = realized_usdt; pnl_pct = realized_pct
        else:
            if roe is not None: pnl_pct = roe
            if STATE.get("margin", 0) > 0: pnl_usdt = STATE["margin"] * (roe / 100)
            else: pnl_usdt = (pnl_pct / 100) * STATE["entry"] * STATE["qty"]
    ...
    TRADE_STATE.update({"in_position": False, "symbol": None, "side": None, "entry": 0.0, "qty": 0.0,
                        "tp1_hit": False, "tp2_hit": False, "trail_on": False, "zone": None, "location": None, "reason": []})
    DASHBOARD_STATE["live_trade_mode"] = False
    ...
    with _TRADE_LOCK:
        ...
        STATE["tp1_hit"] = False; STATE["tp2_hit"] = False; STATE["trail_activated"] = False
        STATE["profit_lock_activated"] = False; STATE["runner_mode"] = False; STATE["trail_tightened"] = False
...
def dynamic_spread_tolerance(symbol):
    ...
    if atr_pct > 2.0: return MAX_SPREAD_PERCENT_VOLATILE
    ...
    try:
        ...
        for s in markets:
            if "USDT" in s and markets[s].get('swap') and markets[s].get('active'):
                clean = s.replace(":USDT", ""); symbols.append(clean)
        return symbols[:200]
    except Exception as e:
...
def rf_proximity_score(rf, adx_val, vol_ok, rsi_val, atr_pct):
    ...
    if adx_val < 18: trend = 0.2
    elif 18 <= adx_val <= 30: trend = 1.0
    elif 30 < adx_val <= 40: trend = 0.6
    else: trend = 0.2
    if 30 <= rsi_val <= 70: rsi_score = 0.5
    elif 20 <= rsi_val < 30 or 70 < rsi_val <= 80: rsi_score = 0.3
    else: rsi_score = 0.0
...
def scan_market_rf(top_n=40):
    ...
    for sym in symbols[:150]:
        try:
            ...
            avg_vol = df['volume'].iloc[-20:].mean(); vol_ok = df['volume'].iloc[-1] >= avg_vol * 0.7
            atr_pct = (atr_val / df['close'].iloc[-1]) * 100 if df['close'].iloc[-1] > 0 else 0
            score = rf_proximity_score(rf, adx_val, vol_ok, rsi_val, atr_pct)
            if score < 0.3: continue
            if rf["triggered"]: status = "TRIGGERED"
            elif score >= 0.6: status = "READY"
            else: status = "PROXIMITY"
            results.append({"symbol": sym, "score": round(score, 3), "rf_signal": rf["signal"], "rf_triggered": rf["triggered"],
                            "rf_distance": round(rf.get("distance", 0), 4), "adx": round(adx_val, 1), "rsi": round(rsi_val, 1),
...
def smart_scanner_v2():
    symbols = get_usdt_perp_symbols()[:150]; buy_candidates = []; sell_candidates = []
    for sym in symbols:
        try:
            df = get_ohlcv_safe(sym, 150)
            if df is None or len(df) < 100: continue
            price = df['close'].iloc[-1]; rf_engine = RFEngine(period=20, multiplier=3.5); rf = rf_engine.compute(df)
            if rf["distance"] is None: continue
            rf_prox = abs(rf["distance"]); vol_ma = df['volume'].iloc[-21:-1].mean()
            if df['volume'].iloc[-1] < 0.5 * vol_ma: continue
            atr_val = compute_atr(df).iloc[-1]; atr_pct = (atr_val / price) * 100 if price > 0 else 0
            if atr_pct < 0.2: continue
            ...
            structure_ctx = detect_structure_shift(df); rejection_buy = candle_rejection(df, "BUY"); rejection_sell = candle_rejection(df, "SELL")
            vol_spike_flag = volume_spike(df); location = compute_location(df, price, "BUY")
            smart_money = SmartMoneyEngine.analyze_smart_money(df); momentum = MomentumFlowEngine.analyze_momentum_flow(df)
            score_mod_buy = 0; score_mod_sell = 0
            if smart_money["smart_money_dominant"]:
                if smart_money["institutional_bias"] == "BUY": score_mod_buy += 2.5
                elif smart_money["institutional_bias"] == "SELL": score_mod_sell += 2.5
            if smart_money["distribution_risk"] > 70: score_mod_sell += 1.5; score_mod_buy -= 2.0
            if smart_money["accumulation_strength"] > 60: score_mod_buy += 1.5; score_mod_sell -= 2.0
            if smart_money["retail_euphoria"]: score_mod_buy -= 1.5; score_mod_sell -= 1.5
            if momentum["trend_expansion"]:
                if momentum["flow_bias"] == "BUY": score_mod_buy += 2.0
                elif momentum["flow_bias"] == "SELL": score_mod_sell += 2.0
            if momentum["momentum_decay"]: score_mod_buy -= 1.5; score_mod_sell -= 1.5
            if momentum["exhaustion_risk"] > 70: score_mod_buy -= 2.0; score_mod_sell -= 2.0
            if momentum["climax_risk"] > 70: score_mod_buy -= 1.5; score_mod_sell -= 1.5
            if momentum["greed_state"]: score_mod_buy -= 1.0; score_mod_sell -= 1.0
            base_score_buy = 0
            if liquidity_ctx == "sell_side_taken": base_score_buy += 2
            if zone_ctx["near_support"]: base_score_buy += 2
            if structure_ctx == "bullish_shift": base_score_buy += 1.5
            if rf_prox < 0.0015: base_score_buy += 2
            elif rf_prox < 0.003: base_score_buy += 1
            if rejection_buy: base_score_buy += 1.5
            if vol_spike_flag: base_score_buy += 1
            base_score_sell = 0
            if liquidity_ctx == "buy_side_taken": base_score_sell += 2
            if zone_ctx["near_resistance"]: base_score_sell += 2
            if structure_ctx == "bearish_shift": base_score_sell += 1.5
            if rf_prox < 0.0015: base_score_sell += 2
            elif rf_prox < 0.003: base_score_sell += 1
            if rejection_sell: base_score_sell += 1.5
            if vol_spike_flag: base_score_sell += 1
            final_score_buy = base_score_buy + score_mod_buy; final_score_sell = base_score_sell + score_mod_sell
            if final_score_buy >= 5:
                buy_candidates.append({"symbol": sym, "score": round(final_score_buy, 2), "rf_prox": round(rf_prox*100, 3),
                                       "liquidity": liquidity_ctx, "zone": zone_ctx, "structure": structure_ctx,
                                       "rejection": rejection_buy, "volume_spike": vol_spike_flag, "location": location,
                                       "smart_money": {"bias": smart_money["institutional_bias"], "bias_detailed": smart_money.get("institutional_bias_detailed", "NEUTRAL"),
                                                       "dominant": smart_money["smart_money_dominant"], "distribution_risk": round(smart_money["distribution_risk"], 1),
                                                       "accumulation": round(smart_money["accumulation_strength"], 1)},
                                       "momentum": {"expansion": momentum["trend_expansion"], "decay": momentum["momentum_decay"],
                                                    "exhaustion_risk": round(momentum["exhaustion_risk"], 1), "greed": momentum["greed_state"]}})
            if final_score_sell >= 5:
                sell_candidates.append({"symbol": sym, "score": round(final_score_sell, 2), "rf_prox": round(rf_prox*100, 3),
                                        "liquidity": liquidity_ctx, "zone": zone_ctx, "structure": structure_ctx,
                                        "rejection": rejection_sell, "volume_spike": vol_spike_flag, "location": compute_location(df, price, "SELL"),
                                        "smart_money": {"bias": smart_money["institutional_bias"], "bias_detailed": smart_money.get("institutional_bias_detailed", "NEUTRAL"),
                                                        "dominant": smart_money["smart_money_dominant"], "distribution_risk": round(smart_money["distribution_risk"], 1),
                                                        "accumulation": round(smart_money["accumulation_strength"], 1)},
                                        "momentum": {"expansion": momentum["trend_expansion"], "decay": momentum["momentum_decay"],
...
def equal_levels_points(highs, lows, tolerance=0.002):
    ...
    def cluster(points, tol):
        ...
        points = sorted(points); clusters = []; current = [points[0]]
        for p in points[1:]:
            if abs(p - current[-1]) / current[-1] < tol: current.append(p)
            else: clusters.append(current); current = [p]
        clusters.append(current); return clusters
    high_clusters = cluster(highs, tolerance); low_clusters = cluster(lows, tolerance)
    for cl in high_clusters:
        if len(cl) >= 2: eq_highs.extend([sum(cl)/len(cl)])
    for cl in low_clusters:
        if len(cl) >= 2: eq_lows.extend([sum(cl)/len(cl)])
...
def detect_equal_highs_lows(df, lookback=50):
    sub = df.iloc[-lookback:]; sh, sl = find_swing_points(sub, window=2)
    return equal_levels_points(sh, sl)

def detect_order_block(df, side, lookback=4):
    if len(df) < lookback+2: return None
    atr = compute_atr(df).iloc[-1]; move = abs(df['close'].iloc[-1] - df['close'].iloc[-2])
    if move < atr * 1.2: return None
    for i in range(2, lookback+2):
        if i >= len(df): break
        candle = df.iloc[-i]
        if side == "BUY" and candle['close'] < candle['open']: return {"low": candle['low'], "high": candle['high'], "idx": -i}
        elif side == "SELL" and candle['close'] > candle['open']: return {"low": candle['low'], "high": candle['high'], "idx": -i}
    return None

def detect_fvg(df, threshold=0.001):
    if len(df) < 2: return None
    prev = df.iloc[-2]; curr = df.iloc[-1]
    if curr['low'] > prev['high'] * (1+threshold): return ("bullish", prev['high'], curr['low'])
    elif curr['high'] < prev['low'] * (1-threshold): return ("bearish", curr['high'], prev['low'])
    return None
...
def evaluate_liquidity_narrative(df, ob, atr, side):
    ...
    price = df['close'].iloc[-1]; pools = build_liquidity_pools(df)
    swept_h, swept_l = detect_sweep(df, pools)
    if side == "BUY" and swept_l: narrative["sweep"] = True
    elif side == "SELL" and swept_h: narrative["sweep"] = True
    bos_up, bos_down = detect_bos(df); struct_shift = detect_structure_shift(df); choch = struct_shift is not None
    if side == "BUY" and (bos_up or (choch and struct_shift == "bullish_shift")): narrative["choch_bos"] = True
    elif side == "SELL" and (bos_down or (choch and struct_shift == "bearish_shift")): narrative["choch_bos"] = True
    zones = get_smart_zones(df.symbol if hasattr(df, 'symbol') else "unknown", df, ob)
    required_zone = None
    if zones:
        if side == "BUY" and zones["buy_zones"]: required_zone = zones["buy_zones"][0]
        elif side == "SELL" and zones["sell_zones"]: required_zone = zones["sell_zones"][0]
    if required_zone:
        dist = abs(price - required_zone["price"]) / price
        if dist < 0.003: narrative["retest"] = True
    if candle_rejection(df, side): narrative["rejection"] = True
    vol_state = classify_volume(df)
    if detect_displacement(df, side, atr, vol_state): narrative["displacement"] = True
    if vol_state in ("expansion", "spike"): narrative["volume_confirmation"] = True
    rf = RFEngine(20, 3.5).compute(df)
    if rf["signal"] == side and abs(rf["distance"]) < 0.003: narrative["rf_alignment"] = True
    score = 0
...
def smart_opportunity_selection():
    candidates = []
    for c in MEMORY.get("scanner_v2_buy", [])[:5]: candidates.append({"symbol": c["symbol"], "side": "BUY", "score": c["score"], "source": "v2"})
    for c in MEMORY.get("scanner_v2_sell", [])[:5]: candidates.append({"symbol": c["symbol"], "side": "SELL", "score": c["score"], "source": "v2"})
    for c in MEMORY.get("rf_watchlist", [])[:10]:
        if c.get("rf_signal") in ("BUY", "SELL"): candidates.append({"symbol": c["symbol"], "side": c["rf_signal"], "score": c["score"], "source": "rf"})
    seen = {}
    for cand in candidates:
        sym = cand["symbol"]
        if sym not in seen or cand["score"] > seen[sym]["score"]: seen[sym] = cand
    candidates = list(seen.values()); best_setup = None; best_score = -1
    for cand in candidates[:15]:
        try:
            sym = cand["symbol"]; side = cand["side"]; df = get_ohlcv_safe(sym, 100)
            if df is None or not validate_dataframe(df, 80): continue
            df.symbol = sym; ob = get_orderbook_cached(sym, limit=10)
            ...
            if smart_money["smart_money_dominant"] and smart_money["institutional_bias"] == side: total_confidence_adjust += 15
            if momentum["trend_expansion"] and momentum["flow_bias"] == side: total_confidence_adjust += 10
            if smart_money["distribution_risk"] > 70: total_confidence_adjust -= 15
            if momentum["momentum_decay"]: total_confidence_adjust -= 12
            if momentum["exhaustion_risk"] > 70: total_confidence_adjust -= 10
            ...
            if adjusted_nscore < 7: continue
            zones = get_smart_zones(sym, df, ob); zone_strength = 0
            if side == "BUY" and zones["buy_zones"]: zone_strength = zones["buy_zones"][0]["strength"]
            elif side == "SELL" and zones["sell_zones"]: zone_strength = zones["sell_zones"][0]["strength"]
            total = adjusted_nscore + zone_strength * 0.5
            if total > best_score: best_score = total; best_setup = (sym, side, total, narrative, zones, df, ob, atr)
        except Exception: continue
    if best_setup and best_score >= 9:
        sym, side, score, narrative, zones, df, ob, atr = best_setup; price = df['close'].iloc[-1]
        leg_class = "REVERSAL"; sl, tp1, tp2 = compute_sl_tp(price, side, leg_class, atr, df)
        reason_str = f"INST_SWEEP+CHOCH+RETEST | nscore={score:.1f}"
        ok = execute_entry(side, sym, price, sl, tp1, tp2, score, reason_str, atr, trade_type="INSTITUTIONAL", entry_type="NARRATIVE", classification="SNIPER")
        if ok: return True
    return False
...
def record_watchlist_entry(symbol, side, narrative, score, smart_money=None, momentum=None):
    ...
    entry = {"symbol": symbol, "side": side, "score": round(score, 2), "state": state, "reasons": reasons_list,
             "trade_type": trade_type, "strength": strength, "last_update": now}
    if smart_money:
...
def cleanup_watchlist(ttl=300):
    ...
    for sym in expired: del MEMORY["watchlist"][sym]

# ========== VWAP ENGINE ==========
def compute_vwap(df):
    tp = (df['high'] + df['low'] + df['close']) / 3; cum_vol = df['volume'].cumsum()
    cum_tp_vol = (tp * df['volume']).cumsum(); vwap = cum_tp_vol / cum_vol; return vwap

def vwap_features(df):
    vwap = compute_vwap(df); price = df['close'].iloc[-1]
...
def detect_exhaustion_zone(df):
    atr = compute_atr(df).iloc[-1]; rsi = compute_rsi(df).iloc[-1]; vw = vwap_features(df); last = df.iloc[-1]
    impulse = (last['high'] - last['low']) >= 1.4 * atr if atr > 0 else False
    stretched = abs(vw["distance"]) >= 0.012; rsi_extreme = rsi >= 70 or rsi <= 30
    if not (impulse and stretched and rsi_extreme): return False, None, None
    if rsi >= 70 and vw["distance"] > 0.012: return True, last['high'], "TOP"
    elif rsi <= 30 and vw["distance"] < -0.012: return True, last['low'], "BOTTOM"
    return False, None, None

def detect_reset(df, zone_price, zone_type):
    last_close = df['close'].iloc[-1]
    if zone_type == "TOP": drop = (zone_price - last_close) / zone_price; return drop >= 0.003
    else: rise = (last_close - zone_price) / zone_price; return rise >= 0.003
...
def confirm_reversal(df, ob, zone_type):
    ...
    body = abs(last['close'] - last['open']); wick_reject = wick > body * 1.5 if body > 0 else False
    macd_hist = compute_macd(df)[2]; macd_flip = macd_first_flip(macd_hist); flow = flow_engine(df)
    flow_agree = (zone_type == "TOP" and flow == "aggressive_sell") or (zone_type == "BOTTOM" and flow == "aggressive_buy")
    obi = orderbook_imbalance(ob); obi_agree = (zone_type == "TOP" and obi < -0.2) or (zone_type == "BOTTOM" and obi > 0.2)
    score = sum([wick_reject, macd_flip, flow_agree, obi_agree]); return score >= 2

def detect_stop_hunt(df):
    pools = build_liquidity_pools(df); swept_high, swept_low = detect_sweep(df, pools); last = df.iloc[-1]
    ...
    if swept_high and reclaim and volume_ok: return True, "SELL"
    elif swept_low and reclaim and volume_ok: return True, "BUY"
    return False, None

def choose_mode(df):
    adx = compute_adx(df).iloc[-1] if len(df) >= 20 else 20; return "TREND" if adx >= 20 else "RANGE"

def smart_decision(df, ob, symbol):
    mode = choose_mode(df); is_hunt, hunt_side = detect_stop_hunt(df)
    if is_hunt and hunt_side: return "STOP_HUNT", hunt_side, {"mode": mode}
...
def find_nearest_opposing_zone(df, side):
    supports, resistances = get_clustered_zones(df, lookback=80, cluster_pct=0.002); price = df['close'].iloc[-1]
    if side == "BUY":
        valid = [r for r in resistances if r > price]
        if valid: nearest = min(valid, key=lambda x: x - price); return nearest, "RESISTANCE"
    else:
        valid = [s for s in supports if s < price]
        if valid: nearest = max(valid, key=lambda x: x); return nearest, "SUPPORT"
    return None, None

def compute_opposing_zone_strength(df, ob, atr, side, zone_price, zone_type):
    score = 0; price = df['close'].iloc[-1]; dist_pct = abs(price - zone_price) / price
    if dist_pct <= 0.002: score += 2
    elif dist_pct <= 0.005: score += 1
    last = df.iloc[-1]; body = abs(last['close'] - last['open']); range_ = last['high'] - last['low']
    if range_ > 0:
        if side == "BUY":
            upper_wick = last['high'] - max(last['open'], last['close'])
            if upper_wick > body * 1.5 and price >= zone_price - 0.002*price: score += 2
        else:
            lower_wick = min(last['open'], last['close']) - last['low']
            if lower_wick > body * 1.5 and price <= zone_price + 0.002*price: score += 2
    vol_state = classify_volume(df)
    if vol_state == "exhaustion": score += 1
    elif vol_state == "neutral" and df['volume'].iloc[-1] < df['volume'].rolling(20).mean().iloc[-1] * 0.8: score += 1
    if side == "BUY":
        if last['high'] > zone_price and last['close'] < zone_price: score += 2
    else:
        if last['low'] < zone_price and last['close'] > zone_price: score += 2
    obi = orderbook_imbalance(ob)
    if side == "BUY" and obi < -0.15: score += 2
    elif side == "SELL" and obi > 0.15: score += 2
    adx_series = compute_adx(df)
    if len(adx_series) >= 2:
        if adx_series.iloc[-1] < adx_series.iloc[-2]: score += 1
    return score
...
def opposing_zone_smart_exit(df, ob, atr, side, entry_price, current_price, state):
    ...
    if zone_price is None: return "HOLD", None
    strength = compute_opposing_zone_strength(df, ob, atr, side, zone_price, zone_type)
    if strength >= 6:
        if not state.get("smart_exit_triggered"): return "EXIT", None
    elif strength >= 4:
        if not state.get("smart_partial_done") and not state.get("smart_exit_triggered"): return "PARTIAL", None
    elif strength >= 2:
        if not state.get("smart_tightened"): return "TIGHTEN", 0.8
    return "HOLD", None
...
def get_di_components(df, period=14):
    ...
    high = df['high']; low = df['low']; close = df['close']
    tr1 = high - low; tr2 = (high - close.shift(1)).abs(); tr3 = (low - close.shift(1)).abs()
    tr = pd.concat([tr1, tr2, tr3], axis=1).max(axis=1); atr = rma(tr, period); atr = atr.clip(lower=1e-9)
    up_move = high.diff(); down_move = -low.diff()
    plus_dm = np.where((up_move > down_move) & (up_move > 0), up_move, 0.0)
    minus_dm = np.where((down_move > up_move) & (down_move > 0), down_move, 0.0)
    plus_dm = pd.Series(plus_dm, index=df.index); minus_dm = pd.Series(minus_dm, index=df.index)
...
def get_vwap_narrative(df):
    vwap = compute_vwap(df); price = df['close'].iloc[-1]
    vwap_last = vwap.iloc[-1]; vwap_prev = vwap.iloc[-2] if len(vwap) > 1 else vwap_last
    distance = (price - vwap_last) / vwap_last if vwap_last != 0 else 0.0
    above = price > vwap_last; below = price < vwap_last
    prev_above = df['close'].iloc[-2] > vwap_prev if len(df) > 1 else above
    reclaim = (not prev_above) and above; reject = prev_above and (not above)
    return {"vwap": vwap_last, "distance": distance, "above": above, "below": below, "reclaim": reclaim, "reject": reject, "slope": vwap_last - vwap_prev}

def compute_enhanced_zone_strength(df, level, zone_type, atr, ob, sweep_detected=False):
    price = df['close'].iloc[-1]; touches = 0; rejection_count = 0; volume_at_touches = []
    for i in range(max(0, len(df)-60), len(df)):
        candle_high = df['high'].iloc[i]; candle_low = df['low'].iloc[i]
        if zone_type == "support":
            if abs(candle_low - level) < atr * 0.5:
                touches += 1
                if i < len(df)-1:
                    next_close = df['close'].iloc[i+1]
                    if next_close > df['close'].iloc[i]: rejection_count += 1
    ...
    if volume_at_touches:
        ...
        if avg_vol_overall > 0: vol_score = min(3.0, avg_vol_touch / avg_vol_overall)
    strength = touches * 1.5 + rejection_count * 2.0 + vol_score
    if sweep_detected: strength += 2.0
    last = df.iloc[-1]; body, range_, upper_wick, lower_wick = candle_metrics(last)
    if zone_type == "support" and lower_wick > body * 1.5 and abs(last['low'] - level) < atr: strength += 2.0
    elif zone_type == "resistance" and upper_wick > body * 1.5 and abs(last['high'] - level) < atr: strength += 2.0
...
def classify_market_narrative(df, ob, atr, side, rf_signal):
    ...
    if plus_di is not None:
        if side == "BUY" and plus_di > minus_di: score += 2.0; reasons.append("DI+ dominance")
        elif side == "SELL" and minus_di > plus_di: score += 2.0; reasons.append("DI- dominance")
        elif abs(plus_di - minus_di) < 5: reasons.append("DI tangled")
    if adx_slope > 1.5: score += 1.5; reasons.append(f"ADX rising ({adx_slope:.1f})")
    elif adx_slope < -1.5: score -= 1.0; reasons.append("ADX falling")
    vwap_n = get_vwap_narrative(df)
    if side == "BUY":
        if vwap_n["above"]: score += 1.5; reasons.append("VWAP above")
        elif vwap_n["reclaim"]: score += 2.0; reasons.append("VWAP reclaim")
    else:
        if vwap_n["below"]: score += 1.5; reasons.append("VWAP below")
        elif vwap_n["reject"]: score += 2.0; reasons.append("VWAP reject")
    pools = build_liquidity_pools(df); swept_h, swept_l = detect_sweep(df, pools)
    sweep_detected = (side == "BUY" and swept_l) or (side == "SELL" and swept_h)
    if sweep_detected: score += 2.5; reasons.append("Liquidity sweep")
    supports, resistances = get_clustered_zones(df, lookback=80, cluster_pct=0.002); zone_strength = 0.0
    if side == "BUY" and supports:
        ...
        if nearest_sup:
            ...
            score += zone_strength * 0.5; reasons.append(f"Zone strength {zone_strength:.1f}")
        ...
        if nearest_res:
            ...
            score += zone_strength * 0.5; reasons.append(f"Zone strength {zone_strength:.1f}")
    bos_up, bos_down = detect_bos(df); struct_shift = detect_structure_shift(df)
    if (side == "BUY" and (bos_up or struct_shift == "bullish_shift")): score += 2.0; reasons.append("Bullish structure")
    elif (side == "SELL" and (bos_down or struct_shift == "bearish_shift")): score += 2.0; reasons.append("Bearish structure")
    vol_state = classify_volume(df)
    if vol_state in ("expansion", "spike"): score += 1.5; reasons.append("Volume expansion")
    elif vol_state == "exhaustion": score -= 1.0; reasons.append("Volume exhaustion")
    if candle_rejection(df, side): score += 1.5; reasons.append("Rejection candle")
    if detect_displacement(df, side, atr, vol_state, body_atr_threshold=0.8, volume_expansion_required=False): score += 1.5; reasons.append("Displacement")
    if rf_signal == side: score += 1.5; reasons.append("RF aligned")
    if adx is not None and adx < 18 and plus_di is not None and abs(plus_di - minus_di) < 6: score = 0; reasons = ["CHOP market (ADX<18 + DI tangled)"]
    if score >= 9.0: classification = "REVERSAL_SNIPER" if (sweep_detected or zone_strength > 5) else "TREND_CONTINUATION"; confidence = "HIGH"
    elif score >= 7.0: classification = "TREND_CONTINUATION" if (bos_up or bos_down or struct_shift) else "ACCUMULATION_LONG" if side == "BUY" else "DISTRIBUTION_SHORT"; confidence = "MEDIUM"
    elif score >= 5.0: classification = "FAKE_BREAKOUT" if not sweep_detected else "LOW_CONFIDENCE"; confidence = "LOW"
    else: classification = "CHOP_NO_TRADE"; confidence = "NO_TRADE"
    return {"classification": classification, "confidence": confidence, "narrative_score": round(score, 2), "reasons": reasons,
            ...
            "adx_slope": adx_slope, "vwap_reclaim": vwap_n["reclaim"], "vwap_reject": vwap_n["reject"]}

def detect_market_regime(df):
    if len(df) < 50: return "RANGE"
    try:
        adx = compute_adx(df).iloc[-1]; plus_di, minus_di, _, _ = get_di_components(df); vwap_n = get_vwap_narrative(df)
        atr = compute_atr(df).iloc[-1]; atr_avg = compute_atr(df).rolling(20).mean().iloc[-1] if len(compute_atr(df))>=20 else atr
        atr_ratio = atr / atr_avg if atr_avg else 1.0
        ema20 = ema(df['close'], 20).iloc[-1]; ema50 = ema(df['close'], 50).iloc[-1] if len(df)>=50 else ema20
        price = df['close'].iloc[-1]; di_delta = abs(plus_di - minus_di)
        if adx < 18 and di_delta < 6: return "CHOP"
        if adx > 20 and di_delta > 5:
            ...
            if bullish_aligned or bearish_aligned: return "TREND"
            if struct == "bullish_shift" and plus_di > minus_di: return "TREND"
            if struct == "bearish_shift" and minus_di > plus_di: return "TREND"
        if adx > 20 and atr_ratio > 1.4: return "EXPANSION"
        if atr_ratio < 0.7 and adx < 25: return "COMPRESSION"
        return "RANGE"
    except: return "RANGE"
...
def get_trend_direction(df):
    try:
        plus_di, minus_di, _, _ = get_di_components(df)
        ema20 = ema(df['close'], 20).iloc[-1]; ema50 = ema(df['close'], 50).iloc[-1] if len(df)>=50 else ema20
        price = df['close'].iloc[-1]; struct = detect_structure_shift(df)
        if (plus_di > minus_di and ema20 > ema50 and price > ema20) or struct == "bullish_shift": return "BULLISH"
        elif (minus_di > plus_di and ema20 < ema50 and price < ema20) or struct == "bearish_shift": return "BEARISH"
        return "NEUTRAL"
    except: return "NEUTRAL"

def adjust_narrative_confidence(narrative, regime, side, trend_direction):
    orig_conf = narrative["confidence"]; score = narrative["narrative_score"]; side_aligned = False
    if (trend_direction == "BULLISH" and side == "BUY") or (trend_direction == "BEARISH" and side == "SELL"): side_aligned = True
    final_conf = orig_conf; final_class = narrative["classification"]
    if regime == "CHOP": return "NO_TRADE", "CHOP_NO_TRADE"
    if orig_conf == "NO_TRADE" or score < 5.0: return "NO_TRADE", "CHOP_NO_TRADE"
    if regime == "TREND":
        if side_aligned:
            if orig_conf == "HIGH": final_conf = "HIGH"; final_class = "SNIPER"
            elif orig_conf == "MEDIUM": final_conf = "MEDIUM"; final_class = "TREND"
            elif orig_conf == "LOW":
                if score >= 5.0: final_conf = "MEDIUM"; final_class = "TREND"
                else: final_conf = "NO_TRADE"; final_class = "NO_TRADE"
        else:
            if orig_conf == "HIGH": final_conf = "HIGH"; final_class = "SNIPER"
            else: final_conf = "NO_TRADE"; final_class = "NO_TRADE"
        ...
        if orig_conf == "HIGH": final_conf = "HIGH"; final_class = "SNIPER"
        elif orig_conf == "MEDIUM" and side_aligned: final_conf = "NO_TRADE"; final_class = "NO_TRADE"
        else: final_conf = "NO_TRADE"; final_class = "NO_TRADE"
    return final_conf, final_class

def evaluate_with_narrative(symbol, side, price, atr_val, df, ob, rf_signal, existing_score=0):
    regime = detect_market_regime(df); trend_dir = get_trend_direction(df)
    ...
    narrative["confidence"] = final_conf; narrative["classification"] = final_class; narrative["regime"] = regime
    ...
    if not should_enter:
        ...
        MEMORY.setdefault("no_entry_feed", []).append({"time": time.time(), "symbol": symbol, "side": side, "reason": reason, "score": narrative["narrative_score"]})
        if len(MEMORY["no_entry_feed"]) > 20: MEMORY["no_entry_feed"] = MEMORY["no_entry_feed"][-20:]
        return False, None, narrative
    STATE["narrative_classification"] = final_class; STATE["narrative_confidence"] = narrative["narrative_score"]; STATE["confidence_level"] = final_conf
...
def narrative_debug():
    ...
    for key, val in MEMORY.items():
        if key.startswith("last_narrative_"):
            debug_data.append({"symbol": key.replace("last_narrative_", ""), "side": val.get("side"), "classification": val.get("classification"),
                               "confidence": val.get("confidence"), "score": val.get("narrative_score"), "reasons": val.get("reasons"), "timestamp": val.get("timestamp")})
    ...
    for cand in radar_candidates:
        ...
        if not any(d["symbol"] == sym for d in debug_data):
            df = get_ohlcv_safe(sym, 100)
            if df is not None:
                ob = get_orderbook_cached(sym, 10); atr = compute_atr(df).iloc[-1] if len(df) > 14 else df['close'].iloc[-1] * 0.01
                side = "BUY"; narrative = classify_market_narrative(df, ob, atr, side, None)
                debug_data.append({"symbol": sym, "side": "analysis", "classification": narrative["classification"], "confidence": narrative["confidence"],
                                   "score": narrative["narrative_score"], "reasons": narrative["reasons"][:5], "timestamp": time.time()})
...
def check_institutional_entry(symbol, side, df, ob, atr, price):
    ...
    reasons = []; pools = build_liquidity_pools(df)
    swept_high, swept_low = detect_sweep(df, pools)
    sweep_ok = (side == "BUY" and swept_low) or (side == "SELL" and swept_high)
    if not sweep_ok: return False, None, "No liquidity sweep"
    reasons.append("Sweep")
    zones = get_smart_zones(symbol, df, ob); zone_ok = False; zone_price = None
    if side == "BUY":
        if zones["buy_zones"] and zones["buy_zones"][0]["strength"] >= 5:
            zone_price = zones["buy_zones"][0]["price"]
            if abs(price - zone_price) / price < 0.003: zone_ok = True; reasons.append(f"Buy zone {zone_price:.4f} (strength {zones['buy_zones'][0]['strength']})")
    else:
        if zones["sell_zones"] and zones["sell_zones"][0]["strength"] >= 5:
            zone_price = zones["sell_zones"][0]["price"]
            if abs(price - zone_price) / price < 0.003: zone_ok = True; reasons.append(f"Sell zone {zone_price:.4f} (strength {zones['sell_zones'][0]['strength']})")
    if not zone_ok:
        fvg = detect_fvg(df)
        if side == "BUY" and fvg and fvg[0] == "bullish":
            if price >= fvg[1] and price <= fvg[2]: zone_ok = True; reasons.append("Bullish FVG")
        elif side == "SELL" and fvg and fvg[0] == "bearish":
            if price >= fvg[1] and price <= fvg[2]: zone_ok = True; reasons.append("Bearish FVG")
    if not zone_ok:
        ob_level = detect_order_block(df, side)
        if side == "BUY" and ob_level:
            if abs(price - ob_level["low"]) / price < 0.003: zone_ok = True; reasons.append("Bullish OB")
        elif side == "SELL" and ob_level:
            if abs(price - ob_level["high"]) / price < 0.003: zone_ok = True; reasons.append("Bearish OB")
    if not zone_ok: return False, None, "No strong zone tap"
    struct_shift = detect_structure_shift(df); bos_up, bos_down = detect_bos(df); choch_ok = False
    if side == "BUY" and (struct_shift == "bullish_shift" or bos_up): choch_ok = True; reasons.append("Bullish MSS/CHoCH")
    elif side == "SELL" and (struct_shift == "bearish_shift" or bos_down): choch_ok = True; reasons.append("Bearish MSS/CHoCH")
    if sweep_ok and not choch_ok: return False, None, "Reversal requires MSS/CHoCH confirmation"
    elif not sweep_ok and not choch_ok: reasons.append("No MSS/CHoCH (trend continuation, optional)")
    rejection_ok = candle_rejection(df, side); vol_state = classify_volume(df)
    displacement_ok = detect_displacement(df, side, atr, vol_state, body_atr_threshold=0.8, volume_expansion_required=False)
    if not (rejection_ok or displacement_ok): return False, None, "No rejection/displacement candle"
    if rejection_ok: reasons.append("Rejection candle")
    if displacement_ok: reasons.append("Displacement")
    volume_ok = vol_state in ("expansion", "spike")
    if not volume_ok: return False, None, "No volume expansion"
    reasons.append(f"Volume {vol_state}")
    adx_series = compute_adx(df)
    if len(adx_series) < 3: return False, None, "Insufficient ADX data"
    adx_now = adx_series.iloc[-1]; adx_prev = adx_series.iloc[-2]; adx_slope = adx_now - adx_prev
    plus_di, minus_di, _, _ = get_di_components(df)
    di_spread = (plus_di - minus_di) if side == "BUY" else (minus_di - plus_di)
    if adx_now < 18: return False, None, f"ADX too low ({adx_now:.1f})"
    if adx_now > 50:
        if adx_slope > 0 and di_spread > 8: reasons.append(f"Strong trend ADX={adx_now:.1f} slope={adx_slope:.1f} DI_spread={di_spread:.1f}")
        else: return False, None, f"Exhaustion risk: ADX>50 but slope={adx_slope:.1f} DI_spread={di_spread:.1f}"
    elif adx_now > 35:
        if adx_slope > 0: reasons.append(f"Strong trend ADX={adx_now:.1f} slope={adx_slope:.1f}")
        else: return False, None, f"ADX high but falling slope ({adx_now:.1f} slope={adx_slope:.1f})"
    else:
        if adx_slope > 0: reasons.append(f"Healthy ADX={adx_now:.1f} rising")
        else: return False, None, f"ADX not rising ({adx_now:.1f} slope={adx_slope:.1f})"
    rf = RFEngine(20, 3.5).compute(df)
    if rf["signal"] != side: return False, None, f"RF signal {rf['signal']} does not match {side}"
    if abs(rf["distance"]) > 0.003: return False, None, f"RF distance {rf['distance']:.4f} too far"
    reasons.append("RF aligned")
    if zone_price:
        move_from_zone = abs(price - zone_price) / zone_price * 100
        if move_from_zone > 0.5: return False, None, f"Price moved {move_from_zone:.2f}% from zone, too late"
    last_candle = df.iloc[-1]; candle_range_pct = (last_candle['high'] - last_candle['low']) / last_candle['close'] * 100
    if candle_range_pct > 1.5 * (atr / price * 100): return False, None, "Large displacement candle already occurred, too late"
    reason_str = " | ".join(reasons)
    return True, "INSTITUTIONAL_SNIPER", reason_str

# ========== DECISION FUNCTIONS ==========
def decision_score_v1(df, ob, atr_val, side):
    es, reasons = early_score(df, ob, atr_val, side); ctx = detect_liquidity_context(df)
    scenario = "TREND"; direction = side
    if ctx == "sell_side_taken" and side == "BUY": scenario = "REVERSAL"
    elif ctx == "buy_side_taken" and side == "SELL": scenario = "REVERSAL"
...
def decide_and_execute_v1(symbol, side, total_score, reasons, price, sl, tp1, tp2):
    ...
    ob = get_orderbook_cached(symbol, 10); atr_val = compute_atr(df).iloc[-1] if len(df) > 14 else price * 0.01
    should_enter, classification, narrative = evaluate_with_narrative(symbol, side, price, atr_val, df, ob, side)
    if not should_enter: return False
    reason_str = f"DECISION_V1 score={total_score} reasons={reasons} | NARR={narrative['classification']}"
    return execute_entry(side, symbol, price, sl, tp1, tp2, total_score, reason_str, atr_val, trade_type="DECISION_V1", entry_type="V1", classification=classification)

def decision_score(df, ob, atr_val, side):
    vol_state = classify_volume(df); scenario = advanced_detect_scenario(df, side, atr_val, vol_state)
    es, reasons = early_score(df, ob, atr_val, side); total = es
    if scenario == "TRAP_REVERSAL": total += 3
    elif scenario == "TREND_CONTINUATION": total += 2
    total = min(10, max(0, total)); direction = side
...
def monitor_watchlist():
    ...
    for c in watchlist:
        sym = c["symbol"]; df = get_ohlcv_safe(sym, 150)
        if df is None or not validate_dataframe(df, 100): continue
        ob = get_orderbook_cached(sym, limit=10)
        if ob is not None:
            price = df['close'].iloc[-1]; atr_val = compute_atr(df).iloc[-1] if len(df) > 14 else price * 0.01
            for side_try in ("BUY", "SELL"):
                ...
                if should_enter:
                    ...
                    if not should_enter_narr: continue
                    sl, tp1, tp2 = compute_sl_tp(price, side_try, "REVERSAL", atr_val, df)
                    ok = execute_entry(side_try, sym, price, sl, tp1, tp2, 85, reason_str, atr_val, trade_type="INSTITUTIONAL_V3", entry_type="SMART_EARLY", classification=classification)
                    if ok: return True
            ...
            if decision == "STOP_HUNT":
                ...
                ok = execute_entry(dec_side, sym, price, sl, tp1, tp2, 8, reason_str, atr_val, trade_type="SMART", entry_type="STOP_HUNT", classification=classification)
                if ok: return True
                ...
                ok = execute_entry(dec_side, sym, price, sl, tp1, tp2, 8, reason_str, atr_val, trade_type="SMART", entry_type="EXHAUSTION", classification=classification)
                if ok: return True
            ...
            price = df['close'].iloc[-1]; atr_val = compute_atr(df).iloc[-1] if len(df) > 14 else price * 0.01
            adx_series = compute_adx(df); adx_val = adx_series.iloc[-1] if adx_series is not None else 20.0
            ...
            if ob_v1 is not None:
                ...
                if dir_v1 and total_v1 >= 5:
                    sl_v1, tp1_v1, tp2_v1 = compute_sl_tp(price, dir_v1, "REVERSAL" if scn_v1 in ("TRAP","REVERSAL") else "EARLY_TREND", atr_val, df)
            ...
            if ob is not None:
                ...
                if total_score >= 7:
                    ...
                    ok = execute_entry(scenario_dir, sym, price, sl, tp1, tp2, total_score, reason_str, atr_val, trade_type="SCENARIO_ENGINE", entry_type="UNIFIED_SNIPER", classification=classification)
                    if ok: return True
                elif total_score >= 5:
                    ...
                    ok = execute_entry(scenario_dir, sym, price, sl, tp1, tp2, total_score, reason_str, atr_val, trade_type="SCENARIO_ENGINE", entry_type="UNIFIED_EARLY", classification=classification)
                    if ok: return True
            ob = get_orderbook_cached(sym, limit=10)
            if ob is None: continue
                ...
                if early_score_val >= 6:
                    ...
                    ok = execute_entry(side, sym, price, sl, tp1, tp2, early_score_val, reason_str, atr_val, trade_type="EARLY_ENGINE", entry_type="EARLY_SNIPER", classification=classification)
                    if ok: return True
                elif early_score_val >= 4:
                    sl, tp1, tp2 = compute_sl_tp(price, side, "EARLY_TREND", atr_val, df)
                    reason_str = f"EARLY_ENTRY ({','.join(early_reasons)}) score={early_score_val} | NARR={narrative['classification']}"
                    ok = execute_entry(side, sym, price, sl, tp1, tp2, early_score_val, reason_str, atr_val, trade_type="EARLY_ENGINE", entry_type="EARLY_ENTRY", classification=classification)
                    if ok: return True
            supports, resistances = get_clustered_zones(df, lookback=120, cluster_pct=0.002)
            location = detect_location(df, price, supports, resistances, threshold=0.003)
            if side == "BUY" and location != "LOW": continue
            if side == "SELL" and location != "HIGH": continue
            ...
            if scenario == "TRAP_REVERSAL": leg_class = "REVERSAL"
            elif scenario == "TREND_CONTINUATION": leg_class = "EARLY_TREND"
            else: leg_class = "TREND_CONTINUATION"
...
def get_last_price(symbol): return get_ticker_safe(symbol)

def update_trailing_simple(current_price):
    if not TRADE_STATE["trail_on"]: return False
    entry = TRADE_STATE["entry"]; side = TRADE_STATE["side"]
    if "trail_stop" not in TRADE_STATE:
        if side == "BUY": TRADE_STATE["trail_stop"] = entry * 1.005
        else: TRADE_STATE["trail_stop"] = entry * 0.995
    if side == "BUY":
